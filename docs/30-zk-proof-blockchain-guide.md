# 零知识证明 (ZK Proof) 区块链实战指南

## 概述

本文档旨在让你**快速理解并应用**零知识证明技术。我们跳过繁琐的数学推导，聚焦于：

1. 核心概念的直觉理解
2. 主流ZK系统的工程选型
3. 区块链中的实际应用
4. 可运行的代码示例

---

## 第一部分：ZK核心概念

### 1.1 什么是零知识证明？

**一句话定义**：证明者（Prover）向验证者（Verifier）证明某个陈述为真，但不泄露任何额外信息。

```
┌─────────────────────────────────────────────────────────────────────┐
│                    零知识证明的三个核心属性                             │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1. 完备性 (Completeness)                                            │
│     如果陈述为真，诚实的证明者总能说服验证者                               │
│                                                                     │
│  2. 可靠性 (Soundness)                                               │
│     如果陈述为假，恶意证明者几乎不可能欺骗验证者                           │
│                                                                     │
│  3. 零知识性 (Zero-Knowledge)                                        │
│     验证者除了知道"陈述为真"外，学不到任何其他信息                         │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 1.2 经典比喻：阿里巴巴洞穴

```
┌─────────────────────────────────────────────────────────────────────┐
│                      阿里巴巴洞穴问题                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│                        入口                                          │
│                         │                                           │
│                    ┌────┴────┐                                      │
│                    │         │                                      │
│               A 路径         B 路径                                  │
│                    │         │                                      │
│                    │         │                                      │
│                    └────┬────┘                                      │
│                         │                                           │
│                    魔法门 🚪                                         │
│                  (需要密码)                                          │
│                                                                     │
│  证明过程：                                                          │
│  1. Peggy（证明者）随机选择A或B进入洞穴                                 │
│  2. Victor（验证者）在入口喊出要求Peggy从A或B出来                        │
│  3. 如果Peggy知道密码，她总能从指定路径出来                              │
│  4. 如果不知道密码，她只有50%概率成功                                    │
│  5. 重复n次，作弊成功概率 = (1/2)^n                                    │
│                                                                     │
│  零知识：Victor只知道Peggy能打开门，但不知道密码是什么                     │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 1.3 区块链中的ZK应用场景

```
┌─────────────────────────────────────────────────────────────────────┐
│                    ZK 在区块链中的核心应用                             │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  1. 扩容 (Scaling) - zkRollup                                │    │
│  │     • 链下执行交易，链上验证证明                                 │    │
│  │     • 代表项目：zkSync, StarkNet, Scroll, Polygon zkEVM       │    │
│  │     • 核心价值：继承L1安全性，大幅提高TPS                        │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  2. 隐私 (Privacy)                                           │    │
│  │     • 隐藏交易金额、发送方、接收方                               │    │
│  │     • 代表项目：Zcash, Tornado Cash, Aztec                    │    │
│  │     • 核心价值：交易可验证但不可追踪                             │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  3. 跨链桥 (Bridges)                                         │    │
│  │     • 证明源链上的状态/交易                                    │    │
│  │     • 代表项目：Succinct, Polyhedra, zkBridge                │    │
│  │     • 核心价值：无需信任假设的跨链验证                           │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  4. 身份验证 (Identity)                                      │    │
│  │     • 证明"我满足条件"而不透露具体信息                           │    │
│  │     • 例如：证明年龄>18，不透露具体生日                          │    │
│  │     • 代表项目：Worldcoin, Polygon ID, Semaphore              │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 1.4 ZK 证明系统分类

```
┌─────────────────────────────────────────────────────────────────────┐
│                      ZK 证明系统家族                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│                         ZK Proofs                                   │
│                             │                                       │
│              ┌──────────────┼──────────────┐                        │
│              │              │              │                        │
│             交互式         非交互式         其他                       │
│        (Interactive)  (Non-Interactive)                             │
│                             │                                       │
│              ┌──────────────┼──────────────┐                        │
│              │              │              │                        │
│           SNARKs        STARKs         Bulletproofs                 │
│              │              │              │                        │
│     ┌────────┼────────┐     │         范围证明                       │
│     │        │        │     │         (Monero)                      │
│  Groth16  PLONK    Halo2    │                                       │
│     │        │        │     │                                       │
│  zkSync1  Scroll   Zcash   StarkNet                                 │
│  Zcash    Polygon  Aztec   StarkEx                                  │
│           zkEVM                                                     │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 1.5 SNARKs vs STARKs：工程选型

| 特性         | SNARKs                                         | STARKs                                     |
|------------|------------------------------------------------|--------------------------------------------|
| **全称**     | Succinct Non-interactive ARgument of Knowledge | Scalable Transparent ARgument of Knowledge |
| **可信设置**   | 需要 (Groth16) 或不需要 (PLONK+KZG)                  | 不需要 (Transparent)                          |
| **证明大小**   | ~200-300 bytes                                 | ~40-200 KB                                 |
| **验证时间**   | ~10ms (恒定)                                     | ~50-100ms (对数增长)                           |
| **证明时间**   | 较慢                                             | 较快                                         |
| **后量子安全**  | ❌ (基于椭圆曲线)                                     | ✅ (基于哈希函数)                                 |
| **链上验证成本** | ~200k gas                                      | ~1M+ gas                                   |
| **代表项目**   | zkSync, Scroll, Polygon zkEVM                  | StarkNet, StarkEx                          |

**工程选型建议**：

```
┌─────────────────────────────────────────────────────────────────────┐
│                         如何选择？                                    │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  优先考虑链上验证成本？                                                 │
│  └── 是 → SNARKs (Groth16/PLONK)                                    │
│                                                                     │
│  需要后量子安全？                                                     │
│  └── 是 → STARKs                                                    │
│                                                                     │
│  不想要可信设置？                                                     │
│  └── 是 → STARKs 或 PLONK (universal setup)                         │
│                                                                     │
│  证明规模超大？                                                       │
│  └── 是 → STARKs (证明时间更优)                                      │
│                                                                     │
│  对延迟敏感？                                                         │
│  └── 是 → SNARKs (验证更快)                                          │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 1.6 ZK 电路：核心抽象

**理解ZK的关键**：将计算转换为"电路"

```
┌─────────────────────────────────────────────────────────────────────┐
│                    从计算到电路的转换                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  原始问题：证明我知道x，使得hash(x) = y                                 │
│                                                                     │
│  步骤 1: 将计算表示为算术电路                                           │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │                                                             │    │
│  │     x (私有输入)      y (公开输入)                             │    │
│  │         │                 │                                 │    │
│  │         ▼                 │                                 │    │
│  │    ┌─────────┐            │                                 │    │
│  │    │  Hash   │            │                                 │    │
│  │    │ Circuit │            │                                 │    │
│  │    └────┬────┘            │                                 │    │
│  │         │                 │                                 │    │
│  │         ▼                 ▼                                 │    │
│  │    ┌─────────────────────────┐                              │    │
│  │    │     Constraint:         │                              │    │
│  │    │   hash(x) === y         │                              │    │
│  │    └─────────────────────────┘                              │    │
│  │                                                             │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  步骤 2: 电路编译为约束系统 (R1CS / AIR / Plonkish)                     │
│                                                                     │
│  步骤 3: 生成证明 (Prover) / 验证证明 (Verifier)                       │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 1.7 约束系统简介

#### R1CS (Rank-1 Constraint System)

SNARKs 最常用的约束系统：

```
每个约束的形式：(A · x) * (B · x) = (C · x)

其中：
- x 是所有变量的向量（包括输入、输出、中间变量）
- A, B, C 是系数矩阵的行向量

示例：证明 x * y = z

变量向量: [1, x, y, z]  (1 是常数项)

约束: x * y = z
转换为 R1CS:
A = [0, 1, 0, 0]  → A · x = x
B = [0, 0, 1, 0]  → B · x = y
C = [0, 0, 0, 1]  → C · x = z

验证: (A · x) * (B · x) = x * y = z = (C · x) ✓
```

#### Plonkish 约束

PLONK 系列使用的更灵活的约束系统：

```
通用门约束：
q_L · a + q_R · b + q_O · c + q_M · (a * b) + q_C = 0

其中：
- a, b, c 是门的输入/输出
- q_L, q_R, q_O, q_M, q_C 是选择器（决定门类型）

示例：
加法门 (a + b = c):  q_L=1, q_R=1, q_O=-1, q_M=0, q_C=0
乘法门 (a * b = c):  q_L=0, q_R=0, q_O=-1, q_M=1, q_C=0
常数门 (c = 5):      q_L=0, q_R=0, q_O=1, q_M=0, q_C=-5
```

### 1.8 ZK 证明生成流程

```
┌─────────────────────────────────────────────────────────────────────┐
│                    ZK 证明完整流程                                    │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │                    1. 电路开发阶段                            │    │
│  │  ┌─────────┐    ┌─────────┐    ┌─────────┐                  │    │
│  │  │ 业务逻辑 │───▶│ DSL 代码 │───▶│ 编译器   │                  │    │
│  │  │         │    │(Circom/ │    │         │                  │    │
│  │  │         │    │ Halo2)  │    │         │                  │    │
│  │  └─────────┘    └─────────┘    └────┬────┘                  │    │
│  │                                     │                       │    │
│  │                                     ▼                       │    │
│  │                              ┌─────────────┐                │    │
│  │                              │ 约束系统     │                │    │
│  │                              │ (R1CS/AIR)  │                │    │
│  │                              └─────────────┘                │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                      │                              │
│                                      ▼                              │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │                    2. 设置阶段 (Setup)                       │    │
│  │                                                             │    │
│  │  可信设置 (Groth16)    或    通用设置 (PLONK)    或    无设置   │    │
│  │       │                         │                    │      │    │
│  │       ▼                         ▼                    │      │    │
│  │   特定电路的               可复用的 SRS                 │      │    │
│  │   证明密钥/验证密钥        (Structured Reference        │      │    │
│  │   (pk, vk)                 String)                   │      │    │
│  │                                                      │      │    │
│  │  ⚠️ 毒性废料问题           ✅ 一次设置多次使用        ✅ STARKs │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                      │                              │
│                                      ▼                              │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │                    3. 证明生成 (Prove)                        │    │
│  │                                                              │    │
│  │  输入：                                                       │    │
│  │  • 电路 + 证明密钥                                            │    │
│  │  • 公开输入 (public inputs)                                   │    │
│  │  • 私有输入 (private inputs / witness)                        │    │
│  │                                                              │    │
│  │  输出：                                                       │    │
│  │  • 证明 (proof)                                               │    │
│  │                                                              │    │
│  │  计算复杂度：O(n log n) ~ O(n²)，通常需要秒到分钟级别             │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                      │                              │
│                                      ▼                              │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │                    4. 验证 (Verify)                          │    │
│  │                                                              │    │
│  │  输入：                                                       │    │
│  │  • 验证密钥 (vk)                                              │    │
│  │  • 公开输入                                                   │    │
│  │  • 证明                                                       │    │
│  │                                                              │    │
│  │  输出：                                                       │    │
│  │  • true / false                                              │    │
│  │                                                              │    │
│  │  计算复杂度：O(1) 或 O(log n)，毫秒级别                         │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 1.9 关键术语速查表

| 术语                        | 含义                                 | 类比        |
|---------------------------|------------------------------------|-----------|
| **Witness**               | 私有输入（证明者知道但不公开）                    | 你知道的密码    |
| **Public Input**          | 公开输入（验证者也能看到）                      | 公开的用户名    |
| **Circuit**               | 计算逻辑的电路表示                          | 程序代码      |
| **Constraint**            | 约束（必须满足的等式）                        | assert 语句 |
| **Proving Key (pk)**      | 生成证明需要的密钥                          | 私钥        |
| **Verification Key (vk)** | 验证证明需要的密钥                          | 公钥        |
| **SRS**                   | Structured Reference String，通用设置参数 | 共享的配置     |
| **Trusted Setup**         | 可信设置仪式                             | 密钥生成仪式    |
| **Soundness**             | 假证明被接受的概率极低                        | 防伪能力      |
| **Completeness**          | 真证明总能被接受                           | 可用性       |
| **Succinctness**          | 证明很小，验证很快                          | 压缩率       |

---

## 第二部分：数学基础（最小必要知识）

> 本节只讲**必须理解**的数学概念，跳过所有可以"黑盒使用"的部分。

### 2.1 有限域 (Finite Field)

**为什么需要？** ZK 证明中所有运算都在有限域上进行，而不是普通整数。

```
┌─────────────────────────────────────────────────────────────────────┐
│                       有限域 F_p 快速理解                             │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  定义：所有运算结果都 mod p 的整数集合 {0, 1, 2, ..., p-1}               │
│                                                                     │
│  示例：F_7 = {0, 1, 2, 3, 4, 5, 6}                                   │
│                                                                     │
│  加法：5 + 4 = 9 mod 7 = 2                                           │
│  乘法：5 * 4 = 20 mod 7 = 6                                          │
│  减法：2 - 5 = -3 mod 7 = 4                                          │
│  除法：2 / 5 = 2 * 5^(-1) = 2 * 3 = 6  (因为 5 * 3 = 15 mod 7 = 1)   │
│                                                                     │
│  关键性质：                                                          │
│  • 加减乘除都封闭（结果仍在域内）                                        │
│  • 没有溢出问题                                                       │
│  • p 必须是素数                                                      │
│                                                                     │
│  常用素数（区块链中）：                                                │
│  • BN254:  21888242871839275222246405745257275088548364400416034... │
│  • BLS12-381: 5231867187480318...                                   │
│  • Goldilocks: 2^64 - 2^32 + 1 (用于 Plonky2)                        │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

**Go 代码示例**：

```go
package main

import (
	"fmt"
	"math/big"
)

// FiniteField 有限域
type FiniteField struct {
	p *big.Int // 素数模数
}

// NewFiniteField 创建有限域
func NewFiniteField(p *big.Int) *FiniteField {
	return &FiniteField{p: p}
}

// Add 域加法
func (f *FiniteField) Add(a, b *big.Int) *big.Int {
	result := new(big.Int).Add(a, b)
	return result.Mod(result, f.p)
}

// Mul 域乘法
func (f *FiniteField) Mul(a, b *big.Int) *big.Int {
	result := new(big.Int).Mul(a, b)
	return result.Mod(result, f.p)
}

// Inv 乘法逆元 (使用费马小定理: a^(-1) = a^(p-2) mod p)
func (f *FiniteField) Inv(a *big.Int) *big.Int {
	exp := new(big.Int).Sub(f.p, big.NewInt(2))
	return new(big.Int).Exp(a, exp, f.p)
}

// Div 域除法
func (f *FiniteField) Div(a, b *big.Int) *big.Int {
	bInv := f.Inv(b)
	return f.Mul(a, bInv)
}

func main() {
	// 使用小素数演示
	p := big.NewInt(7)
	field := NewFiniteField(p)

	a := big.NewInt(5)
	b := big.NewInt(4)

	fmt.Printf("F_%d:\n", p)
	fmt.Printf("%d + %d = %d\n", a, b, field.Add(a, b))                         // 5 + 4 = 2
	fmt.Printf("%d * %d = %d\n", a, b, field.Mul(a, b))                         // 5 * 4 = 6
	fmt.Printf("%d / %d = %d\n", big.NewInt(2), a, field.Div(big.NewInt(2), a)) // 2 / 5 = 6
}
```

### 2.2 椭圆曲线基础

**为什么需要？** SNARKs 的安全性基于椭圆曲线离散对数问题。

```
┌─────────────────────────────────────────────────────────────────────┐
│                      椭圆曲线快速理解                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  方程：y² = x³ + ax + b (在有限域 F_p 上)                              │
│                                                                     │
│  曲线上的点形成一个群：                                                 │
│  • 单位元：无穷远点 O (点 + O = 点)                                    │
│  • 加法：两点连线与曲线第三交点的关于 x 轴对称点                           │
│  • 标量乘法：n * P = P + P + ... + P (n 次)                           │
│                                                                     │
│  关键操作（你需要知道的）：                                              
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  点加法：P + Q = R                                           │    │
│  │  标量乘法：n * G = P  (G 是生成元)                             │    │
│  │                                                             │    │
│  │  核心性质（离散对数难题）：                                     │    │
│  │  • 已知 G 和 n，计算 P = n * G 很容易                          │    │
│  │  • 已知 G 和 P，求 n 几乎不可能                                │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  常用曲线：                                                          │
│  • BN254 (alt_bn128)：以太坊预编译合约支持                             │
│  • BLS12-381：以太坊 2.0 签名                                         │
│  • secp256k1：比特币/以太坊账户签名                                     │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 2.3 多项式承诺 (Polynomial Commitment)

**为什么需要？** 这是现代 ZK 证明系统的核心构建块。

```
┌─────────────────────────────────────────────────────────────────────┐
│                     多项式承诺核心概念                                 │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  问题：我有一个多项式 f(x)，想让你相信 f(a) = b，但不告诉你 f(x)           │
│                                                                     │
│  方案：多项式承诺                                                     │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  1. Commit: 生成多项式的承诺 C = Commit(f)                    │    │
│  │     • C 很小（一个群元素）                                    │    │
│  │     • 无法从 C 反推 f                                        │    │
│  │     • 绑定性：C 唯一确定 f                                    │    │
│  │                                                             │    │
│  │  2. Open: 证明 f(a) = b                                      │    │
│  │     • 生成证明 π                                             │    │
│  │     • 验证者检查 Verify(C, a, b, π)                          │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  主流方案：                                                          │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  KZG (Kate-Zaverucha-Goldberg)                              │    │
│  │  • 需要可信设置                                               │    │
│  │  • 证明大小：1 个群元素 (~48 bytes)                            │    │
│  │  • 验证：2 次配对运算                                          │    │
│  │  • 用于：PLONK, Scroll, Polygon zkEVM                        │    │
│  └─────────────────────────────────────────────────────────────┘    │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  FRI (Fast Reed-Solomon IOP)                                │    │
│  │  • 无需可信设置                                               │    │
│  │  • 证明大小：O(log²n) 个哈希                                   │    │
│  │  • 后量子安全                                                 │    │
│  │  • 用于：STARKs, Plonky2                                     │    │
│  └─────────────────────────────────────────────────────────────┘    │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  IPA (Inner Product Argument)                               │    │
│  │  • 无需可信设置                                               │    │
│  │  • 证明大小：O(log n) 个群元素                                 │    │
│  │  • 用于：Halo2, Bulletproofs                                 │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 2.4 KZG 承诺简化理解

```
┌─────────────────────────────────────────────────────────────────────┐
│                      KZG 承诺工作原理                                 │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  设置阶段（可信设置）：                                                 │
│  • 选择秘密值 τ（设置后必须销毁）                                       │
│  • 计算 SRS = [G, τG, τ²G, τ³G, ..., τⁿG]                           │
│    其中 G 是椭圆曲线生成元                                             │
│                                                                     │
│  承诺阶段：                                                          │
│  • 多项式 f(x) = a₀ + a₁x + a₂x² + ... + aₙxⁿ                        │
│  • 承诺 C = a₀G + a₁(τG) + a₂(τ²G) + ... = f(τ)·G                   │
│    注意：我们不知道 τ，但能计算 f(τ)·G                                  │
│                                                                     │
│  证明 f(z) = y：                                                     │
│  • 构造商多项式 q(x) = (f(x) - y) / (x - z)                           │
│  • 证明 π = q(τ)·G                                                   │
│                                                                     │
│  验证：                                                              │
│  • 使用配对运算检查 e(C - yG, G) = e(π, τG - zG)                       │
│  • 等价于检查 f(τ) - y = q(τ) · (τ - z)                               │
│  • 即 f(τ) - y = (f(τ) - y)，恒成立当且仅当 f(z) = y                   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

**Go 代码示例**（概念演示）：

```go
package main

import (
	"github.com/consensys/gnark-crypto/ecc/bn254"
	"github.com/consensys/gnark-crypto/ecc/bn254/fr"
)

// KZGSetup 表示 KZG 设置参数
type KZGSetup struct {
	G1Powers []bn254.G1Affine // [G, τG, τ²G, ...]
	G2Tau    bn254.G2Affine   // τ·G2
}

// Polynomial 表示多项式
type Polynomial []fr.Element

// Commit 计算多项式承诺
func (setup *KZGSetup) Commit(poly Polynomial) bn254.G1Affine {
	var commitment bn254.G1Affine

	// C = Σ aᵢ · (τⁱ·G)
	for i, coeff := range poly {
		var term bn254.G1Affine
		term.ScalarMultiplication(&setup.G1Powers[i], coeff.BigInt(new(big.Int)))
		commitment.Add(&commitment, &term)
	}

	return commitment
}

// CreateProof 创建 f(z) = y 的证明
func (setup *KZGSetup) CreateProof(poly Polynomial, z, y fr.Element) bn254.G1Affine {
	// 计算商多项式 q(x) = (f(x) - y) / (x - z)
	quotient := dividePolynomial(poly, z, y)

	// 返回 q(τ)·G
	return setup.Commit(quotient)
}

// Verify 验证证明
func (setup *KZGSetup) Verify(
	commitment bn254.G1Affine,
	z, y fr.Element,
	proof bn254.G1Affine,
) bool {
	// 验证: e(C - y·G, G2) = e(proof, τ·G2 - z·G2)

	var yG, zG2 bn254.G1Affine
	var g2Gen bn254.G2Affine
	g2Gen.FromJacobian(&bn254.G2Affine{}.Generator())

	// C - y·G
	yG.ScalarMultiplication(&setup.G1Powers[0], y.BigInt(new(big.Int)))
	var lhs bn254.G1Affine
	lhs.Sub(&commitment, &yG)

	// τ·G2 - z·G2
	zG2.ScalarMultiplication(&g2Gen, z.BigInt(new(big.Int)))
	var rhs bn254.G2Affine
	rhs.Sub(&setup.G2Tau, &zG2)

	// 配对检查
	pairingLhs, _ := bn254.Pair([]bn254.G1Affine{lhs}, []bn254.G2Affine{g2Gen})
	pairingRhs, _ := bn254.Pair([]bn254.G1Affine{proof}, []bn254.G2Affine{rhs})

	return pairingLhs.Equal(&pairingRhs)
}
```

### 2.5 小结：你需要记住的

```
┌─────────────────────────────────────────────────────────────────────┐
│                      数学基础小结                                    │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1. 有限域：所有 ZK 运算在 mod p 下进行                              │
│     → 记住：运算结果总在 [0, p-1] 范围内                             │
│                                                                     │
│  2. 椭圆曲线：提供"单向门"安全性                                     │
│     → 记住：n*G 容易算，从 P 求 n 很难                               │
│                                                                     │
│  3. 多项式承诺：ZK 的核心工具                                        │
│     → 记住：承诺一个多项式，然后证明它在某点的值                      │
│                                                                     │
│  4. 实际使用：你通常不需要手写这些                                   │
│     → 使用库：gnark, arkworks, halo2                                │
│     → 关注电路设计，数学由库处理                                     │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 第三部分：区块链应用实践

### 3.1 zkRollup 架构详解

```
┌─────────────────────────────────────────────────────────────────────┐
│                      zkRollup 完整架构                               │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Layer 1 (以太坊主网)                                                │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │                                                             │    │
│  │  ┌───────────────┐    ┌───────────────┐                     │    │
│  │  │ zkRollup 合约  │    │  Verifier 合约│                     │    │
│  │  │               │    │               │                     │    │
│  │  │ • State Root  │◄───│ • 验证 ZK 证明 │                     │    │
│  │  │ • L2 存款/提款 │    │ • ~200k gas   │                     │    │
│  │  │ • 批量提交     │    │               │                     │    │
│  │  └───────────────┘    └───────────────┘                     │    │
│  │                                                             │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                           ▲                                         │
│                           │ 提交批次 + 证明                          │
│                           │                                         │
│  Layer 2 (zkRollup)       │                                         │
│  ┌────────────────────────┼────────────────────────────────────┐    │
│  │                        │                                    │    │
│  │  ┌─────────────────────┴─────────────────────┐              │    │
│  │  │              Sequencer / Prover           │              │    │
│  │  │  ┌─────────┐  ┌─────────┐  ┌─────────┐    │              │    │
│  │  │  │ 收集 TX  │─▶│ 执行 TX │─▶│ 生成证明  │    │              │    │
│  │  │  └─────────┘  └─────────┘  └─────────┘    │              │    │
│  │  └───────────────────────────────────────────┘              │    │
│  │                        ▲                                    │    │
│  │                        │                                    │    │
│  │  ┌─────────────────────┴─────────────────────┐              │    │
│  │  │              用户交易                      │              │    │
│  │  │  TX1, TX2, TX3, ... TXn                   │              │    │
│  │  └───────────────────────────────────────────┘              │    │
│  │                                                             │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  数据流：                                                            │
│  1. 用户提交 L2 交易                                                 │
│  2. Sequencer 排序并执行交易                                         │
│  3. Prover 为批量交易生成 ZK 证明                                     │
│  4. 提交新状态根 + 证明到 L1                                          │
│  5. L1 验证证明，更新状态                                              │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 3.2 zkRollup 状态证明

zkRollup 需要证明的核心陈述：

```
证明: 给定旧状态根 S_old 和一批交易 [tx1, tx2, ..., txn]，
     执行后得到新状态根 S_new

即: S_new = STF(S_old, [tx1, tx2, ..., txn])
    其中 STF = State Transition Function
```

**状态转换电路**：

```
┌─────────────────────────────────────────────────────────────────────┐
│                  zkRollup 状态转换电路                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  公开输入 (Public Inputs):                                           │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • old_state_root: bytes32                                  │    │
│  │  • new_state_root: bytes32                                  │    │
│  │  • transactions_hash: bytes32 (交易批次的哈希)                │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  私有输入 (Witness):                                                 │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • transactions: [tx1, tx2, ..., txn]                       │    │
│  │  • merkle_proofs: 每个账户的 Merkle 证明                      │    │
│  │  • signatures: 每笔交易的签名                                 │    │
│  │  • account_states: 每个账户的状态 (nonce, balance)            │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  电路约束：                                                          │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  for each tx in transactions:                               │    │
│  │    1. 验证签名正确                                            │    │
│  │    2. 验证 nonce 正确                                        │    │
│  │    3. 验证余额充足                                            │    │
│  │    4. 验证 Merkle 证明 (账户在旧状态树中)                       │    │
│  │    5. 更新账户状态 (扣款/收款/更新 nonce)                       │    │
│  │    6. 计算新的 Merkle 根                                     │    │
│  │                                                             │    │
│  │  final_root === new_state_root                              │    │
│  │  hash(transactions) === transactions_hash                   │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 3.3 zkEVM 类型对比

```
┌─────────────────────────────────────────────────────────────────────┐
│                      zkEVM 类型分类 (Vitalik)                        │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Type 1: 完全以太坊等效                                              │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • 与以太坊完全相同（包括哈希、状态树等）                         │    │
│  │  • 无需任何代码修改                                           │    │
│  │  • 证明生成非常慢（小时级）                                     │    │
│  │  • 代表：Taiko, (部分) Scroll                                │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  Type 2: 完全 EVM 等效                                              │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • EVM 完全相同，但底层数据结构可能不同                          │    │
│  │  • 应用无需修改                                              │    │
│  │  • 证明较慢（分钟级）                                          │    │
│  │  • 代表：Scroll, Polygon zkEVM                               │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  Type 2.5: EVM 等效但 gas 不同                                      │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • 某些操作 gas 成本不同                                     │    │
│  │  • 大部分应用兼容                                            │    │
│  │  • 证明较快                                                  │    │
│  │  • 代表：Scroll                                              │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  Type 3: 接近 EVM 等效                                              │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • 移除了难以证明的特性 (如某些预编译)                        │    │
│  │  • 需要小幅修改才能部署                                       │    │
│  │  • 证明更快                                                  │    │
│  │  • 代表：早期 Scroll                                         │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  Type 4: 高级语言等效                                               │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • 编译 Solidity/Vyper 到自定义 VM                           │    │
│  │  • 不是 EVM 字节码兼容                                       │    │
│  │  • 证明最快（秒级）                                          │    │
│  │  • 代表：zkSync Era, StarkNet                                │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  权衡：兼容性 ←───────────────────────────────────→ 性能            │
│        Type 1                                          Type 4       │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 3.4 主流 zkRollup 项目对比

| 项目                | 证明系统           | zkEVM 类型       | TPS   | 特点               |
|-------------------|----------------|----------------|-------|------------------|
| **zkSync Era**    | PLONK + Boojum | Type 4         | ~2000 | 原生账户抽象, LLVM 编译器 |
| **StarkNet**      | STARK          | Type 4 (Cairo) | ~1000 | 后量子安全, Cairo 语言  |
| **Scroll**        | PLONK + KZG    | Type 2         | ~500  | 最接近以太坊, 社区驱动     |
| **Polygon zkEVM** | PLONK + FRI    | Type 2         | ~500  | 开源, 递归证明         |
| **Linea**         | Groth16        | Type 2         | ~500  | ConsenSys 背书     |
| **Taiko**         | SGX + ZK       | Type 1         | ~200  | Based rollup     |

### 3.5 zkRollup 交互实践

**Go 代码：与 zkSync Era 交互**

```go
package main

import (
	"context"
	"crypto/ecdsa"
	"fmt"
	"math/big"

	"github.com/ethereum/go-ethereum/common"
	"github.com/ethereum/go-ethereum/crypto"
	"github.com/ethereum/go-ethereum/ethclient"
	"github.com/zksync-sdk/zksync2-go/accounts"
	"github.com/zksync-sdk/zksync2-go/clients"
	"github.com/zksync-sdk/zksync2-go/types"
)

func main() {
	// 连接 zkSync Era
	zkClient, err := clients.Dial("https://mainnet.era.zksync.io")
	if err != nil {
		panic(err)
	}
	defer zkClient.Close()

	// L1 以太坊客户端 (用于存款/提款)
	ethClient, err := ethclient.Dial("https://eth.llamarpc.com")
	if err != nil {
		panic(err)
	}
	defer ethClient.Close()

	// 加载私钥
	privateKey, err := crypto.HexToECDSA("YOUR_PRIVATE_KEY")
	if err != nil {
		panic(err)
	}

	// 创建钱包
	wallet, err := accounts.NewWallet(privateKey, zkClient, ethClient)
	if err != nil {
		panic(err)
	}

	ctx := context.Background()

	// 查询余额
	balance, err := wallet.Balance(ctx, utils.EthAddress, nil)
	if err != nil {
		panic(err)
	}
	fmt.Printf("L2 Balance: %s ETH\n", weiToEth(balance))

	// 发送 L2 交易
	to := common.HexToAddress("0x...")
	amount := big.NewInt(1e16) // 0.01 ETH

	tx, err := wallet.Transfer(ctx, accounts.TransferTransaction{
		To:     to,
		Amount: amount,
		Token:  utils.EthAddress,
	})
	if err != nil {
		panic(err)
	}

	// 等待交易确认
	receipt, err := zkClient.WaitMined(ctx, tx.Hash())
	if err != nil {
		panic(err)
	}
	fmt.Printf("TX confirmed in block %d\n", receipt.BlockNumber)
}

func weiToEth(wei *big.Int) string {
	eth := new(big.Float).Quo(
		new(big.Float).SetInt(wei),
		new(big.Float).SetInt(big.NewInt(1e18)),
	)
	return eth.Text('f', 6)
}
```

**与 StarkNet 交互（Go/Rust）**

```go
package main

import (
	"context"
	"fmt"

	"github.com/NethermindEth/starknet.go/account"
	"github.com/NethermindEth/starknet.go/rpc"
	"github.com/NethermindEth/starknet.go/utils"
)

func main() {
	// 连接 StarkNet
	client, err := rpc.NewClient("https://starknet-mainnet.public.blastapi.io")
	if err != nil {
		panic(err)
	}

	// 创建账户 (StarkNet 使用不同的签名方案)
	privateKey := "0x..." // StarkNet 私钥
	accountAddress := "0x..."

	account, err := account.NewAccount(
		client,
		utils.HexToFelt(accountAddress),
		utils.HexToFelt(privateKey),
	)
	if err != nil {
		panic(err)
	}

	ctx := context.Background()

	// 查询余额 (调用 ERC20 合约)
	ethAddress := utils.HexToFelt("0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7")

	result, err := client.Call(ctx, rpc.FunctionCall{
		ContractAddress:    ethAddress,
		EntryPointSelector: utils.GetSelectorFromName("balanceOf"),
		Calldata:           []*felt.Felt{account.AccountAddress},
	}, rpc.BlockID{Tag: "latest"})
	if err != nil {
		panic(err)
	}

	fmt.Printf("Balance: %s\n", result[0].String())

	// 发送交易
	tx, err := account.Execute(ctx, []rpc.FunctionCall{
		{
			ContractAddress:    ethAddress,
			EntryPointSelector: utils.GetSelectorFromName("transfer"),
			Calldata: []*felt.Felt{
				utils.HexToFelt("0x..."),  // 接收地址
				utils.HexToFelt("0x1000"), // 金额 low
				utils.HexToFelt("0x0"),    // 金额 high
			},
		},
	}, rpc.ExecuteDetails{})
	if err != nil {
		panic(err)
	}

	fmt.Printf("TX Hash: %s\n", tx.TransactionHash.String())
}
```

### 3.6 ZK 隐私应用

```
┌─────────────────────────────────────────────────────────────────────┐
│                    ZK 隐私协议架构                                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  以 Tornado Cash 类协议为例：                                        │
│                                                                     │
│  存款流程：                                                          │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  1. 用户生成随机 secret 和 nullifier                         │    │
│  │  2. commitment = hash(secret, nullifier)                    │    │
│  │  3. 将 commitment 插入 Merkle 树                             │    │
│  │  4. 存入固定金额 (如 1 ETH)                                  │    │
│  │                                                             │    │
│  │  链上存储：                                                   │    │
│  │  • Merkle 树（所有 commitment）                              │    │
│  │  • 已使用的 nullifier 集合                                    │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  取款流程：                                                          │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  用户生成 ZK 证明，证明：                                     │    │
│  │  1. 我知道某个 secret 和 nullifier                          │    │
│  │  2. hash(secret, nullifier) 在 Merkle 树中                  │    │
│  │  3. 这个 nullifier 没有被使用过                              │    │
│  │                                                             │    │
│  │  公开输入：                                                   │    │
│  │  • Merkle root (验证 commitment 存在)                        │    │
│  │  • nullifier hash (防止双花)                                 │    │
│  │  • recipient address (接收地址)                              │    │
│  │                                                             │    │
│  │  私有输入：                                                   │    │
│  │  • secret                                                   │    │
│  │  • nullifier                                                │    │
│  │  • Merkle proof                                             │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  隐私保证：                                                          │
│  • 验证者无法知道是哪笔存款                                          │
│  • 无法关联存款人和取款人                                            │
│  • 金额固定进一步增强隐私                                            │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 3.7 ZK 跨链桥

```
┌─────────────────────────────────────────────────────────────────────┐
│                      ZK 跨链桥架构                                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  传统桥 vs ZK 桥：                                                   │
│                                                                     │
│  传统多签桥：                                                        │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • 依赖 N/M 多签验证者                                       │    │
│  │  • 信任假设：诚实多数                                        │    │
│  │  • 攻击面：贿赂/黑客攻击验证者                                │    │
│  │  • 历史损失：数十亿美元                                       │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  ZK 桥：                                                            │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • 证明源链上的状态/交易                                      │    │
│  │  • 信任假设：仅密码学                                        │    │
│  │  • 无需信任任何验证者                                        │    │
│  │  • 安全性 = 源链安全性 + ZK 安全性                           │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  ZK Light Client 桥工作流程：                                        │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │                                                             │    │
│  │   Chain A                           Chain B                 │    │
│  │   ┌─────────┐                       ┌─────────┐             │    │
│  │   │ Block N │                       │ Verifier│             │    │
│  │   │ Header  │                       │ Contract│             │    │
│  │   └────┬────┘                       └────▲────┘             │    │
│  │        │                                 │                  │    │
│  │        │    ┌─────────────────┐          │                  │    │
│  │        └───▶│   ZK Prover    │──────────┘                  │    │
│  │             │                 │                             │    │
│  │             │ 证明:           │                             │    │
│  │             │ 1. Block N 有效 │                             │    │
│  │             │ 2. TX 在 Block N│                             │    │
│  │             │ 3. 共识签名有效 │                             │    │
│  │             └─────────────────┘                             │    │
│  │                                                             │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

**ZK 跨链桥项目**：

| 项目                       | 方案                   | 支持链 | 特点       |
|--------------------------|----------------------|-----|----------|
| **Succinct**             | SNARK + Light Client | 多链  | 证明共识签名   |
| **Polyhedra (zkBridge)** | STARK                | 多链  | 去中心化证明网络 |
| **=nil; Foundation**     | PLONK                | 多链  | 通用证明市场   |
| **Lagrange**             | State Proof          | 多链  | 证明存储状态   |

### 3.8 ZK 身份与凭证

```
┌─────────────────────────────────────────────────────────────────────┐
│                    ZK 身份验证                                       │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  场景：证明"我年龄 >= 18"而不透露具体生日                             │
│                                                                     │
│  传统方案：                                                          │
│  • 出示身份证 → 对方知道你的生日、地址、姓名...                        │
│                                                                     │
│  ZK 方案：                                                           │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │                                                             │    │
│  │  发行方 (如政府)           用户                   验证方      │    │
│  │  ┌─────────┐              ┌─────────┐           ┌─────────┐ │    │
│  │  │ 签名    │              │         │           │         │ │    │
│  │  │ 用户    │─────────────▶│ 生成    │──────────▶│ 验证    │ │    │
│  │  │ 凭证    │              │ ZK 证明 │           │ 证明    │ │    │
│  │  └─────────┘              └─────────┘           └─────────┘ │    │
│  │                                                             │    │
│  │  签名凭证包含:              ZK 证明:              验证方得知: │    │
│  │  • 姓名: Alice             • 我有有效凭证         • 用户>=18 │    │
│  │  • 生日: 1990-01-15        • 凭证被授权机构签名    • 凭证有效 │    │
│  │  • 证件号: xxx             • 我的年龄 >= 18                  │    │
│  │                                                             │    │
│  │  验证方不知道:                                                │    │
│  │  • 具体姓名、生日、证件号                                      │    │
│  │  • 只知道"某个有效凭证持有者年龄>=18"                          │    │
│  │                                                             │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  代表项目：                                                          │
│  • Worldcoin: 证明"我是人类"且未注册过                               │
│  • Polygon ID: 通用凭证框架                                         │
│  • Semaphore: 匿名群组成员证明                                       │
│  • Sismo: 链上声誉聚合                                              │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 第四部分：代码实战

### 4.1 ZK 开发框架选择

```
┌─────────────────────────────────────────────────────────────────────┐
│                      ZK 开发框架对比                                 │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  语言 / 框架          证明系统         适用场景                       │
│  ──────────────────────────────────────────────────────────────────  │
│  Circom (JS DSL)      Groth16/PLONK    入门学习, 小型电路            │
│  gnark (Go)           Groth16/PLONK    生产级, Go 生态               │
│  arkworks (Rust)      多种             底层开发, 高性能              │
│  halo2 (Rust)         PLONK/IPA        Scroll, Zcash                │
│  Cairo (Rust-like)    STARK            StarkNet 专用                │
│  Noir (Rust-like)     多种             Aztec, 新手友好               │
│  SP1 (Rust)           STARK            zkVM, 通用 Rust 证明          │
│  Risc0 (Rust)         STARK            zkVM, RISC-V 证明            │
│                                                                     │
│  推荐学习路径：                                                       │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  入门 → Circom (概念理解) → gnark/Noir (实际开发)            │    │
│  │       → arkworks/halo2 (深入研究)                           │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 4.2 Circom 快速入门

**安装**：

```bash
# 安装 Circom 编译器
curl --proto '=https' --tlsv1.2 https://sh.rustup.rs -sSf | sh
git clone https://github.com/iden3/circom.git
cd circom
cargo build --release
cargo install --path circom

# 安装 snarkjs
npm install -g snarkjs
```

**示例 1：简单乘法证明**

```circom
// multiply.circom
// 证明: 我知道 a 和 b，使得 a * b = c

pragma circom 2.0.0;

template Multiply() {
    // 私有输入
    signal input a;
    signal input b;

    // 公开输出
    signal output c;

    // 约束: a * b = c
    c <== a * b;
}

component main = Multiply();
```

**编译和生成证明**：

```bash
# 编译电路
circom multiply.circom --r1cs --wasm --sym

# 生成可信设置 (Powers of Tau)
snarkjs powersoftau new bn128 12 pot12_0000.ptau -v
snarkjs powersoftau contribute pot12_0000.ptau pot12_0001.ptau --name="First" -v
snarkjs powersoftau prepare phase2 pot12_0001.ptau pot12_final.ptau -v

# 为特定电路生成密钥
snarkjs groth16 setup multiply.r1cs pot12_final.ptau multiply_0000.zkey
snarkjs zkey contribute multiply_0000.zkey multiply_final.zkey --name="Contributor" -v
snarkjs zkey export verificationkey multiply_final.zkey verification_key.json

# 生成 witness
echo '{"a": "3", "b": "4"}' > input.json
node multiply_js/generate_witness.js multiply_js/multiply.wasm input.json witness.wtns

# 生成证明
snarkjs groth16 prove multiply_final.zkey witness.wtns proof.json public.json

# 验证证明
snarkjs groth16 verify verification_key.json public.json proof.json

# 导出 Solidity 验证合约
snarkjs zkey export solidityverifier multiply_final.zkey verifier.sol
```

**示例 2：Merkle 树成员证明**

```circom
// merkle.circom
// 证明: 我知道一个叶子节点和 Merkle 路径，使得它属于某个 Merkle 树

pragma circom 2.0.0;

include "circomlib/poseidon.circom";

template MerkleProof(levels) {
    signal input leaf;
    signal input pathElements[levels];
    signal input pathIndices[levels];  // 0 = 左, 1 = 右
    signal output root;

    signal hashes[levels + 1];
    hashes[0] <== leaf;

    component hashers[levels];

    for (var i = 0; i < levels; i++) {
        hashers[i] = Poseidon(2);

        // 根据 pathIndex 决定左右顺序
        var left = (1 - pathIndices[i]) * hashes[i] + pathIndices[i] * pathElements[i];
        var right = pathIndices[i] * hashes[i] + (1 - pathIndices[i]) * pathElements[i];

        hashers[i].inputs[0] <== left;
        hashers[i].inputs[1] <== right;
        hashes[i + 1] <== hashers[i].out;
    }

    root <== hashes[levels];
}

// 20 层 Merkle 树，可容纳 2^20 = 100万+ 叶子
component main {public [root]} = MerkleProof(20);
```

### 4.3 gnark (Go) 实战

**安装**：

```bash
go get github.com/consensys/gnark
go get github.com/consensys/gnark-crypto
```

**示例 1：Hash 原像证明**

```go
package main

import (
	"fmt"

	"github.com/consensys/gnark-crypto/ecc"
	"github.com/consensys/gnark/backend/groth16"
	"github.com/consensys/gnark/frontend"
	"github.com/consensys/gnark/frontend/cs/r1cs"
	"github.com/consensys/gnark/std/hash/mimc"
)

// HashPreimageCircuit 证明: 我知道 preimage，使得 hash(preimage) = hash
type HashPreimageCircuit struct {
	// 私有输入
	Preimage frontend.Variable

	// 公开输入
	Hash frontend.Variable `gnark:",public"`
}

// Define 定义电路约束
func (circuit *HashPreimageCircuit) Define(api frontend.API) error {
	// 创建 MiMC 哈希器
	mimc, _ := mimc.NewMiMC(api)
	mimc.Write(circuit.Preimage)
	computedHash := mimc.Sum()

	// 约束: 计算的哈希必须等于公开的哈希
	api.AssertIsEqual(computedHash, circuit.Hash)

	return nil
}

func main() {
	// 1. 编译电路
	var circuit HashPreimageCircuit
	r1cs, err := frontend.Compile(ecc.BN254.ScalarField(), r1cs.NewBuilder, &circuit)
	if err != nil {
		panic(err)
	}
	fmt.Printf("电路约束数量: %d\n", r1cs.GetNbConstraints())

	// 2. 设置 (生成 pk, vk)
	pk, vk, err := groth16.Setup(r1cs)
	if err != nil {
		panic(err)
	}

	// 3. 计算真实的哈希值 (链下)
	preimageValue := 12345
	goMimc := mimc.NewMiMC()
	goMimc.Write([]byte{byte(preimageValue)})
	hashValue := goMimc.Sum(nil)

	// 4. 创建 witness
	assignment := HashPreimageCircuit{
		Preimage: preimageValue,
		Hash:     hashValue,
	}
	witness, err := frontend.NewWitness(&assignment, ecc.BN254.ScalarField())
	if err != nil {
		panic(err)
	}

	// 5. 生成证明
	proof, err := groth16.Prove(r1cs, pk, witness)
	if err != nil {
		panic(err)
	}
	fmt.Println("证明生成成功!")

	// 6. 验证证明
	publicWitness, _ := witness.Public()
	err = groth16.Verify(proof, vk, publicWitness)
	if err != nil {
		panic(err)
	}
	fmt.Println("证明验证成功!")

	// 7. 导出 Solidity 验证合约
	f, _ := os.Create("verifier.sol")
	vk.ExportSolidity(f)
	f.Close()
	fmt.Println("Solidity 合约已生成")
}
```

**示例 2：签名验证电路**

```go
package main

import (
	"github.com/consensys/gnark/frontend"
	"github.com/consensys/gnark/std/algebra/native/twistededwards"
	"github.com/consensys/gnark/std/signature/eddsa"
)

// SignatureVerifyCircuit 验证 EdDSA 签名
type SignatureVerifyCircuit struct {
	PublicKey eddsa.PublicKey   `gnark:",public"`
	Signature eddsa.Signature   `gnark:",public"`
	Message   frontend.Variable `gnark:",public"`
}

func (circuit *SignatureVerifyCircuit) Define(api frontend.API) error {
	// 获取曲线
	curve, _ := twistededwards.NewEdCurve(api, twistededwards.BN254)

	// 创建 MiMC 哈希
	mimc, _ := mimc.NewMiMC(api)

	// 验证签名
	eddsa.Verify(curve, circuit.Signature, circuit.Message, circuit.PublicKey, &mimc)

	return nil
}
```

**示例 3：范围证明 (Range Proof)**

```go
package main

import (
	"github.com/consensys/gnark/frontend"
	"github.com/consensys/gnark/std/rangecheck"
)

// RangeProofCircuit 证明: value 在 [0, 2^bits) 范围内
type RangeProofCircuit struct {
	Value frontend.Variable // 私有: 要证明的值
	Max   frontend.Variable `gnark:",public"` // 公开: 最大值
}

func (circuit *RangeProofCircuit) Define(api frontend.API) error {
	// 使用范围检查器
	rangeChecker := rangecheck.New(api)

	// 检查 value 在 64 位范围内
	rangeChecker.Check(circuit.Value, 64)

	// 检查 value <= max
	diff := api.Sub(circuit.Max, circuit.Value)
	rangeChecker.Check(diff, 64)

	return nil
}

// 应用场景: 隐私交易中证明"我有足够余额"而不透露具体金额
```

### 4.4 Halo2 (Rust) 示例

**Cargo.toml**：

```toml
[dependencies]
halo2_proofs = "0.3"
halo2curves = "0.4"
```

**示例：简单算术电路**

```rust
use halo2_proofs::{
    circuit::{Layouter, SimpleFloorPlanner, Value},
    plonk::{Advice, Circuit, Column, ConstraintSystem, Error, Selector},
    poly::Rotation,
    pasta::Fp,
};

// 电路配置
#[derive(Clone, Debug)]
struct SimpleConfig {
    advice: [Column<Advice>; 2],
    selector: Selector,
}

// 电路定义: 证明 a * b = c
#[derive(Default)]
struct SimpleCircuit {
    a: Value<Fp>,
    b: Value<Fp>,
}

impl Circuit<Fp> for SimpleCircuit {
    type Config = SimpleConfig;
    type FloorPlanner = SimpleFloorPlanner;

    fn without_witnesses(&self) -> Self {
        Self::default()
    }

    fn configure(meta: &mut ConstraintSystem<Fp>) -> Self::Config {
        let advice = [meta.advice_column(), meta.advice_column()];
        let selector = meta.selector();

        // 启用相等约束
        meta.enable_equality(advice[0]);
        meta.enable_equality(advice[1]);

        // 定义乘法门
        meta.create_gate("mul", |meta| {
            let s = meta.query_selector(selector);
            let a = meta.query_advice(advice[0], Rotation::cur());
            let b = meta.query_advice(advice[1], Rotation::cur());
            let c = meta.query_advice(advice[0], Rotation::next());

            // s * (a * b - c) = 0
            // 当 s = 1 时，要求 a * b = c
            vec![s * (a * b - c)]
        });

        SimpleConfig { advice, selector }
    }

    fn synthesize(
        &self,
        config: Self::Config,
        mut layouter: impl Layouter<Fp>,
    ) -> Result<(), Error> {
        layouter.assign_region(
            || "multiply",
            |mut region| {
                // 启用选择器
                config.selector.enable(&mut region, 0)?;

                // 分配 a
                region.assign_advice(
                    || "a",
                    config.advice[0],
                    0,
                    || self.a,
                )?;

                // 分配 b
                region.assign_advice(
                    || "b",
                    config.advice[1],
                    0,
                    || self.b,
                )?;

                // 分配 c = a * b
                let c = self.a.and_then(|a| self.b.map(|b| a * b));
                region.assign_advice(
                    || "c",
                    config.advice[0],
                    1,
                    || c,
                )?;

                Ok(())
            },
        )
    }
}

fn main() {
    use halo2_proofs::{
        dev::MockProver,
        pasta::Fp,
    };

    // 创建电路实例
    let circuit = SimpleCircuit {
        a: Value::known(Fp::from(3)),
        b: Value::known(Fp::from(4)),
    };

    // 使用 MockProver 测试
    let prover = MockProver::run(4, &circuit, vec![]).unwrap();
    prover.assert_satisfied();

    println!("电路测试通过!");
}
```

### 4.5 SP1 / Risc0 zkVM

**zkVM 的优势**：无需手写电路，直接证明 Rust 程序执行

**SP1 示例**：

```rust
// program/main.rs - 要证明的程序
#![no_main]
sp1_zkvm::entrypoint!(main);

use sha2::{Digest, Sha256};

pub fn main() {
    // 读取私有输入
    let preimage: Vec<u8> = sp1_zkvm::io::read();

    // 计算哈希
    let mut hasher = Sha256::new();
    hasher.update(&preimage);
    let hash = hasher.finalize();

    // 输出公开结果
    sp1_zkvm::io::commit(&hash[..]);
}
```

```rust
// script/main.rs - 生成证明
use sp1_sdk::{ProverClient, SP1Stdin};

fn main() {
    // 初始化
    let client = ProverClient::new();
    let elf = include_bytes!("../../program/elf/riscv32im-succinct-zkvm-elf");

    // 准备输入
    let mut stdin = SP1Stdin::new();
    stdin.write(&b"secret preimage".to_vec());

    // 生成证明
    let (pk, vk) = client.setup(elf);
    let proof = client.prove(&pk, stdin).expect("proving failed");

    // 验证
    client.verify(&proof, &vk).expect("verification failed");

    println!("证明验证成功!");

    // 获取公开输出
    let hash: Vec<u8> = proof.public_values.read();
    println!("Hash: {:?}", hash);
}
```

### 4.6 链上验证合约

**Groth16 验证合约 (Solidity)**：

```solidity
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

/// @title Groth16 验证合约
/// @notice 由 snarkjs 或 gnark 自动生成
contract Groth16Verifier {
    // 配对预编译合约地址
    uint256 constant PRIME_Q = 21888242871839275222246405745257275088696311157297823662689037894645226208583;

    // 验证密钥 (由 setup 生成)
    struct VerifyingKey {
        uint256[2] alpha1;
        uint256[2][2] beta2;
        uint256[2][2] gamma2;
        uint256[2][2] delta2;
        uint256[2][] IC;
    }

    VerifyingKey vk;

    constructor() {
        // 初始化验证密钥 (从 gnark/snarkjs 导出)
        vk.alpha1 = [uint256(0x...), uint256(0x...)];
        vk.beta2 = [[uint256(0x...), uint256(0x...)], [uint256(0x...), uint256(0x...)]];
        // ... 其他参数
    }

    /// @notice 验证 Groth16 证明
    /// @param proof 证明数据 [a, b, c]
    /// @param publicInputs 公开输入
    function verify(
        uint256[8] calldata proof,
        uint256[] calldata publicInputs
    ) public view returns (bool) {
        require(publicInputs.length + 1 == vk.IC.length, "Invalid inputs length");

        // 计算线性组合
        uint256[2] memory vk_x = vk.IC[0];
        for (uint256 i = 0; i < publicInputs.length; i++) {
            require(publicInputs[i] < PRIME_Q, "Input too large");
            vk_x = addition(
                vk_x,
                scalar_mul(vk.IC[i + 1], publicInputs[i])
            );
        }

        // 配对检查
        // e(A, B) = e(alpha, beta) * e(vk_x, gamma) * e(C, delta)
        return pairing(
            negate(proof[0 : 2]),   // -A
            proof[2 : 6],           // B
            vk.alpha1,
            vk.beta2,
            vk_x,
            vk.gamma2,
            proof[6 : 8],           // C
            vk.delta2
        );
    }

    // 椭圆曲线运算 (使用预编译合约)
    function addition(uint256[2] memory p1, uint256[2] memory p2)
    internal view returns (uint256[2] memory r)
    {
        assembly {
            let success := staticcall(sub(gas(), 2000), 6, p1, 0x80, r, 0x40)
            if iszero(success) {revert(0, 0)}
        }
    }

    function scalar_mul(uint256[2] memory p, uint256 s)
    internal view returns (uint256[2] memory r)
    {
        uint256[3] memory input;
        input[0] = p[0];
        input[1] = p[1];
        input[2] = s;
        assembly {
            let success := staticcall(sub(gas(), 2000), 7, input, 0x60, r, 0x40)
            if iszero(success) {revert(0, 0)}
        }
    }

    function pairing(/* ... */) internal view returns (bool) {
        // 调用配对预编译合约 (0x08)
        // ...
    }
}
```

### 4.7 完整应用：隐私投票系统

```
┌─────────────────────────────────────────────────────────────────────┐
│                    隐私投票系统架构                                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  需求：                                                              │
│  1. 只有注册选民可以投票                                              │
│  2. 每人只能投一票                                                   │
│  3. 投票内容保密                                                     │
│  4. 任何人可以验证投票结果                                            │
│                                                                     │
│  ZK 方案：                                                           │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  证明内容:                                                   │    │
│  │  1. 我拥有某个注册的身份 (在 Merkle 树中)                     │    │
│  │  2. 我的 nullifier 没有被使用过 (防止双重投票)                │    │
│  │  3. 我的投票是有效选项之一                                    │    │
│  │                                                             │    │
│  │  公开输入: Merkle root, nullifier hash, vote option         │    │
│  │  私有输入: identity, merkle proof, nullifier                │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

**Circom 电路**：

```circom
pragma circom 2.0.0;

include "circomlib/poseidon.circom";
include "circomlib/comparators.circom";

template PrivateVoting(levels, numOptions) {
    // 公开输入
    signal input merkleRoot;
    signal input nullifierHash;
    signal input voteOption;

    // 私有输入
    signal input identitySecret;
    signal input identityNullifier;
    signal input merklePathElements[levels];
    signal input merklePathIndices[levels];

    // 1. 验证身份在 Merkle 树中
    component identityHasher = Poseidon(2);
    identityHasher.inputs[0] <== identitySecret;
    identityHasher.inputs[1] <== identityNullifier;
    signal identityCommitment <== identityHasher.out;

    // 计算 Merkle 根
    component merkleProof = MerkleTreeChecker(levels);
    merkleProof.leaf <== identityCommitment;
    for (var i = 0; i < levels; i++) {
        merkleProof.pathElements[i] <== merklePathElements[i];
        merkleProof.pathIndices[i] <== merklePathIndices[i];
    }
    merkleRoot === merkleProof.root;

    // 2. 验证 nullifier hash
    component nullifierHasher = Poseidon(1);
    nullifierHasher.inputs[0] <== identityNullifier;
    nullifierHash === nullifierHasher.out;

    // 3. 验证投票选项有效 (0 到 numOptions-1)
    component validOption = LessThan(32);
    validOption.in[0] <== voteOption;
    validOption.in[1] <== numOptions;
    validOption.out === 1;
}

component main {public [merkleRoot, nullifierHash, voteOption]} = PrivateVoting(20, 5);
```

**Go 投票合约交互**：

```go
package main

import (
	"context"
	"math/big"

	"github.com/ethereum/go-ethereum/accounts/abi/bind"
	"github.com/ethereum/go-ethereum/common"
	"github.com/ethereum/go-ethereum/ethclient"
)

// 投票系统客户端
type VotingClient struct {
	contract *VotingContract
	prover   *ZKProver
}

// 投票
func (c *VotingClient) Vote(
	ctx context.Context,
	identity *Identity,
	voteOption uint64,
	merkleProof *MerkleProof,
) error {
	// 1. 生成 ZK 证明
	proof, publicInputs, err := c.prover.GenerateProof(
		identity.Secret,
		identity.Nullifier,
		voteOption,
		merkleProof,
	)
	if err != nil {
		return err
	}

	// 2. 提交到链上
	tx, err := c.contract.Vote(
		&bind.TransactOpts{},
		publicInputs.MerkleRoot,
		publicInputs.NullifierHash,
		big.NewInt(int64(voteOption)),
		proof,
	)
	if err != nil {
		return err
	}

	// 3. 等待确认
	receipt, err := bind.WaitMined(ctx, c.contract.client, tx)
	if err != nil {
		return err
	}

	fmt.Printf("投票成功! TX: %s\n", receipt.TxHash.Hex())
	return nil
}
```

---

## 第五部分：性能优化与最佳实践

### 5.1 电路优化技巧

```
┌─────────────────────────────────────────────────────────────────────┐
│                      电路优化指南                                    │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1. 减少约束数量                                                     │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • 使用位分解代替比较运算                                     │    │
│  │  • 合并相似约束                                               │    │
│  │  • 使用查找表 (lookup) 代替复杂计算                           │    │
│  │                                                             │    │
│  │  // 差: 多次约束                                             │    │
│  │  for i in 0..n { assert(arr[i] < 256) }                     │    │
│  │                                                             │    │
│  │  // 好: 使用范围查找表                                        │    │
│  │  range_check(arr, 8_bits)                                    │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  2. 选择合适的哈希函数                                               │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  哈希函数          约束数 (per hash)   适用场景               │    │
│  │  ────────────────────────────────────────────────────────   │    │
│  │  Poseidon          ~300              ZK 友好，首选          │    │
│  │  MiMC              ~500              较老但成熟              │    │
│  │  Pedersen          ~1500             承诺场景                │    │
│  │  SHA256            ~25000            兼容性需求              │    │
│  │  Keccak256         ~140000           以太坊兼容              │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  3. 并行化证明生成                                                   │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • 使用 GPU 加速 (CUDA/Metal)                                │    │
│  │  • 多核 CPU 并行                                             │    │
│  │  • 分布式证明生成                                            │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  4. 递归证明                                                        │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  场景: 验证大量交易                                           │    │
│  │                                                             │    │
│  │  方案 A: 一个大证明 (慢)                                      │    │
│  │  proof = prove(tx1, tx2, ..., tx1000)                       │    │
│  │                                                             │    │
│  │  方案 B: 递归证明 (快)                                        │    │
│  │  proof1 = prove(tx1, tx2, ..., tx100)                       │    │
│  │  proof2 = prove(tx101, ..., tx200)                          │    │
│  │  ...                                                        │    │
│  │  final = prove(proof1, proof2, ..., proof10)                │    │
│  │                                                             │    │
│  │  优势: 可并行，增量更新                                       │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 5.2 安全注意事项

```
┌─────────────────────────────────────────────────────────────────────┐
│                      ZK 安全清单                                     │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ⚠️ 常见漏洞:                                                        │
│                                                                     │
│  1. 未约束信号                                                       │
│     ✗ signal x;  // x 可以是任意值!                                  │
│     ✓ signal x; x * x === x;  // 约束 x 只能是 0 或 1                │
│                                                                     │
│  2. 约束不完整                                                       │
│     ✗ a * b === c;  // 如果 a = 0，b 可以是任意值                    │
│     ✓ a !== 0; a * b === c;                                        │
│                                                                     │
│  3. 整数溢出                                                         │
│     ✗ result <== a + b;  // 可能溢出域大小                           │
│     ✓ 确保输入在安全范围内                                           │
│                                                                     │
│  4. 可信设置泄露                                                     │
│     ✗ 使用未销毁 toxic waste 的设置                                  │
│     ✓ 使用公开的 Powers of Tau 仪式参数                              │
│                                                                     │
│  5. 信息泄露                                                         │
│     ✗ 通过执行时间、证明大小泄露私有输入信息                          │
│     ✓ 使用固定时间算法，统一证明大小                                  │
│                                                                     │
│  ✅ 安全检查:                                                        │
│  • 使用形式化验证工具 (Ecne, Picus)                                  │
│  • 审计所有约束完整性                                                │
│  • 测试边界条件和零值                                                │
│  • 审查公开/私有输入划分                                             │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 5.3 工具链与资源

```
┌─────────────────────────────────────────────────────────────────────┐
│                      开发工具与资源                                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  开发工具:                                                           │
│  • Circomspect: Circom 静态分析器                                   │
│  • gnark-debug: gnark 调试工具                                      │
│  • halo2-analyzer: Halo2 电路分析                                   │
│                                                                     │
│  测试框架:                                                           │
│  • MockProver (halo2): 快速本地测试                                 │
│  • circom_tester: Circom 单元测试                                   │
│  • gnark test package: Go 测试集成                                  │
│                                                                     │
│  学习资源:                                                           │
│  • ZK Whiteboard Sessions (YouTube)                                 │
│  • 0xPARC Learning Group                                           │
│  • zkSNARKs in a Nutshell                                          │
│  • Halo2 Book (https://zcash.github.io/halo2/)                     │
│  • gnark Docs (https://docs.gnark.consensys.net/)                  │
│                                                                     │
│  开源项目参考:                                                       │
│  • Semaphore: 匿名信号协议                                          │
│  • Tornado Cash: 隐私交易 (教育用)                                  │
│  • zkSync: 开源 zkRollup                                           │
│  • Scroll: zkEVM 实现                                               │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 总结：快速上手路线

```
┌─────────────────────────────────────────────────────────────────────┐
│                      ZK 学习路线图                                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Week 1: 概念理解                                                    │
│  ├── 理解 ZK 三属性 (完备性/可靠性/零知识)                            │
│  ├── 了解 SNARKs vs STARKs 区别                                     │
│  └── 阅读简单电路示例                                                │
│                                                                     │
│  Week 2: 动手实践                                                    │
│  ├── 安装 Circom + snarkjs                                         │
│  ├── 编写第一个电路 (乘法/哈希)                                      │
│  └── 生成并验证证明                                                  │
│                                                                     │
│  Week 3: 进阶开发                                                    │
│  ├── 学习 gnark (Go) 或 arkworks (Rust)                            │
│  ├── 实现 Merkle 树证明                                             │
│  └── 部署 Solidity 验证合约                                         │
│                                                                     │
│  Week 4: 应用构建                                                    │
│  ├── 选择具体应用场景 (隐私/扩容/身份)                               │
│  ├── 设计完整电路                                                    │
│  └── 集成前后端                                                      │
│                                                                     │
│  持续学习:                                                           │
│  ├── 研究 zkEVM 实现                                                │
│  ├── 了解递归证明                                                    │
│  └── 关注新证明系统 (Plonky3, Binius)                               │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘

关键记忆点:

1. ZK = 证明计算正确性，不泄露输入
2. 电路 = 将程序转换为约束系统
3. SNARKs: 证明小/验证快/需要设置
4. STARKs: 透明/后量子安全/证明大
5. 实际开发: 用库，专注电路设计
```

---

## 第六部分：递归证明 (Recursive Proofs)

### 6.1 什么是递归证明？

**核心思想**：在电路内部验证另一个 ZK 证明，实现"证明的证明"。

```
┌─────────────────────────────────────────────────────────────────────┐
│                      递归证明核心概念                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  普通证明:                                                           │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  Prover                              Verifier               │    │
│  │    │                                    │                   │    │
│  │    │  proof = Prove(circuit, witness)   │                   │    │
│  │    │ ──────────────────────────────────▶│                   │    │
│  │    │                                    │ Verify(vk, proof) │    │
│  │    │                                    │ → true/false      │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  递归证明:                                                           │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │                                                             │    │
│  │  proof₁ ─┐                                                  │    │
│  │          │     ┌──────────────────┐                         │    │
│  │  proof₂ ─┼────▶│  Aggregation     │                         │    │
│  │          │     │  Circuit         │────▶ aggregated_proof   │    │
│  │  proof₃ ─┘     │                  │                         │    │
│  │                │  验证 proof₁,₂,₃  │                         │    │
│  │                │  在电路内部!       │                         │    │
│  │                └──────────────────┘                         │    │
│  │                                                             │    │
│  │  最终: 1 个小证明 = 验证 N 个原始证明                         │    │
│  │                                                             │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  应用场景:                                                           │
│  • zkRollup: 聚合数千笔交易的证明                                    │
│  • 增量可验证计算 (IVC): 持续累积计算                                │
│  • 证明压缩: 减少链上验证成本                                        │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 6.2 递归证明的挑战

```
┌─────────────────────────────────────────────────────────────────────┐
│                      递归证明技术挑战                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  核心问题: 在电路中验证证明需要"原生"支持验证算法                      │
│                                                                     │
│  挑战 1: 域不匹配 (Field Mismatch)                                   │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • 证明系统 A 使用域 F_p                                     │    │
│  │  • 在 F_p 电路中验证需要模拟 F_q 运算                         │    │
│  │  • 非原生域运算开销巨大 (~1000x)                              │    │
│  │                                                             │    │
│  │  解决方案:                                                   │    │
│  │  • 循环曲线 (Cycle of Curves): 两条曲线互为对方的标量域       │    │
│  │  • 例: Pasta curves (Pallas + Vesta) 用于 Halo2              │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  挑战 2: 验证电路大小                                                │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • Groth16 验证需要配对运算 → 电路约束数百万                   │    │
│  │  • PLONK 验证相对简单但仍需大量约束                           │    │
│  │                                                             │    │
│  │  解决方案:                                                   │    │
│  │  • 使用验证友好的证明系统 (IPA-based)                         │    │
│  │  • 延迟验证 (Deferred Verification)                          │    │
│  │  • 折叠方案 (Folding Schemes)                                │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  挑战 3: 可信设置传递                                                │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  • 每层递归需要不同的 SRS?                                    │    │
│  │  • 解决: 使用通用 SRS (Universal Setup)                       │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 6.3 循环曲线 (Cycle of Curves)

```
┌─────────────────────────────────────────────────────────────────────┐
│                      Pasta Curves (Pallas + Vesta)                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  定义:                                                              │
│  • Pallas: y² = x³ + 5 over F_p                                    │
│  • Vesta:  y² = x³ + 5 over F_q                                    │
│  • 关键: p = |Vesta| (Vesta 群的阶)                                 │
│         q = |Pallas| (Pallas 群的阶)                                │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │                                                             │    │
│  │    Pallas (F_p)                    Vesta (F_q)              │    │
│  │    ┌──────────┐                    ┌──────────┐             │    │
│  │    │ 标量域=F_q│◄──────────────────│ 基域=F_q  │             │    │
│  │    │ 基域=F_p  │──────────────────▶│ 标量域=F_p│             │    │
│  │    └──────────┘                    └──────────┘             │    │
│  │         │                               │                   │    │
│  │         │  Pallas 电路的                │  Vesta 电路的      │    │
│  │         │  witness 在 F_q              │  witness 在 F_p    │    │
│  │         │                               │                   │    │
│  │         └───────────┬───────────────────┘                   │    │
│  │                     │                                       │    │
│  │                     ▼                                       │    │
│  │         Pallas 证明可以在 Vesta 电路中                       │    │
│  │         原生验证 (无需模拟域运算)                            │    │
│  │                                                             │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  工作流程:                                                           │
│  1. 在 Pallas 上生成 proof₁                                         │
│  2. 在 Vesta 电路中验证 proof₁，生成 proof₂                         │
│  3. 在 Pallas 电路中验证 proof₂，生成 proof₃                        │
│  4. 交替进行，最终聚合                                              │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 6.4 递归证明实现 (Halo2 风格)

```rust
use halo2_proofs::{
    circuit::{Layouter, SimpleFloorPlanner, Value},
    plonk::{Circuit, ConstraintSystem, Error, VerifyingKey},
    poly::commitment::Params,
};

/// 递归验证电路
/// 在电路内部验证另一个 Halo2 证明
struct RecursiveCircuit<F: PrimeField> {
    // 要验证的内部证明
    inner_proof: Option<Vec<u8>>,
    inner_instances: Option<Vec<Vec<F>>>,

    // 新的计算
    new_input: Value<F>,
    new_output: Value<F>,
}

impl<F: PrimeField> Circuit<F> for RecursiveCircuit<F> {
    type Config = RecursiveConfig;
    type FloorPlanner = SimpleFloorPlanner;

    fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
        // 配置验证电路
        let advice = meta.advice_column();
        let instance = meta.instance_column();

        // 验证器配置
        let verifier_config = VerifierChip::configure(meta);

        // 新计算配置
        let computation_config = ComputationChip::configure(meta);

        RecursiveConfig {
            verifier_config,
            computation_config,
            advice,
            instance,
        }
    }

    fn synthesize(
        &self,
        config: Self::Config,
        mut layouter: impl Layouter<F>,
    ) -> Result<(), Error> {
        // 1. 验证内部证明
        layouter.assign_region(
            || "verify inner proof",
            |mut region| {
                // 在电路中执行验证
                // 这是递归的核心：验证逻辑被编码为约束
                config.verifier_config.verify_proof(
                    &mut region,
                    self.inner_proof.as_ref(),
                    self.inner_instances.as_ref(),
                )?;
                Ok(())
            },
        )?;

        // 2. 执行新计算
        layouter.assign_region(
            || "new computation",
            |mut region| {
                let input = region.assign_advice(
                    || "input",
                    config.advice,
                    0,
                    || self.new_input,
                )?;

                // 执行计算...
                let output = self.new_input.map(|x| x * x);
                region.assign_advice(
                    || "output",
                    config.advice,
                    1,
                    || output,
                )?;

                Ok(())
            },
        )
    }
}

/// 增量可验证计算 (IVC) 链
struct IVCChain<F: PrimeField> {
    current_proof: Option<Vec<u8>>,
    current_state: F,
    step_count: usize,
}

impl<F: PrimeField> IVCChain<F> {
    pub fn new(initial_state: F) -> Self {
        Self {
            current_proof: None,
            current_state: initial_state,
            step_count: 0,
        }
    }

    /// 执行一步计算并生成新证明
    pub fn step<C: Circuit<F>>(&mut self, step_circuit: C) -> Result<(), Error> {
        // 构建递归电路
        let recursive_circuit = RecursiveCircuit {
            inner_proof: self.current_proof.clone(),
            inner_instances: None,
            new_input: Value::known(self.current_state),
            new_output: Value::unknown(),
        };

        // 生成新证明
        let new_proof = create_proof(&recursive_circuit)?;

        self.current_proof = Some(new_proof);
        self.step_count += 1;

        Ok(())
    }

    /// 验证最终证明
    pub fn verify(&self) -> Result<bool, Error> {
        if let Some(proof) = &self.current_proof {
            verify_proof(proof)
        } else {
            Ok(true)
        }
    }
}
```

### 6.5 gnark 递归证明

```go
package recursion

import (
	"github.com/consensys/gnark/backend/groth16"
	"github.com/consensys/gnark/frontend"
	"github.com/consensys/gnark/std/algebra/emulated/sw_bn254"
	stdgroth16 "github.com/consensys/gnark/std/recursion/groth16"
)

// RecursiveCircuit 递归验证电路
type RecursiveCircuit struct {
	// 要验证的证明 (BN254 曲线上的 Groth16)
	Proof        stdgroth16.Proof[sw_bn254.G1Affine, sw_bn254.G2Affine]
	VerifyingKey stdgroth16.VerifyingKey[sw_bn254.G1Affine, sw_bn254.G2Affine, sw_bn254.GTEl]
	InnerWitness stdgroth16.Witness[sw_bn254.ScalarField]

	// 新计算
	NewInput  frontend.Variable
	NewOutput frontend.Variable `gnark:",public"`
}

func (c *RecursiveCircuit) Define(api frontend.API) error {
	// 1. 创建验证器
	verifier, err := stdgroth16.NewVerifier[sw_bn254.ScalarField, sw_bn254.G1Affine, sw_bn254.G2Affine, sw_bn254.GTEl](api)
	if err != nil {
		return err
	}

	// 2. 在电路中验证内部证明
	err = verifier.AssertProof(c.VerifyingKey, c.Proof, c.InnerWitness)
	if err != nil {
		return err
	}

	// 3. 执行新计算
	result := api.Mul(c.NewInput, c.NewInput)
	api.AssertIsEqual(result, c.NewOutput)

	return nil
}

// AggregationCircuit 证明聚合电路
type AggregationCircuit struct {
	Proofs       []stdgroth16.Proof[sw_bn254.G1Affine, sw_bn254.G2Affine]
	VerifyingKey stdgroth16.VerifyingKey[sw_bn254.G1Affine, sw_bn254.G2Affine, sw_bn254.GTEl]
	Witnesses    []stdgroth16.Witness[sw_bn254.ScalarField]

	AggregatedHash frontend.Variable `gnark:",public"`
}

func (c *AggregationCircuit) Define(api frontend.API) error {
	verifier, _ := stdgroth16.NewVerifier[sw_bn254.ScalarField, sw_bn254.G1Affine, sw_bn254.G2Affine, sw_bn254.GTEl](api)

	// 验证所有证明
	for i := range c.Proofs {
		err := verifier.AssertProof(c.VerifyingKey, c.Proofs[i], c.Witnesses[i])
		if err != nil {
			return err
		}
	}

	// 计算聚合哈希
	mimc, _ := mimc.NewMiMC(api)
	for _, w := range c.Witnesses {
		for _, pub := range w.Public {
			mimc.Write(pub)
		}
	}
	hash := mimc.Sum()
	api.AssertIsEqual(hash, c.AggregatedHash)

	return nil
}
```

### 6.6 zkRollup 递归架构

```
┌─────────────────────────────────────────────────────────────────────┐
│                  zkRollup 递归证明架构                               │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  交易层 (每笔交易一个证明):                                          │
│  ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐   │
│  │ tx1 │ │ tx2 │ │ tx3 │ │ tx4 │ │ tx5 │ │ tx6 │ │ tx7 │ │ tx8 │   │
│  └──┬──┘ └──┬──┘ └──┬──┘ └──┬──┘ └──┬──┘ └──┬──┘ └──┬──┘ └──┬──┘   │
│     │       │       │       │       │       │       │       │       │
│     └───┬───┘       └───┬───┘       └───┬───┘       └───┬───┘       │
│         │               │               │               │           │
│         ▼               ▼               ▼               ▼           │
│  聚合层 1:                                                          │
│  ┌─────────┐       ┌─────────┐       ┌─────────┐       ┌─────────┐  │
│  │ agg_1_1 │       │ agg_1_2 │       │ agg_1_3 │       │ agg_1_4 │  │
│  └────┬────┘       └────┬────┘       └────┬────┘       └────┬────┘  │
│       │                 │                 │                 │       │
│       └────────┬────────┘                 └────────┬────────┘       │
│                │                                   │                │
│                ▼                                   ▼                │
│  聚合层 2:                                                          │
│         ┌───────────┐                       ┌───────────┐           │
│         │  agg_2_1  │                       │  agg_2_2  │           │
│         └─────┬─────┘                       └─────┬─────┘           │
│               │                                   │                 │
│               └─────────────┬─────────────────────┘                 │
│                             │                                       │
│                             ▼                                       │
│  最终证明:                                                           │
│                    ┌─────────────────┐                              │
│                    │   Final Proof   │                              │
│                    │  (提交到 L1)     │                              │
│                    └─────────────────┘                              │
│                                                                     │
│  优势:                                                               │
│  • 并行化: 每层可以并行生成                                          │
│  • 延迟低: 不需要等待所有交易                                         │
│  • 成本分摊: 每笔交易分摊验证成本 ≈ O(1)                             │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 第七部分：Lookup Arguments

### 7.1 为什么需要 Lookup？

```
┌─────────────────────────────────────────────────────────────────────┐
│                      Lookup 的动机                                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  问题: 某些操作用算术约束实现代价很高                                  │
│                                                                     │
│  示例 1: 范围检查 (x < 256)                                          │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  算术方式:                                                   │    │
│  │  • 将 x 分解为 8 个 bit: x = b₀ + 2b₁ + 4b₂ + ... + 128b₇   │    │
│  │  • 每个 bit 约束: bᵢ * (1 - bᵢ) = 0                         │    │
│  │  • 需要 8 个约束 + 8 个变量                                  │    │
│  │                                                             │    │
│  │  Lookup 方式:                                                │    │
│  │  • 预定义表 T = {0, 1, 2, ..., 255}                         │    │
│  │  • 证明 x ∈ T                                                │    │
│  │  • 1 个 lookup，开销远小于位分解                             │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  示例 2: XOR 操作 (a ⊕ b = c)                                       │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  算术方式: 需要完整的位分解                                   │    │
│  │  • 分解 a, b 为 bits                                        │    │
│  │  • 对每对 bit 计算 XOR                                       │    │
│  │  • 重组结果                                                  │    │
│  │  • 256 位数需要 ~768 个约束                                  │    │
│  │                                                             │    │
│  │  Lookup 方式:                                                │    │
│  │  • 预定义 XOR 表: {(a, b, a⊕b) | a,b ∈ [0,255]}             │    │
│  │  • 查表验证                                                  │    │
│  │  • 大幅减少约束                                              │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  常见 Lookup 表:                                                     │
│  • 范围表: {0, 1, ..., 2ⁿ-1}                                       │
│  • 位运算表: XOR, AND, OR                                           │
│  • SHA256 轮函数表                                                  │
│  • 椭圆曲线点表 (用于签名验证)                                       │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 7.2 Plookup 协议原理

```
┌─────────────────────────────────────────────────────────────────────┐
│                      Plookup 核心思想                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  目标: 证明向量 f 的所有元素都在表 T 中                               │
│                                                                     │
│  f = [f₁, f₂, ..., fₙ]  (查询向量，可有重复)                        │
│  T = [t₁, t₂, ..., tₘ]  (表，无重复)                                │
│                                                                     │
│  核心观察: 如果 f ⊆ T，那么排序后的 (f ∪ T) 有特殊结构               │
│                                                                     │
│  示例:                                                               │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  T = [1, 2, 3, 4]                                           │    │
│  │  f = [2, 2, 4, 1]  (查询，有重复)                            │    │
│  │                                                             │    │
│  │  合并并排序: s = sort(f ∪ T) = [1, 1, 2, 2, 2, 3, 4, 4]     │    │
│  │                                                             │    │
│  │  关键性质: 相邻元素要么相等，要么差值在 T 的相邻元素中          │    │
│  │  s[i+1] - s[i] ∈ {0} ∪ {t[j+1] - t[j]}                      │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  协议步骤:                                                           │
│  1. Prover 构造排序后的向量 s                                        │
│  2. 使用随机挑战 γ 压缩多列为单列                                    │
│  3. 使用 Grand Product Argument 验证排序正确性                       │
│  4. 验证相邻差值约束                                                 │
│                                                                     │
│  Grand Product (简化):                                               │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  验证: ∏(γ + fᵢ) · ∏(γ + tⱼ) = ∏(γ + sₖ)                    │    │
│  │                                                             │    │
│  │  使用多项式:                                                 │    │
│  │  • F(X) = ∏(X - fᵢ)                                         │    │
│  │  • T(X) = ∏(X - tⱼ)                                         │    │
│  │  • S(X) = ∏(X - sₖ)                                         │    │
│  │                                                             │    │
│  │  验证: F(γ) · T(γ) = S(γ)                                   │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 7.3 Halo2 Lookup 实现

```rust
use halo2_proofs::{
    circuit::{Layouter, Value},
    plonk::{Advice, Column, ConstraintSystem, Error, Selector, TableColumn},
    poly::Rotation,
    pasta::Fp,
};

/// 范围检查电路配置 (使用 Lookup)
#[derive(Clone, Debug)]
struct RangeCheckConfig {
    value: Column<Advice>,
    range_table: TableColumn,
    selector: Selector,
}

impl RangeCheckConfig {
    fn configure(
        meta: &mut ConstraintSystem<Fp>,
        value: Column<Advice>,
    ) -> Self {
        let range_table = meta.lookup_table_column();
        let selector = meta.complex_selector();

        // 定义 lookup: value 必须在 range_table 中
        meta.lookup("range check", |meta| {
            let s = meta.query_selector(selector);
            let v = meta.query_advice(value, Rotation::cur());

            // 当 selector 启用时，v 必须在表中
            vec![(s * v, range_table)]
        });

        Self {
            value,
            range_table,
            selector,
        }
    }

    /// 加载范围表 [0, 1, ..., 2^k - 1]
    fn load_table(
        &self,
        layouter: &mut impl Layouter<Fp>,
        k: usize,
    ) -> Result<(), Error> {
        layouter.assign_table(
            || "range table",
            |mut table| {
                for i in 0..(1 << k) {
                    table.assign_cell(
                        || format!("row {}", i),
                        self.range_table,
                        i,
                        || Value::known(Fp::from(i as u64)),
                    )?;
                }
                Ok(())
            },
        )
    }
}

/// XOR 表电路配置
#[derive(Clone, Debug)]
struct XorTableConfig {
    input_a: Column<Advice>,
    input_b: Column<Advice>,
    output: Column<Advice>,

    table_a: TableColumn,
    table_b: TableColumn,
    table_xor: TableColumn,

    selector: Selector,
}

impl XorTableConfig {
    fn configure(meta: &mut ConstraintSystem<Fp>) -> Self {
        let input_a = meta.advice_column();
        let input_b = meta.advice_column();
        let output = meta.advice_column();

        let table_a = meta.lookup_table_column();
        let table_b = meta.lookup_table_column();
        let table_xor = meta.lookup_table_column();

        let selector = meta.complex_selector();

        // 多列 lookup
        meta.lookup("xor lookup", |meta| {
            let s = meta.query_selector(selector);
            let a = meta.query_advice(input_a, Rotation::cur());
            let b = meta.query_advice(input_b, Rotation::cur());
            let c = meta.query_advice(output, Rotation::cur());

            vec![
                (s.clone() * a, table_a),
                (s.clone() * b, table_b),
                (s * c, table_xor),
            ]
        });

        Self {
            input_a, input_b, output,
            table_a, table_b, table_xor,
            selector,
        }
    }

    /// 加载 XOR 表 (8-bit)
    fn load_xor_table(&self, layouter: &mut impl Layouter<Fp>) -> Result<(), Error> {
        layouter.assign_table(
            || "xor table",
            |mut table| {
                let mut row = 0;
                for a in 0u64..256 {
                    for b in 0u64..256 {
                        table.assign_cell(|| "a", self.table_a, row, || Value::known(Fp::from(a)))?;
                        table.assign_cell(|| "b", self.table_b, row, || Value::known(Fp::from(b)))?;
                        table.assign_cell(|| "xor", self.table_xor, row, || Value::known(Fp::from(a ^ b)))?;
                        row += 1;
                    }
                }
                Ok(())
            },
        )
    }
}
```

### 7.4 gnark Lookup 实现

```go
package lookup

import (
	"github.com/consensys/gnark/frontend"
	"github.com/consensys/gnark/std/lookup/logderivlookup"
)

// RangeCheckCircuit 使用 lookup 的范围检查
type RangeCheckCircuit struct {
	Values []frontend.Variable
	Bits   int // 范围 [0, 2^Bits)
}

func (c *RangeCheckCircuit) Define(api frontend.API) error {
	// 创建范围表
	rangeTable := make([]frontend.Variable, 1<<c.Bits)
	for i := range rangeTable {
		rangeTable[i] = i
	}

	// 使用 log-derivative lookup
	t := logderivlookup.New(api)
	t.Insert(rangeTable)

	// 检查所有值都在表中
	for _, v := range c.Values {
		t.Lookup(v)
	}

	return nil
}

// XORCircuit 使用 lookup 的 XOR
type XORCircuit struct {
	A, B   frontend.Variable
	Result frontend.Variable `gnark:",public"`
}

func (c *XORCircuit) Define(api frontend.API) error {
	// 构建 XOR 表
	xorTable := make([]frontend.Variable, 256*256)
	for a := 0; a < 256; a++ {
		for b := 0; b < 256; b++ {
			xorTable[a*256+b] = a ^ b
		}
	}

	t := logderivlookup.New(api)
	t.Insert(xorTable)

	// 计算索引并查表
	index := api.Add(api.Mul(c.A, 256), c.B)
	result := t.Lookup(index)

	api.AssertIsEqual(result[0], c.Result)
	return nil
}
```

### 7.5 Lookup 性能对比

```
┌─────────────────────────────────────────────────────────────────────┐
│                      Lookup vs 算术约束性能对比                       │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  操作                │ 算术约束数    │ Lookup 开销   │ 加速比        │
│  ─────────────────────────────────────────────────────────────────  │
│  范围检查 (8-bit)    │ ~16          │ ~1           │ 16x          │
│  范围检查 (16-bit)   │ ~32          │ ~2           │ 16x          │
│  XOR (8-bit)        │ ~24          │ ~1           │ 24x          │
│  XOR (256-bit)      │ ~768         │ ~32          │ 24x          │
│  AND (8-bit)        │ ~16          │ ~1           │ 16x          │
│  SHA256 round       │ ~25000       │ ~3000        │ 8x           │
│  Keccak permutation │ ~140000      │ ~15000       │ 9x           │
│  ECDSA 验证         │ ~500000      │ ~50000       │ 10x          │
│                                                                     │
│  最佳实践:                                                           │
│  • 对于重复使用的操作，使用 lookup                                   │
│  • 表大小控制在 2^16 ~ 2^20                                         │
│  • 合并相关的 lookup 减少开销                                        │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

## Part 8: PLONK 协议深度解析

### 8.1 PLONK 概述

PLONK (Permutations over Lagrange-bases for Oecumenical Noninteractive arguments of Knowledge) 是目前最流行的通用零知识证明协议之一。

```
┌─────────────────────────────────────────────────────────────────────┐
│                         PLONK 核心特性                              │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐ │
│  │   通用 Setup    │    │   高效证明      │    │   小证明尺寸    │ │
│  │                 │    │                 │    │                 │ │
│  │ 一次 setup     │    │ O(n log n)     │    │  ~400 bytes    │ │
│  │ 支持任意电路   │    │ 证明时间        │    │  恒定大小       │ │
│  └─────────────────┘    └─────────────────┘    └─────────────────┘ │
│                                                                     │
│  PLONK vs 其他协议:                                                 │
│  ┌───────────────┬──────────┬──────────┬──────────┬──────────────┐ │
│  │ 特性          │ Groth16  │ PLONK    │ STARK    │ Halo2        │ │
│  ├───────────────┼──────────┼──────────┼──────────┼──────────────┤ │
│  │ Setup         │ 电路特定 │ 通用     │ 透明     │ 通用+透明    │ │
│  │ 证明大小      │ ~200B    │ ~400B    │ ~50KB    │ ~400B        │ │
│  │ 验证时间      │ ~3ms     │ ~5ms     │ ~10ms    │ ~5ms         │ │
│  │ 假设          │ 配对     │ 配对     │ 哈希     │ 椭圆曲线     │ │
│  │ 后量子安全    │ 否       │ 否       │ 是       │ 否           │ │
│  └───────────────┴──────────┴──────────┴──────────┴──────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 8.2 PLONK 约束系统

PLONK 使用一种称为 "Plonkish" 的算术化方式:

```
┌─────────────────────────────────────────────────────────────────────┐
│                      PLONK 门约束结构                               │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  标准 PLONK 门方程:                                                 │
│                                                                     │
│  q_L · a + q_R · b + q_O · c + q_M · (a·b) + q_C = 0               │
│                                                                     │
│  其中:                                                              │
│  • a, b: 左右输入线                                                 │
│  • c: 输出线                                                        │
│  • q_L, q_R, q_O: 线性选择器                                       │
│  • q_M: 乘法选择器                                                  │
│  • q_C: 常数                                                        │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │        示例: 计算 x² + x + 5                                  │ │
│  ├───────────────────────────────────────────────────────────────┤ │
│  │  行  │  a   │  b   │  c   │ q_L │ q_R │ q_O │ q_M │ q_C      │ │
│  │ ───────────────────────────────────────────────────────────── │ │
│  │  0   │  x   │  x   │ x²   │  0  │  0  │ -1  │  1  │  0       │ │
│  │      │      │      │      │      (x · x - x² = 0)             │ │
│  │  1   │  x²  │  x   │  y   │  1  │  1  │ -1  │  0  │  5       │ │
│  │      │      │      │      │  (x² + x + 5 - y = 0)             │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 8.3 置换论证 (Permutation Argument)

PLONK 的核心创新是使用置换论证来实现 wire 之间的连接:

```
┌─────────────────────────────────────────────────────────────────────┐
│                      置换论证原理                                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  问题: 证明两个多项式 f 和 g 表示同一个多重集合的置换              │
│                                                                     │
│  原始方法 (Grand Product Argument):                                 │
│                                                                     │
│  构造累积多项式 Z(X):                                               │
│                                                                     │
│         ∏ᵢ (f(ωⁱ) + β·ωⁱ + γ)                                      │
│  Z(X) = ────────────────────────                                    │
│         ∏ᵢ (g(ωⁱ) + β·σ(ωⁱ) + γ)                                   │
│                                                                     │
│  其中:                                                              │
│  • ω: n 次单位根                                                    │
│  • β, γ: 随机挑战值                                                 │
│  • σ: 置换多项式                                                    │
│                                                                     │
│  验证条件:                                                          │
│  1. Z(1) = 1  (边界条件)                                           │
│  2. Z(ωⁿ) = 1 (循环条件)                                           │
│  3. 递推关系:                                                       │
│                                                                     │
│     Z(ωX) · (g(X) + β·σ(X) + γ) = Z(X) · (f(X) + β·X + γ)         │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                    置换示意图                                  │ │
│  │                                                                │ │
│  │    Wire A      Wire B      Wire C                             │ │
│  │    ┌───┐       ┌───┐       ┌───┐                              │ │
│  │  0 │ x │ ←───→ │ x │ 0     │x² │ 0                            │ │
│  │    ├───┤   σ   ├───┤       ├───┤                              │ │
│  │  1 │x² │ ←───→ │ x │ 1     │ y │ 1                            │ │
│  │    └───┘       └───┘       └───┘                              │ │
│  │                                                                │ │
│  │  σ 定义了哪些位置应该相等                                      │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 8.4 PLONK 协议流程

```
┌─────────────────────────────────────────────────────────────────────┐
│                     PLONK 证明生成流程                              │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Round 1: 承诺 witness 多项式                                       │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 构造 a(X), b(X), c(X) 插值多项式                         │   │
│  │  • 计算承诺 [a]₁, [b]₁, [c]₁                                │   │
│  │  • 添加 blinding factors 保护零知识                         │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                          ↓                                          │
│  Round 2: 置换论证 (收到挑战 β, γ)                                  │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 计算累积多项式 z(X)                                      │   │
│  │  • 计算承诺 [z]₁                                            │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                          ↓                                          │
│  Round 3: 商多项式 (收到挑战 α)                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 构造商多项式 t(X) = (门约束 + α·置换约束) / Zₕ(X)        │   │
│  │  • 分割 t(X) = t_lo + X^n · t_mid + X^{2n} · t_hi          │   │
│  │  • 计算承诺 [t_lo]₁, [t_mid]₁, [t_hi]₁                      │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                          ↓                                          │
│  Round 4: 求值 (收到挑战 ζ)                                         │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 计算求值: ā = a(ζ), b̄ = b(ζ), c̄ = c(ζ), ...            │   │
│  │  • 计算 z(ωζ) 用于置换论证                                  │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                          ↓                                          │
│  Round 5: 开放证明 (收到挑战 v)                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 构造线性化多项式 r(X)                                    │   │
│  │  • 计算开放证明 W_ζ(X), W_{ωζ}(X)                          │   │
│  │  • 输出证明 π = ([a]₁, [b]₁, [c]₁, [z]₁, [t]₁, W_ζ, W_ωζ)  │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 8.5 PLONK 验证算法

```go
// PLONK 验证器伪代码
package plonk

import (
	"math/big"
)

type PlonkProof struct {
	// Round 1: witness 承诺
	A, B, C Point // [a]₁, [b]₁, [c]₁

	// Round 2: 累积多项式承诺
	Z Point // [z]₁

	// Round 3: 商多项式承诺
	Tlo, Tmid, Thi Point

	// Round 4: 求值
	AEval, BEval, CEval *big.Int // a(ζ), b(ζ), c(ζ)
	S1Eval, S2Eval      *big.Int // σ₁(ζ), σ₂(ζ)
	ZEvalOmega          *big.Int // z(ωζ)

	// Round 5: 开放证明
	Wz, Wzomega Point // KZG 开放证明
}

type PlonkVerifier struct {
	// 预处理 (来自 setup)
	Qm, Ql, Qr, Qo, Qc Point   // 选择器承诺
	S1, S2, S3         Point   // 置换承诺
	X2                 PointG2 // [x]₂

	// 公共参数
	N      int      // 电路大小
	Omega  *big.Int // N 次单位根
	K1, K2 *big.Int // coset 生成元
}

func (v *PlonkVerifier) Verify(proof *PlonkProof, publicInputs []*big.Int) bool {
	// Step 1: 验证公共输入在域内
	for _, pi := range publicInputs {
		if !inField(pi) {
			return false
		}
	}

	// Step 2: 重新计算挑战值 (Fiat-Shamir)
	beta, gamma := computeChallenge1(proof.A, proof.B, proof.C)
	alpha := computeChallenge2(proof.Z)
	zeta := computeChallenge3(proof.Tlo, proof.Tmid, proof.Thi)
	nu := computeChallenge4(proof.AEval, proof.BEval, proof.CEval)
	mu := computeChallenge5(proof.Wz, proof.Wzomega)

	// Step 3: 计算消失多项式在 ζ 处的值
	zhZeta := computeZH(zeta, v.N) // ζ^n - 1

	// Step 4: 计算拉格朗日多项式 L₁(ζ)
	l1Zeta := computeL1(zeta, v.N, v.Omega)

	// Step 5: 计算公共输入多项式 PI(ζ)
	piZeta := evaluatePI(publicInputs, zeta, v.N)

	// Step 6: 验证门约束
	// r = a·b·qm + a·ql + b·qr + c·qo + qc + PI(ζ)
	//   + α·[(a + β·ζ + γ)(b + β·k₁·ζ + γ)(c + β·k₂·ζ + γ)·z
	//       - (a + β·σ₁(ζ) + γ)(b + β·σ₂(ζ) + γ)·β·σ₃(X)·z(ωζ)]
	//   + α²·(z - 1)·L₁(ζ)

	// Step 7: 配对检查
	// e(W_ζ + μ·W_{ωζ}, [x]₂) = e(ζ·W_ζ + μ·ω·ζ·W_{ωζ} + F - E, [1]₂)

	return verifyPairing(proof, v, zeta, mu, zhZeta)
}

// 计算消失多项式 Z_H(X) = X^n - 1
func computeZH(zeta *big.Int, n int) *big.Int {
	result := new(big.Int).Exp(zeta, big.NewInt(int64(n)), fieldOrder)
	result.Sub(result, big.NewInt(1))
	return result.Mod(result, fieldOrder)
}

// 计算第一个拉格朗日基多项式 L_1(ζ) = (ζ^n - 1) / (n · (ζ - 1))
func computeL1(zeta *big.Int, n int, omega *big.Int) *big.Int {
	// L_1(ζ) = (ζ^n - 1) / (n · (ζ - 1))
	numerator := computeZH(zeta, n)

	zetaMinusOne := new(big.Int).Sub(zeta, big.NewInt(1))
	denominator := new(big.Int).Mul(big.NewInt(int64(n)), zetaMinusOne)

	return new(big.Int).Div(numerator, denominator)
}
```

### 8.6 Custom Gates 扩展

Plonkish 系统支持自定义门来优化特定操作:

```rust
// Halo2 自定义门实现
use halo2_proofs::{
    circuit::{Layouter, SimpleFloorPlanner, Value},
    plonk::{Advice, Column, ConstraintSystem, Error, Expression, Selector},
    poly::Rotation,
};

/// 自定义 Poseidon 轮函数门
struct PoseidonConfig {
    state: [Column<Advice>; 3],
    round_constants: [Column<Advice>; 3],
    s_full: Selector,    // 完整轮选择器
    s_partial: Selector, // 部分轮选择器
}

impl PoseidonConfig {
    fn configure(meta: &mut ConstraintSystem<Fr>) -> Self {
        let state = [
            meta.advice_column(),
            meta.advice_column(),
            meta.advice_column(),
        ];
        let round_constants = [
            meta.advice_column(),
            meta.advice_column(),
            meta.advice_column(),
        ];
        let s_full = meta.selector();
        let s_partial = meta.selector();

        // 完整轮门: state' = MDS · (state + rc)^5
        meta.create_gate("full round", |meta| {
            let s = meta.query_selector(s_full);

            let state_cur: Vec<_> = state.iter()
                .map(|c| meta.query_advice(*c, Rotation::cur()))
                .collect();
            let state_next: Vec<_> = state.iter()
                .map(|c| meta.query_advice(*c, Rotation::next()))
                .collect();
            let rc: Vec<_> = round_constants.iter()
                .map(|c| meta.query_advice(*c, Rotation::cur()))
                .collect();

            // S-box: x^5
            let sbox = |x: Expression<Fr>| {
                let x2 = x.clone() * x.clone();
                let x4 = x2.clone() * x2.clone();
                x4 * x
            };

            // 应用 S-box
            let after_sbox: Vec<_> = state_cur.iter()
                .zip(rc.iter())
                .map(|(s, r)| sbox(s.clone() + r.clone()))
                .collect();

            // MDS 矩阵乘法
            let mds = get_mds_matrix();
            let after_mds = mds_multiply(&mds, &after_sbox);

            // 约束: state_next = MDS · sbox(state + rc)
            (0..3).map(|i| {
                s.clone() * (state_next[i].clone() - after_mds[i].clone())
            }).collect::<Vec<_>>()
        });

        // 部分轮门: 只对第一个元素应用 S-box
        meta.create_gate("partial round", |meta| {
            let s = meta.query_selector(s_partial);

            let state_cur: Vec<_> = state.iter()
                .map(|c| meta.query_advice(*c, Rotation::cur()))
                .collect();
            let state_next: Vec<_> = state.iter()
                .map(|c| meta.query_advice(*c, Rotation::next()))
                .collect();
            let rc: Vec<_> = round_constants.iter()
                .map(|c| meta.query_advice(*c, Rotation::cur()))
                .collect();

            let sbox = |x: Expression<Fr>| {
                let x2 = x.clone() * x.clone();
                let x4 = x2.clone() * x2.clone();
                x4 * x
            };

            // 只对第一个元素应用 sbox
            let mut after_sbox = vec![sbox(state_cur[0].clone() + rc[0].clone())];
            for i in 1..3 {
                after_sbox.push(state_cur[i].clone() + rc[i].clone());
            }

            let mds = get_mds_matrix();
            let after_mds = mds_multiply(&mds, &after_sbox);

            (0..3).map(|i| {
                s.clone() * (state_next[i].clone() - after_mds[i].clone())
            }).collect::<Vec<_>>()
        });

        Self {
            state,
            round_constants,
            s_full,
            s_partial,
        }
    }
}
```

### 8.7 PLONK 优化技术

```
┌─────────────────────────────────────────────────────────────────────┐
│                     PLONK 性能优化策略                              │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1. FFT 优化                                                        │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 使用 4-step FFT 减少内存访问                             │   │
│  │  • SIMD 并行化蝶形运算                                      │   │
│  │  • 缓存友好的数据布局                                       │   │
│  │  • GPU 加速的 NTT 实现                                      │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  2. 多标量乘法 (MSM) 优化                                           │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • Pippenger 算法: O(n/log n) 点加法                        │   │
│  │  • 预计算窗口法                                             │   │
│  │  • 批量仿射坐标转换                                         │   │
│  │  • GPU 并行 MSM                                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  3. 多项式承诺优化                                                  │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 批量开放: 多个点一个证明                                 │   │
│  │  • Lagrange 基直接承诺                                      │   │
│  │  • 预计算的验证密钥                                         │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  4. 证明聚合                                                        │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 同电路批处理验证                                         │   │
│  │  • 跨电路递归聚合                                           │   │
│  │  • 概率性批量验证                                           │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  性能基准 (2^20 约束):                                              │
│  ┌──────────────────┬────────────┬────────────┬────────────────┐  │
│  │ 操作             │ CPU (单核) │ CPU (多核) │ GPU           │  │
│  ├──────────────────┼────────────┼────────────┼────────────────┤  │
│  │ 证明生成         │ 60s        │ 15s        │ 3s            │  │
│  │ 验证             │ 5ms        │ 5ms        │ 5ms           │  │
│  │ FFT (2^20)       │ 2s         │ 0.5s       │ 0.1s          │  │
│  │ MSM (2^20)       │ 10s        │ 3s         │ 0.5s          │  │
│  └──────────────────┴────────────┴────────────┴────────────────┘  │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 8.8 gnark PLONK 实现

```go
package main

import (
	"github.com/consensys/gnark-crypto/ecc"
	"github.com/consensys/gnark/backend/plonk"
	"github.com/consensys/gnark/frontend"
	"github.com/consensys/gnark/frontend/cs/scs"
	"github.com/consensys/gnark/test"
)

// 复杂电路示例: Merkle 树验证
type MerkleCircuit struct {
	// 公共输入
	Root frontend.Variable `gnark:",public"`
	Leaf frontend.Variable `gnark:",public"`

	// 私有输入
	Path     [32]frontend.Variable // Merkle 路径
	PathBits [32]frontend.Variable // 路径方向 (0=左, 1=右)
}

func (c *MerkleCircuit) Define(api frontend.API) error {
	// 使用 MiMC 哈希
	mimc, _ := mimc.NewMiMC(api)

	current := c.Leaf

	for i := 0; i < 32; i++ {
		// 根据路径方向选择左右
		left := api.Select(c.PathBits[i], c.Path[i], current)
		right := api.Select(c.PathBits[i], current, c.Path[i])

		// 计算哈希
		mimc.Reset()
		mimc.Write(left, right)
		current = mimc.Sum()
	}

	// 验证计算出的根与给定根匹配
	api.AssertIsEqual(current, c.Root)

	return nil
}

func main() {
	// 编译电路
	var circuit MerkleCircuit
	ccs, _ := frontend.Compile(ecc.BN254.ScalarField(), scs.NewBuilder, &circuit)

	// 设置
	srs, _ := test.NewKZGSRS(ccs)
	pk, vk, _ := plonk.Setup(ccs, srs)

	// 准备 witness
	assignment := &MerkleCircuit{
		Root: "12345...", // 实际根值
		Leaf: "67890...", // 叶子值
		// ... 填充路径
	}

	witness, _ := frontend.NewWitness(assignment, ecc.BN254.ScalarField())
	publicWitness, _ := witness.Public()

	// 生成证明
	proof, _ := plonk.Prove(ccs, pk, witness)

	// 验证
	err := plonk.Verify(proof, vk, publicWitness)
	if err != nil {
		panic("verification failed")
	}
}
```

---

## Part 9: FRI 协议详解

### 9.1 FRI 概述

FRI (Fast Reed-Solomon Interactive Oracle Proof of Proximity) 是 STARK 的核心组件，用于证明多项式的低度性质。

```
┌─────────────────────────────────────────────────────────────────────┐
│                         FRI 核心特性                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  目标: 证明函数 f: D → F 接近某个低度多项式                         │
│                                                                     │
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐ │
│  │   透明 Setup    │    │   后量子安全    │    │   快速验证      │ │
│  │                 │    │                 │    │                 │ │
│  │ 无需可信设置   │    │ 仅基于哈希     │    │ O(log² n)      │ │
│  │ 仅使用哈希    │    │ 抗量子攻击     │    │ 验证时间        │ │
│  └─────────────────┘    └─────────────────┘    └─────────────────┘ │
│                                                                     │
│  FRI 在 ZK 系统中的位置:                                            │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                                                               │ │
│  │  STARK = AIR (算术化) + FRI (低度测试) + Merkle (承诺)       │ │
│  │                                                               │ │
│  │  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐       │ │
│  │  │    AIR      │ →  │    FRI      │ →  │   Merkle    │       │ │
│  │  │ 约束系统    │    │ 低度证明    │    │ 哈希承诺    │       │ │
│  │  └─────────────┘    └─────────────┘    └─────────────┘       │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 9.2 Reed-Solomon 码与低度测试

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Reed-Solomon 码基础                              │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  定义: RS[F, D, k] = { (f(x₁), ..., f(xₙ)) : deg(f) < k }          │
│                                                                     │
│  其中:                                                              │
│  • F: 有限域                                                        │
│  • D = {x₁, ..., xₙ} ⊂ F: 求值域                                   │
│  • k: 最大度数                                                      │
│                                                                     │
│  Reed-Solomon 码的性质:                                             │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                                                               │ │
│  │  码率 ρ = k/n        (信息位/码长)                            │ │
│  │  最小距离 d = n - k + 1  (汉明距离)                           │ │
│  │                                                               │ │
│  │  ┌─────────────────────────────────────────────────────────┐ │ │
│  │  │  例: n=8, k=2 (线性多项式)                              │ │ │
│  │  │                                                         │ │ │
│  │  │  多项式 f(x) = ax + b                                   │ │ │
│  │  │                                                         │ │ │
│  │  │  码字: (f(0), f(1), f(2), ..., f(7))                   │ │ │
│  │  │                                                         │ │ │
│  │  │  任意 2 点可唯一确定 f → 最小距离 = 7                  │ │ │
│  │  └─────────────────────────────────────────────────────────┘ │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  低度测试问题:                                                      │
│  给定函数 f: D → F，验证 f 是否接近度数 < k 的多项式               │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 9.3 FRI 协议流程

```
┌─────────────────────────────────────────────────────────────────────┐
│                       FRI 折叠过程                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  核心思想: 逐步将度数减半，直到变成常数                             │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                                                               │ │
│  │  初始: f₀(x), deg < d, 域大小 n                              │ │
│  │    ↓                                                          │ │
│  │  第1轮: f₁(x), deg < d/2, 域大小 n/2                         │ │
│  │    ↓                                                          │ │
│  │  第2轮: f₂(x), deg < d/4, 域大小 n/4                         │ │
│  │    ↓                                                          │ │
│  │   ...                                                         │ │
│  │    ↓                                                          │ │
│  │  最终: fₖ = 常数                                              │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  折叠操作 (以因子 2 为例):                                          │
│                                                                     │
│  给定 f(x) = f_even(x²) + x · f_odd(x²)                            │
│                                                                     │
│  定义 f'(y) = f_even(y) + α · f_odd(y)    (α 是随机挑战)           │
│                                                                     │
│  性质: deg(f') = deg(f)/2                                          │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │               FRI 折叠图示                                    │ │
│  │                                                               │ │
│  │  域 D₀ (大小 n):     x₀  x₁  x₂  x₃  x₄  x₅  x₆  x₇         │ │
│  │                       │   │   │   │   │   │   │   │          │ │
│  │                       └┬──┘   └┬──┘   └┬──┘   └┬──┘          │ │
│  │  映射 x → x²          │       │       │       │              │ │
│  │                       ↓       ↓       ↓       ↓              │ │
│  │  域 D₁ (大小 n/2):   y₀      y₁      y₂      y₃              │ │
│  │                       │       │       │       │              │ │
│  │                       └───┬───┘       └───┬───┘              │ │
│  │                           │               │                  │ │
│  │                           ↓               ↓                  │ │
│  │  域 D₂ (大小 n/4):       z₀              z₁                  │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 9.4 FRI 承诺阶段

```go
// FRI 承诺实现
package fri

import (
	"crypto/sha256"
	"math/big"
)

type FRICommitment struct {
	Layers      [][]FieldElement // 每一层的求值
	MerkleTrees []*MerkleTree    // 每一层的 Merkle 承诺
	FinalValue  FieldElement     // 最终常数值
}

type FRIParams struct {
	BlowupFactor int            // 扩展因子 (通常 4-16)
	NumQueries   int            // 查询数量
	MaxDegree    int            // 最大度数
	Domain       []FieldElement // 求值域
}

// 承诺阶段
func (p *FRIParams) Commit(polynomial []FieldElement) (*FRICommitment, []FieldElement) {
	commitment := &FRICommitment{
		Layers:      make([][]FieldElement, 0),
		MerkleTrees: make([]*MerkleTree, 0),
	}

	// 当前多项式求值
	currentEvals := p.evaluateOnDomain(polynomial)
	challenges := make([]FieldElement, 0)

	// 迭代折叠直到度数为常数
	for len(currentEvals) > p.BlowupFactor {
		// 承诺当前层
		commitment.Layers = append(commitment.Layers, currentEvals)
		tree := NewMerkleTree(currentEvals)
		commitment.MerkleTrees = append(commitment.MerkleTrees, tree)

		// 生成随机挑战 (Fiat-Shamir)
		alpha := p.generateChallenge(tree.Root())
		challenges = append(challenges, alpha)

		// 折叠
		currentEvals = p.foldLayer(currentEvals, alpha)
	}

	// 存储最终常数
	commitment.FinalValue = currentEvals[0]

	return commitment, challenges
}

// 折叠单层
func (p *FRIParams) foldLayer(evals []FieldElement, alpha FieldElement) []FieldElement {
	n := len(evals) / 2
	folded := make([]FieldElement, n)

	for i := 0; i < n; i++ {
		// f(x) 和 f(-x) 的值
		fx := evals[i]
		fMinusX := evals[n+i]

		// f_even = (f(x) + f(-x)) / 2
		// f_odd = (f(x) - f(-x)) / (2x)
		fEven := fx.Add(fMinusX).Mul(InverseOf2)
		fOdd := fx.Sub(fMinusX).Mul(InverseOf2).Mul(p.Domain[i].Inverse())

		// 折叠: f'(x²) = f_even + α · f_odd
		folded[i] = fEven.Add(alpha.Mul(fOdd))
	}

	return folded
}

// 在域上求值
func (p *FRIParams) evaluateOnDomain(poly []FieldElement) []FieldElement {
	// 使用 FFT 进行快速求值
	return FFT(poly, p.Domain)
}

// Fiat-Shamir 生成挑战
func (p *FRIParams) generateChallenge(merkleRoot []byte) FieldElement {
	hash := sha256.Sum256(merkleRoot)
	return FieldElementFromBytes(hash[:])
}
```

### 9.5 FRI 查询阶段

```go
// FRI 查询和验证
type FRIQuery struct {
Index       int
Values      []FieldElement // 每层的查询值
AuthPaths   [][]*MerkleProof // Merkle 认证路径
}

// 生成查询证明
func (c *FRICommitment) GenerateQueries(
params *FRIParams,
challenges []FieldElement,
) []FRIQuery {
queries := make([]FRIQuery, params.NumQueries)

for q := 0; q < params.NumQueries; q++ {
// 随机选择查询索引
index := params.sampleIndex(q, len(c.Layers[0]))

query := FRIQuery{
Index:     index,
Values:    make([]FieldElement, len(c.Layers)),
AuthPaths: make([][]*MerkleProof, len(c.Layers)),
}

currentIndex := index
for layer := 0; layer < len(c.Layers); layer++ {
// 获取查询点和其"兄弟"点
layerSize := len(c.Layers[layer])
siblingIndex := (currentIndex + layerSize/2) % layerSize

// 记录值
query.Values[layer] = c.Layers[layer][currentIndex]

// 生成 Merkle 证明
query.AuthPaths[layer] = append(query.AuthPaths[layer],
c.MerkleTrees[layer].GetProof(currentIndex),
c.MerkleTrees[layer].GetProof(siblingIndex),
)

// 更新索引 (折叠后的索引)
currentIndex = currentIndex % (layerSize / 2)
}

queries[q] = query
}

return queries
}

// 验证 FRI 证明
func VerifyFRI(
params *FRIParams,
commitment *FRICommitment,
challenges []FieldElement,
queries []FRIQuery,
) bool {

for _, query := range queries {
currentIndex := query.Index

for layer := 0; layer < len(commitment.MerkleTrees); layer++ {
layerSize := len(commitment.Layers[layer])
siblingIndex := (currentIndex + layerSize/2) % layerSize

// 验证 Merkle 证明
if !commitment.MerkleTrees[layer].Verify(
query.AuthPaths[layer][0],
currentIndex,
) {
return false
}

// 获取 f(x) 和 f(-x)
fx := query.Values[layer]
fMinusX := commitment.Layers[layer][siblingIndex]

// 计算折叠值
alpha := challenges[layer]
fEven := fx.Add(fMinusX).Mul(InverseOf2)
fOdd := fx.Sub(fMinusX).Mul(InverseOf2).Mul(
params.Domain[currentIndex].Inverse(),
)
expectedNext := fEven.Add(alpha.Mul(fOdd))

// 验证下一层的值
if layer < len(commitment.MerkleTrees)-1 {
nextIndex := currentIndex % (layerSize / 2)
if !query.Values[layer+1].Equal(expectedNext) {
return false
}
currentIndex = nextIndex
} else {
// 最后一层,检查常数
if !expectedNext.Equal(commitment.FinalValue) {
return false
}
}
}
}

return true
}
```

### 9.6 FRI 安全性分析

```
┌─────────────────────────────────────────────────────────────────────┐
│                      FRI 安全性参数                                 │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  安全级别计算:                                                      │
│                                                                     │
│  给定:                                                              │
│  • ρ = k/n: 码率 (通常 1/4 到 1/16)                                │
│  • t: 查询数量                                                      │
│  • λ: 安全参数 (通常 128 bit)                                      │
│                                                                     │
│  安全误差 ε ≤ (1 - δ)^t                                            │
│                                                                     │
│  其中 δ = 1 - √ρ (唯一解码半径)                                    │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │              参数选择示例                                      │ │
│  ├───────────────────────────────────────────────────────────────┤ │
│  │  码率 ρ    │ δ          │ 128-bit 所需查询数                 │ │
│  │ ───────────────────────────────────────────────────────────── │ │
│  │  1/4       │ 0.5        │ ~185 次                             │ │
│  │  1/8       │ 0.646      │ ~127 次                             │ │
│  │  1/16      │ 0.75       │ ~92 次                              │ │
│  │  1/32      │ 0.823      │ ~70 次                              │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  证明大小:                                                          │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                                                               │ │
│  │  证明大小 ≈ t × log(n) × 32 bytes (Merkle 路径)              │ │
│  │                 × log(d)          (折叠层数)                  │ │
│  │                                                               │ │
│  │  例: n = 2²⁰, d = 2¹⁶, t = 100                               │ │
│  │  证明大小 ≈ 100 × 20 × 16 × 32 ≈ 1 MB                        │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  优化技术:                                                          │
│  • Grinding: 增加工作量证明减少查询数                              │
│  • 批处理: 多个多项式共享 Merkle 树                                │
│  • DEEP-FRI: 在单点进行深度测试                                    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 9.7 DEEP-FRI 优化

```go
// DEEP-FRI: 更高效的 FRI 变体
type DEEPFRIProof struct {
// 标准 FRI 承诺
FRICommitment *FRICommitment

// DEEP 组合
DeepPoint     FieldElement // 域外随机点 z
DeepValues    []FieldElement // 在 z 点的求值
QuotientProof *FRICommitment // 商多项式证明
}

// DEEP-FRI 关键思想:
// 不直接证明 f(x) 是低度多项式
// 而是证明 (f(x) - f(z)) / (x - z) 是低度多项式
// 其中 z 是域外的随机点

func DEEPFRIProve(
params *FRIParams,
f []FieldElement, // 多项式系数
constraint func ([]FieldElement) FieldElement, // 约束
) *DEEPFRIProof {

// Step 1: 承诺原始多项式
fCommit, _ := params.Commit(f)

// Step 2: 收到随机点 z (Fiat-Shamir)
z := params.generateChallenge(fCommit.MerkleTrees[0].Root())

// Step 3: 计算 f(z)
fz := EvaluatePolynomial(f, z)

// Step 4: 构造商多项式 g(x) = (f(x) - f(z)) / (x - z)
g := make([]FieldElement, len(f)-1)
// ... 多项式除法

// Step 5: 对商多项式运行 FRI
gCommit, challenges := params.Commit(g)

// Step 6: 生成查询
queries := gCommit.GenerateQueries(params, challenges)

return &DEEPFRIProof{
FRICommitment: fCommit,
DeepPoint:     z,
DeepValues:    []FieldElement{fz},
QuotientProof: gCommit,
}
}

// DEEP-FRI 优势:
// 1. 单点测试即可确认多项式度数
// 2. 减少需要的查询数量
// 3. 更小的证明尺寸
```

### 9.8 FRI 在 STARK 中的应用

```
┌─────────────────────────────────────────────────────────────────────┐
│                    STARK = AIR + FRI                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  完整 STARK 证明流程:                                               │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  1. 生成执行轨迹 (Trace)                                    │   │
│  │     • t₀, t₁, t₂, ..., tₙ₋₁ (状态序列)                     │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                          ↓                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  2. 插值为多项式 T(x)                                       │   │
│  │     • T(ωⁱ) = tᵢ for i = 0, ..., n-1                       │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                          ↓                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  3. 构造约束多项式 C(x)                                     │   │
│  │     • C(x) = constraint(T(x), T(ωx), ...)                  │   │
│  │     • C(x) 在 trace 域上为零                               │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                          ↓                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  4. 构造商多项式                                            │   │
│  │     • Q(x) = C(x) / Z_trace(x)                             │   │
│  │     • Z_trace(x) = ∏(x - ωⁱ) 是消失多项式                  │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                          ↓                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  5. 组合并运行 FRI                                          │   │
│  │     • 证明 Q(x) 是低度多项式                               │   │
│  │     • 使用随机线性组合减少多项式数量                        │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                          ↓                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  6. 生成查询证明                                            │   │
│  │     • 选择随机查询点                                        │   │
│  │     • 提供 Merkle 认证路径                                  │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

```rust
// Rust STARK 实现框架 (使用 winterfell 风格)
use std::marker::PhantomData;

pub trait Air {
    type BaseField: Field;
    type PublicInputs;

    // 定义约束
    fn evaluate_transition<E: FieldElement>(
        &self,
        frame: &EvaluationFrame<E>,
        periodic_values: &[E],
        result: &mut [E],
    );

    // 边界约束
    fn get_boundary_constraints(&self) -> Vec<BoundaryConstraint>;

    // 获取轨迹信息
    fn trace_length(&self) -> usize;
    fn trace_width(&self) -> usize;
}

// 示例: 斐波那契 STARK
pub struct FibonacciAir {
    trace_length: usize,
    result: u64,
}

impl Air for FibonacciAir {
    type BaseField = Fp;
    type PublicInputs = u64;

    fn evaluate_transition<E: FieldElement>(
        &self,
        frame: &EvaluationFrame<E>,
        _periodic: &[E],
        result: &mut [E],
    ) {
        let current = frame.current();
        let next = frame.next();

        // 约束: f(n+2) = f(n+1) + f(n)
        // 转换为: next[0] - current[0] - current[1] = 0
        //         next[1] - current[1] - next[0] = 0
        result[0] = next[0] - current[0] - current[1];
        result[1] = next[1] - current[1] - next[0];
    }

    fn get_boundary_constraints(&self) -> Vec<BoundaryConstraint> {
        vec![
            // f(0) = 1
            BoundaryConstraint::new(0, 0, E::ONE),
            // f(1) = 1
            BoundaryConstraint::new(0, 1, E::ONE),
            // f(n-1) = result
            BoundaryConstraint::new(
                self.trace_length - 1,
                1,
                E::from(self.result),
            ),
        ]
    }

    fn trace_length(&self) -> usize {
        self.trace_length
    }

    fn trace_width(&self) -> usize {
        2
    }
}
```

---

## Part 10: zkEVM 电路设计

### 10.1 zkEVM 架构概述

```
┌─────────────────────────────────────────────────────────────────────┐
│                      zkEVM 整体架构                                 │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                        EVM 执行                               │ │
│  │  ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐      │ │
│  │  │  交易   │ → │  状态   │ → │  操作码  │ → │ 新状态  │      │ │
│  │  │  输入   │   │  读取   │   │  执行   │   │  写入   │      │ │
│  │  └─────────┘   └─────────┘   └─────────┘   └─────────┘      │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                              ↓                                      │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                      电路层                                   │ │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐        │ │
│  │  │ EVM 电路 │ │ 状态电路 │ │ 字节码   │ │ 存储电路 │        │ │
│  │  │          │ │          │ │ 电路     │ │          │        │ │
│  │  └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘        │ │
│  │       │            │            │            │               │ │
│  │       └────────────┴────────────┴────────────┘               │ │
│  │                         ↓                                    │ │
│  │               ┌──────────────────┐                          │ │
│  │               │   聚合电路       │                          │ │
│  │               │   (Aggregator)   │                          │ │
│  │               └──────────────────┘                          │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                              ↓                                      │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                      证明层                                   │ │
│  │                                                               │ │
│  │    PLONK/STARK 证明  →  递归聚合  →  最终证明 (~400B)        │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 10.2 zkEVM 电路分类

```
┌─────────────────────────────────────────────────────────────────────┐
│                      核心电路组件                                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1. EVM 电路 (EVM Circuit)                                         │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 验证每条 EVM 指令的执行                                  │   │
│  │  • 约束操作码语义                                            │   │
│  │  • 管理程序计数器 (PC)                                       │   │
│  │  • 追踪 gas 消耗                                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  2. 状态电路 (State Circuit)                                       │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 验证账户状态转换                                          │   │
│  │  • 余额、nonce、代码哈希变化                                 │   │
│  │  • 存储槽读写                                                 │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  3. 字节码电路 (Bytecode Circuit)                                  │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 验证合约字节码正确性                                      │   │
│  │  • PUSH 指令数据提取                                         │   │
│  │  • 代码哈希计算                                              │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  4. 复制电路 (Copy Circuit)                                        │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • CALLDATACOPY, CODECOPY, RETURNDATACOPY                   │   │
│  │  • 验证内存复制操作                                          │   │
│  │  • 处理跨区域数据移动                                        │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  5. Keccak 电路                                                    │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • SHA3 操作码                                               │   │
│  │  • 地址派生                                                  │   │
│  │  • 存储键计算                                                │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 10.3 EVM 电路实现

```rust
// zkEVM EVM 电路 (Halo2 风格)
use halo2_proofs::{
    circuit::{Layouter, Value},
    plonk::{Advice, Column, ConstraintSystem, Error, Expression, Selector},
    poly::Rotation,
};

/// EVM 执行步骤
#[derive(Clone, Debug)]
pub struct ExecutionStep {
    pub pc: u64,
    pub opcode: u8,
    pub stack: Vec<U256>,
    pub memory: Vec<u8>,
    pub gas_left: u64,
    pub rwc: u32,  // 读写计数器
}

/// EVM 电路配置
#[derive(Clone)]
pub struct EvmCircuitConfig {
    // 执行状态列
    pub pc: Column<Advice>,
    pub opcode: Column<Advice>,
    pub gas_left: Column<Advice>,
    pub stack_pointer: Column<Advice>,
    pub memory_size: Column<Advice>,
    pub rwc: Column<Advice>,

    // 栈操作列
    pub stack_value: [Column<Advice>; 4],

    // 选择器
    pub q_step: Selector,
    pub q_step_first: Selector,
    pub q_step_last: Selector,

    // 操作码特定选择器
    pub opcode_selectors: OpcodeSelectors,
}

#[derive(Clone)]
pub struct OpcodeSelectors {
    pub q_add: Selector,
    pub q_mul: Selector,
    pub q_sub: Selector,
    pub q_div: Selector,
    pub q_push: Selector,
    pub q_pop: Selector,
    pub q_jump: Selector,
    pub q_jumpi: Selector,
    pub q_sload: Selector,
    pub q_sstore: Selector,
    pub q_call: Selector,
    // ... 更多操作码
}

impl EvmCircuitConfig {
    pub fn configure(meta: &mut ConstraintSystem<Fr>) -> Self {
        let pc = meta.advice_column();
        let opcode = meta.advice_column();
        let gas_left = meta.advice_column();
        let stack_pointer = meta.advice_column();
        let memory_size = meta.advice_column();
        let rwc = meta.advice_column();

        let stack_value = [
            meta.advice_column(),
            meta.advice_column(),
            meta.advice_column(),
            meta.advice_column(),
        ];

        let q_step = meta.selector();
        let q_step_first = meta.selector();
        let q_step_last = meta.selector();

        // ADD 操作码约束
        let q_add = meta.selector();
        meta.create_gate("ADD opcode", |meta| {
            let q = meta.query_selector(q_add);

            let a = meta.query_advice(stack_value[0], Rotation::cur());
            let b = meta.query_advice(stack_value[1], Rotation::cur());
            let result = meta.query_advice(stack_value[0], Rotation::next());

            // 栈指针变化: pop 2, push 1 → SP' = SP - 1
            let sp_cur = meta.query_advice(stack_pointer, Rotation::cur());
            let sp_next = meta.query_advice(stack_pointer, Rotation::next());

            // Gas 消耗: ADD = 3
            let gas_cur = meta.query_advice(gas_left, Rotation::cur());
            let gas_next = meta.query_advice(gas_left, Rotation::next());

            vec![
                // result = (a + b) mod 2^256
                q.clone() * (result - a - b),  // 简化,实际需要处理溢出
                // SP' = SP - 1
                q.clone() * (sp_next - sp_cur + Expression::Constant(Fr::one())),
                // Gas' = Gas - 3
                q * (gas_next - gas_cur + Expression::Constant(Fr::from(3))),
            ]
        });

        // SLOAD 操作码约束 (需要与状态电路交互)
        let q_sload = meta.selector();
        meta.create_gate("SLOAD opcode", |meta| {
            let q = meta.query_selector(q_sload);

            let key = meta.query_advice(stack_value[0], Rotation::cur());
            let value = meta.query_advice(stack_value[0], Rotation::next());
            let rwc_cur = meta.query_advice(rwc, Rotation::cur());
            let rwc_next = meta.query_advice(rwc, Rotation::next());

            vec![
                // RWC 增加 1 (读操作)
                q.clone() * (rwc_next - rwc_cur - Expression::Constant(Fr::one())),
                // 实际值验证通过 lookup 到状态电路
            ]
        });

        // JUMP 操作码约束
        let q_jump = meta.selector();
        meta.create_gate("JUMP opcode", |meta| {
            let q = meta.query_selector(q_jump);

            let dest = meta.query_advice(stack_value[0], Rotation::cur());
            let pc_next = meta.query_advice(pc, Rotation::next());
            let sp_cur = meta.query_advice(stack_pointer, Rotation::cur());
            let sp_next = meta.query_advice(stack_pointer, Rotation::next());

            vec![
                // PC' = dest
                q.clone() * (pc_next - dest),
                // SP' = SP - 1
                q * (sp_next - sp_cur + Expression::Constant(Fr::one())),
                // 注意: 需要额外约束 dest 是有效的 JUMPDEST
            ]
        });

        // ... 配置更多操作码

        Self {
            pc,
            opcode,
            gas_left,
            stack_pointer,
            memory_size,
            rwc,
            stack_value,
            q_step,
            q_step_first,
            q_step_last,
            opcode_selectors: OpcodeSelectors {
                q_add,
                q_mul: meta.selector(),
                q_sub: meta.selector(),
                q_div: meta.selector(),
                q_push: meta.selector(),
                q_pop: meta.selector(),
                q_jump,
                q_jumpi: meta.selector(),
                q_sload,
                q_sstore: meta.selector(),
                q_call: meta.selector(),
            },
        }
    }
}
```

### 10.4 状态电路实现

```rust
// 状态电路: 追踪所有状态读写操作
use halo2_proofs::plonk::*;

#[derive(Clone, Copy, Debug)]
pub enum RwOp {
    Read = 0,
    Write = 1,
}

#[derive(Clone, Copy, Debug)]
pub enum Target {
    Memory = 0,
    Stack = 1,
    Storage = 2,
    CallContext = 3,
    Account = 4,
    TxAccessList = 5,
}

/// 读写行
#[derive(Clone, Debug)]
pub struct RwRow {
    pub rwc: u32,           // 全局读写计数器
    pub is_write: bool,
    pub target: Target,
    pub id: u64,            // call_id 或 tx_id
    pub address: U256,      // 地址或键
    pub field_tag: u8,      // 字段类型
    pub storage_key: U256,
    pub value: U256,
    pub value_prev: U256,   // 写操作的前值
}

/// 状态电路配置
#[derive(Clone)]
pub struct StateCircuitConfig {
    pub rwc: Column<Advice>,
    pub is_write: Column<Advice>,
    pub target: Column<Advice>,
    pub id: Column<Advice>,
    pub address: Column<Advice>,
    pub field_tag: Column<Advice>,
    pub storage_key: Column<Advice>,
    pub value: Column<Advice>,
    pub value_prev: Column<Advice>,

    pub q_first: Selector,
    pub q_last: Selector,
    pub q_sorted: Selector,
}

impl StateCircuitConfig {
    pub fn configure(meta: &mut ConstraintSystem<Fr>) -> Self {
        // ... 列定义

        // 核心约束: 确保 RW 操作按 (target, id, address, field, storage_key, rwc) 排序
        // 这允许高效验证读取值的正确性

        meta.create_gate("sorted constraint", |meta| {
            let q = meta.query_selector(q_sorted);

            // 当前行
            let target_cur = meta.query_advice(target, Rotation::cur());
            let id_cur = meta.query_advice(id, Rotation::cur());
            let address_cur = meta.query_advice(address, Rotation::cur());
            let rwc_cur = meta.query_advice(rwc, Rotation::cur());

            // 下一行
            let target_next = meta.query_advice(target, Rotation::next());
            let id_next = meta.query_advice(id, Rotation::next());
            let address_next = meta.query_advice(address, Rotation::next());
            let rwc_next = meta.query_advice(rwc, Rotation::next());

            // 排序约束: 按字典序排列
            // (target, id, address, ..., rwc) <= (target', id', address', ..., rwc')
            // 简化实现,实际需要多级比较
            vec![
                // 如果 target 相同,检查 id
                // 如果 target 和 id 相同,检查 address
                // ...
                // 最后检查 rwc 严格递增 (同位置)
            ]
        });

        // 读写一致性约束
        meta.create_gate("read consistency", |meta| {
            let q = meta.query_selector(q_sorted);

            let is_write = meta.query_advice(is_write, Rotation::cur());
            let value = meta.query_advice(value, Rotation::cur());
            let value_prev = meta.query_advice(value_prev, Rotation::cur());

            // 上一行的值
            let prev_value = meta.query_advice(value, Rotation::prev());
            let prev_is_write = meta.query_advice(is_write, Rotation::prev());

            // 如果当前是读操作,值必须等于最近的写入值
            // 如果是第一次访问,值为 0

            vec![
                // 读操作: value = prev_value (如果是同一位置)
                // 写操作: value_prev = prev_value
            ]
        });

        // ... 返回配置
        unimplemented!()
    }
}
```

### 10.5 Keccak 电路

```rust
// Keccak-256 电路实现
// Keccak 使用位运算 (XOR, AND, NOT, ROT),在 ZK 中开销很大

/// Keccak 电路配置
pub struct KeccakCircuitConfig {
    // 状态矩阵 (5x5x64 bits)
    pub state: [[Column<Advice>; 5]; 5],

    // 轮常数
    pub round_constants: Column<Fixed>,

    // 选择器
    pub q_absorb: Selector,
    pub q_round: Selector,
    pub q_squeeze: Selector,

    // 位分解列 (用于验证位运算)
    pub bits: [Column<Advice>; 64],
}

/// Keccak 轮函数约束
impl KeccakCircuitConfig {
    pub fn configure(meta: &mut ConstraintSystem<Fr>) -> Self {
        // Keccak-f[1600] 的 5 个步骤

        // 1. θ (Theta) - 列奇偶校验
        meta.create_gate("theta", |meta| {
            // C[x] = A[x,0] ⊕ A[x,1] ⊕ A[x,2] ⊕ A[x,3] ⊕ A[x,4]
            // D[x] = C[x-1] ⊕ rot(C[x+1], 1)
            // A'[x,y] = A[x,y] ⊕ D[x]

            // 使用 lookup 验证 XOR 操作
            vec![]
        });

        // 2. ρ (Rho) - 位旋转
        meta.create_gate("rho", |meta| {
            // A'[x,y] = rot(A[x,y], r[x,y])
            // r[x,y] 是预定义的旋转偏移量

            // 验证旋转: 将值分解为位,重新组合
            vec![]
        });

        // 3. π (Pi) - 位置置换
        meta.create_gate("pi", |meta| {
            // A'[y, 2x+3y] = A[x,y]
            // 这是纯粹的线置换,只需要正确赋值
            vec![]
        });

        // 4. χ (Chi) - 非线性层
        meta.create_gate("chi", |meta| {
            // A'[x,y] = A[x,y] ⊕ (¬A[x+1,y] ∧ A[x+2,y])

            // 这是 Keccak 唯一的非线性操作
            // 需要使用 lookup 或位分解来验证
            vec![]
        });

        // 5. ι (Iota) - 轮常数加法
        meta.create_gate("iota", |meta| {
            // A'[0,0] = A[0,0] ⊕ RC[round]
            // RC 是预计算的轮常数
            vec![]
        });

        unimplemented!()
    }
}

// Keccak 优化策略
/*
┌─────────────────────────────────────────────────────────────────────┐
│                    Keccak 电路优化                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1. 稀疏表示                                                        │
│     • 将 64-bit word 分解为 4-bit chunks                           │
│     • 减少 XOR 的约束数量                                          │
│                                                                     │
│  2. 查找表                                                          │
│     • 预计算 XOR, AND 操作的查找表                                  │
│     • 使用 Plookup 验证位运算                                       │
│                                                                     │
│  3. 批处理                                                          │
│     • 多个 Keccak 调用共享表                                        │
│     • 减少每次调用的边际成本                                        │
│                                                                     │
│  4. 特殊编码                                                        │
│     • θ-ρ-π 可以合并优化                                           │
│     • 使用代数技巧减少约束                                          │
│                                                                     │
│  性能对比:                                                          │
│  ┌─────────────────────────┬───────────────┬───────────────┐       │
│  │ 方法                    │ 每轮约束数    │ 总约束 (24轮) │       │
│  ├─────────────────────────┼───────────────┼───────────────┤       │
│  │ 朴素实现                │ ~150,000      │ ~3,600,000    │       │
│  │ 位分解 + Lookup         │ ~25,000       │ ~600,000      │       │
│  │ 稀疏表示                │ ~6,000        │ ~144,000      │       │
│  │ 高度优化 (PSE zkEVM)    │ ~3,000        │ ~72,000       │       │
│  └─────────────────────────┴───────────────┴───────────────┘       │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
*/
```

### 10.6 电路间通信: Bus Mapping

```rust
// Bus Mapping: 连接各个子电路
// 使用 Lookup Arguments 实现电路间数据验证

/// Bus 操作类型
#[derive(Clone, Copy)]
pub enum BusOp {
    // 内存操作
    MemoryRead { call_id: u64, address: u64, value: U256 },
    MemoryWrite { call_id: u64, address: u64, value: U256 },

    // 栈操作
    StackRead { call_id: u64, index: usize, value: U256 },
    StackWrite { call_id: u64, index: usize, value: U256 },

    // 存储操作
    StorageRead { address: Address, key: U256, value: U256 },
    StorageWrite { address: Address, key: U256, value: U256, value_prev: U256 },

    // 代码操作
    BytecodeRead { code_hash: H256, index: u64, value: u8 },

    // 账户操作
    AccountRead { address: Address, field: AccountField, value: U256 },
}

/// Bus Mapping 配置
pub struct BusMappingConfig {
    // EVM 电路发出的操作
    pub evm_ops: Column<Advice>,

    // 状态电路的 RW 表
    pub rw_table: [Column<Advice>; 8],

    // 字节码表
    pub bytecode_table: [Column<Advice>; 4],

    // Keccak 表
    pub keccak_table: [Column<Advice>; 3],
}

impl BusMappingConfig {
    pub fn configure_lookups(meta: &mut ConstraintSystem<Fr>) {
        // EVM 电路查找状态电路
        meta.lookup_any("evm_to_state", |meta| {
            // 当 EVM 执行 SLOAD/SSTORE 时
            // 必须在状态电路的 RW 表中找到对应条目
            vec![]
        });

        // EVM 电路查找字节码电路
        meta.lookup_any("evm_to_bytecode", |meta| {
            // 每条执行的指令必须在字节码表中
            vec![]
        });

        // 状态电路查找 Keccak 电路
        meta.lookup_any("state_to_keccak", |meta| {
            // 存储键计算需要 Keccak
            vec![]
        });
    }
}

/*
┌─────────────────────────────────────────────────────────────────────┐
│                  电路互连示意图                                     │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌──────────────┐                   ┌──────────────┐               │
│  │  EVM 电路    │                   │  状态电路    │               │
│  │              │   lookup          │              │               │
│  │  SLOAD ──────────────────────────► RW Table    │               │
│  │  SSTORE ─────────────────────────►             │               │
│  │              │                   │              │               │
│  │  PC ─────────────────────────┐   └──────────────┘               │
│  │              │               │                                  │
│  └──────────────┘               │   ┌──────────────┐               │
│                                 │   │  字节码电路  │               │
│                                 └──►│              │               │
│                                     │  Code Table  │               │
│                                     └──────────────┘               │
│                                                                     │
│  ┌──────────────┐                   ┌──────────────┐               │
│  │  Copy 电路   │                   │  Keccak 电路 │               │
│  │              │   lookup          │              │               │
│  │  CODECOPY ───────────────────────► Hash Table  │               │
│  │  SHA3 ───────────────────────────►             │               │
│  │              │                   │              │               │
│  └──────────────┘                   └──────────────┘               │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
*/
```

### 10.7 zkEVM 实现对比

```
┌─────────────────────────────────────────────────────────────────────┐
│                    主流 zkEVM 实现对比                              │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  Polygon zkEVM (Type 2)                                     │   │
│  │  • 证明系统: PLONK + KZG                                    │   │
│  │  • 语言: Go (节点) + Rust (证明器)                          │   │
│  │  • 特点: 生产就绪,高兼容性                                  │   │
│  │  • 约束数: ~2B (整个块)                                     │   │
│  │  • 证明时间: ~3 分钟 (GPU)                                  │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  zkSync Era (Type 4)                                        │   │
│  │  • 证明系统: 定制 PLONK (Boojum)                            │   │
│  │  • 语言: Rust                                               │   │
│  │  • 特点: 自定义 VM,高性能                                   │   │
│  │  • 约束数: 优化的电路设计                                   │   │
│  │  • 证明时间: ~10 秒 (GPU)                                   │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  Scroll (Type 2-3)                                          │   │
│  │  • 证明系统: Halo2 + KZG                                    │   │
│  │  • 语言: Rust                                               │   │
│  │  • 特点: 基于 PSE zkEVM,字节码兼容                          │   │
│  │  • 约束数: ~1.5B (整个块)                                   │   │
│  │  • 证明时间: ~5 分钟 (GPU)                                  │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  StarkNet (Type 4)                                          │   │
│  │  • 证明系统: STARK                                          │   │
│  │  • 语言: Rust + Cairo                                       │   │
│  │  • 特点: 后量子安全,无可信设置                              │   │
│  │  • 约束数: AIR 约束                                         │   │
│  │  • 证明时间: ~30 秒 (CPU)                                   │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  Linea (Type 2)                                             │   │
│  │  • 证明系统: 定制 SNARK                                     │   │
│  │  • 语言: Go + Kotlin                                        │   │
│  │  • 特点: Consensys 开发,vortex 证明器                       │   │
│  │  • 约束数: 优化的设计                                       │   │
│  │  • 证明时间: ~15 秒 (GPU)                                   │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  性能对比:                                                          │
│  ┌─────────────────┬──────────┬──────────┬──────────┬──────────┐  │
│  │ 指标            │ Polygon  │ zkSync   │ Scroll   │ StarkNet │  │
│  ├─────────────────┼──────────┼──────────┼──────────┼──────────┤  │
│  │ TPS (理论)      │ 2000     │ 3000     │ 1500     │ 1000     │  │
│  │ 证明成本/tx     │ ~$0.01   │ ~$0.005  │ ~$0.01   │ ~$0.02   │  │
│  │ 最终性          │ ~30 min  │ ~1 hour  │ ~30 min  │ ~3 hour  │  │
│  │ 后量子安全      │ 否       │ 否       │ 否       │ 是       │  │
│  └─────────────────┴──────────┴──────────┴──────────┴──────────┘  │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

## Part 11: Nova/Supernova 折叠方案

### 11.1 折叠方案概述

Nova 是一种增量可验证计算 (IVC) 方案，使用"折叠"技术实现高效的递归证明。

```
┌─────────────────────────────────────────────────────────────────────┐
│                      折叠方案 vs 传统递归                           │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  传统递归证明:                                                      │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                                                               │ │
│  │  π₁ ──► Verify(π₁) ──► π₁' ──► Verify(π₁') ──► π₁'' ──► ... │ │
│  │                                                               │ │
│  │  每一步都需要完整的 SNARK 证明                                │ │
│  │  开销: O(|C|) × 步数                                          │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  Nova 折叠:                                                         │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                                                               │ │
│  │  (u₁, w₁) ──┐                                                 │ │
│  │             ├──► Fold ──► (u', w')  累积实例                  │ │
│  │  (u₂, w₂) ──┘                                                 │ │
│  │                                                               │ │
│  │  折叠不需要证明,只需要简单的线性组合!                         │ │
│  │  开销: O(1) 每步                                              │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  性能对比:                                                          │
│  ┌─────────────────────┬────────────────┬────────────────┐        │
│  │ 方案                │ 每步证明时间   │ 验证器开销     │        │
│  ├─────────────────────┼────────────────┼────────────────┤        │
│  │ Groth16 递归        │ ~秒级          │ O(|C|)         │        │
│  │ Halo2 递归          │ ~100ms         │ O(|C|)         │        │
│  │ Nova 折叠           │ ~毫秒级        │ O(1)           │        │
│  │ Nova + Spartan      │ ~10ms          │ O(1)           │        │
│  └─────────────────────┴────────────────┴────────────────┘        │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 11.2 R1CS 放松与折叠

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Relaxed R1CS 核心概念                            │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  标准 R1CS:                                                         │
│  A · z ⊙ B · z = C · z                                              │
│  其中 z = (1, x, w)  (公共输入 x, witness w)                        │
│                                                                     │
│  Relaxed R1CS:                                                      │
│  A · z ⊙ B · z = u · (C · z) + E                                   │
│                                                                     │
│  其中:                                                              │
│  • u: 标量 (放松因子)                                              │
│  • E: 误差向量                                                      │
│                                                                     │
│  当 u = 1, E = 0 时,等价于标准 R1CS                                │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │              Committed Relaxed R1CS                           │ │
│  │                                                               │ │
│  │  实例 (Instance): U = (Com(W), Com(E), u, x)                  │ │
│  │  witness: W = (w, E)                                          │ │
│  │                                                               │ │
│  │  Com(W), Com(E) 是多项式承诺                                  │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  折叠操作:                                                          │
│  给定两个实例-witness 对 (U₁, W₁) 和 (U₂, W₂)                      │
│                                                                     │
│  1. 计算交叉项 T = A·z₁ ⊙ B·z₂ + A·z₂ ⊙ B·z₁ - u₁·C·z₂ - u₂·C·z₁ │
│  2. 承诺交叉项: Com(T)                                             │
│  3. 收到随机挑战 r                                                 │
│  4. 计算折叠:                                                       │
│     • W' = W₁ + r · W₂                                             │
│     • E' = E₁ + r · T + r² · E₂                                    │
│     • u' = u₁ + r · u₂                                             │
│     • x' = x₁ + r · x₂                                             │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 11.3 Nova IVC 协议

```go
// Nova IVC 实现
package nova

import (
	"github.com/consensys/gnark-crypto/ecc"
)

// Relaxed R1CS 实例
type RelaxedR1CSInstance struct {
	CommW []byte     // Pedersen 承诺 witness
	CommE []byte     // 承诺误差向量
	U     *big.Int   // 放松因子
	X     []*big.Int // 公共输入
}

// Relaxed R1CS witness
type RelaxedR1CSWitness struct {
	W []FieldElement // witness 向量
	E []FieldElement // 误差向量
}

// Nova 证明者状态
type NovaProver struct {
	// 电路描述
	R1CS *R1CS

	// 当前累积实例
	RunningInstance *RelaxedR1CSInstance
	RunningWitness  *RelaxedR1CSWitness

	// 步骤计数
	Step int
}

// 初始化
func NewNovaProver(r1cs *R1CS) *NovaProver {
	return &NovaProver{
		R1CS: r1cs,
		RunningInstance: &RelaxedR1CSInstance{
			U: big.NewInt(1),
			X: make([]*big.Int, r1cs.NumPublicInputs),
			// CommW, CommE 初始化为零
		},
		RunningWitness: &RelaxedR1CSWitness{
			W: make([]FieldElement, r1cs.NumWitness),
			E: make([]FieldElement, r1cs.NumConstraints),
		},
		Step: 0,
	}
}

// 执行一步计算并折叠
func (p *NovaProver) Prove(input []FieldElement) (*NovaProof, error) {
	// Step 1: 执行电路得到新的 witness
	newWitness, err := p.R1CS.Compute(input, p.Step)
	if err != nil {
		return nil, err
	}

	// Step 2: 创建新实例
	newInstance := &RelaxedR1CSInstance{
		U: big.NewInt(1),
		X: publicInputsFromWitness(newWitness),
	}
	newInstance.CommW = Commit(newWitness.W)
	newInstance.CommE = Commit(make([]FieldElement, p.R1CS.NumConstraints)) // E = 0

	// Step 3: 计算交叉项 T
	T := p.computeCrossTerm(p.RunningWitness, &RelaxedR1CSWitness{
		W: newWitness.W,
		E: make([]FieldElement, p.R1CS.NumConstraints),
	})
	commT := Commit(T)

	// Step 4: 生成随机挑战 (Fiat-Shamir)
	r := HashToField(
		p.RunningInstance.CommW,
		p.RunningInstance.CommE,
		newInstance.CommW,
		commT,
	)

	// Step 5: 折叠
	foldedInstance, foldedWitness := p.fold(
		p.RunningInstance, p.RunningWitness,
		newInstance, &RelaxedR1CSWitness{W: newWitness.W, E: make([]FieldElement, p.R1CS.NumConstraints)},
		T, r,
	)

	// Step 6: 更新运行状态
	p.RunningInstance = foldedInstance
	p.RunningWitness = foldedWitness
	p.Step++

	return &NovaProof{
		CommT:       commT,
		NewInstance: newInstance,
		Step:        p.Step,
	}, nil
}

// 计算交叉项
func (p *NovaProver) computeCrossTerm(w1, w2 *RelaxedR1CSWitness) []FieldElement {
	z1 := constructZ(p.RunningInstance, w1)
	z2 := constructZ(&RelaxedR1CSInstance{U: big.NewInt(1)}, w2)

	// T = A·z₁ ⊙ B·z₂ + A·z₂ ⊙ B·z₁ - u₁·C·z₂ - u₂·C·z₁
	Az1 := p.R1CS.A.Mul(z1)
	Az2 := p.R1CS.A.Mul(z2)
	Bz1 := p.R1CS.B.Mul(z1)
	Bz2 := p.R1CS.B.Mul(z2)
	Cz1 := p.R1CS.C.Mul(z1)
	Cz2 := p.R1CS.C.Mul(z2)

	T := make([]FieldElement, p.R1CS.NumConstraints)
	for i := range T {
		T[i] = Az1[i].Mul(Bz2[i]).Add(Az2[i].Mul(Bz1[i]))
		T[i] = T[i].Sub(Cz2[i].Mul(p.RunningInstance.U))
		T[i] = T[i].Sub(Cz1[i]) // u₂ = 1
	}

	return T
}

// 折叠两个实例
func (p *NovaProver) fold(
	inst1 *RelaxedR1CSInstance, wit1 *RelaxedR1CSWitness,
	inst2 *RelaxedR1CSInstance, wit2 *RelaxedR1CSWitness,
	T []FieldElement, r *big.Int,
) (*RelaxedR1CSInstance, *RelaxedR1CSWitness) {

	// W' = W₁ + r · W₂
	newW := make([]FieldElement, len(wit1.W))
	for i := range newW {
		newW[i] = wit1.W[i].Add(wit2.W[i].Mul(r))
	}

	// E' = E₁ + r · T + r² · E₂
	r2 := new(big.Int).Mul(r, r)
	newE := make([]FieldElement, len(wit1.E))
	for i := range newE {
		newE[i] = wit1.E[i].Add(T[i].Mul(r)).Add(wit2.E[i].Mul(r2))
	}

	// u' = u₁ + r · u₂
	newU := new(big.Int).Add(inst1.U, new(big.Int).Mul(r, inst2.U))

	// 更新承诺 (同态性)
	newCommW := AddCommitments(inst1.CommW, ScalarMulCommitment(inst2.CommW, r))
	newCommE := AddCommitments(
		inst1.CommE,
		AddCommitments(
			ScalarMulCommitment(Commit(T), r),
			ScalarMulCommitment(inst2.CommE, r2),
		),
	)

	return &RelaxedR1CSInstance{
			CommW: newCommW,
			CommE: newCommE,
			U:     newU,
			X:     foldPublicInputs(inst1.X, inst2.X, r),
		}, &RelaxedR1CSWitness{
			W: newW,
			E: newE,
		}
}
```

### 11.4 Nova 验证器

```go
// Nova 验证器
type NovaVerifier struct {
R1CS *R1CS
}

// 验证 Nova IVC 证明
func (v *NovaVerifier) Verify(
initialInput []*big.Int,
finalOutput []*big.Int,
proofs []*NovaProof,
finalInstance *RelaxedR1CSInstance,
) bool {

// Step 1: 重放折叠过程 (只使用承诺)
runningInstance := &RelaxedR1CSInstance{
U:     big.NewInt(1),
X:     initialInput,
CommW: ZeroCommitment(),
CommE: ZeroCommitment(),
}

for _, proof := range proofs {
// 重新计算随机挑战
r := HashToField(
runningInstance.CommW,
runningInstance.CommE,
proof.NewInstance.CommW,
proof.CommT,
)

// 验证折叠 (只更新承诺)
runningInstance = v.foldInstances(runningInstance, proof.NewInstance, proof.CommT, r)
}

// Step 2: 验证最终实例匹配
if !CommitmentsEqual(runningInstance.CommW, finalInstance.CommW) {
return false
}
if !CommitmentsEqual(runningInstance.CommE, finalInstance.CommE) {
return false
}
if runningInstance.U.Cmp(finalInstance.U) != 0 {
return false
}

// Step 3: 最终执行一次 SNARK 证明验证
// 验证最终累积实例满足 Relaxed R1CS
return v.verifySNARK(finalInstance)
}

// 最终 SNARK 验证 (使用 Spartan)
func (v *NovaVerifier) verifySNARK(instance *RelaxedR1CSInstance) bool {
// 使用 Spartan 或其他 SNARK 验证 Relaxed R1CS
// 只需要一次完整的 SNARK 证明
return true
}
```

### 11.5 SuperNova: 多指令折叠

```
┌─────────────────────────────────────────────────────────────────────┐
│                      SuperNova 架构                                 │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Nova 的局限: 只支持单一电路的迭代                                  │
│                                                                     │
│  SuperNova 扩展: 支持多个不同电路的选择性执行                       │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                                                               │ │
│  │  电路集合 F = {F₁, F₂, ..., Fₖ}                               │ │
│  │                                                               │ │
│  │  每步选择一个电路执行:                                        │ │
│  │                                                               │ │
│  │  Step 1: F₂(x₀) → x₁                                         │ │
│  │  Step 2: F₁(x₁) → x₂                                         │ │
│  │  Step 3: F₃(x₂) → x₃                                         │ │
│  │  ...                                                          │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  应用场景:                                                          │
│  • zkVM: 不同操作码对应不同电路                                     │
│  • 智能合约: 不同函数对应不同电路                                   │
│  • 状态机: 不同状态转换对应不同电路                                 │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

```rust
// SuperNova 实现 (Rust 风格)
use std::collections::HashMap;

/// SuperNova 证明者
pub struct SuperNovaProver<F: Field> {
    /// 电路集合
    circuits: Vec<R1CS<F>>,

    /// 每个电路的累积实例
    running_instances: Vec<RelaxedR1CSInstance<F>>,

    /// 当前步骤
    step: usize,
}

impl<F: Field> SuperNovaProver<F> {
    /// 执行一步并折叠
    pub fn prove_step(
        &mut self,
        circuit_index: usize,  // 选择哪个电路
        input: Vec<F>,
    ) -> SuperNovaProof<F> {
        assert!(circuit_index < self.circuits.len());

        // 获取选中的电路
        let circuit = &self.circuits[circuit_index];

        // 执行电路
        let witness = circuit.compute(&input);

        // 创建新实例
        let new_instance = RelaxedR1CSInstance::new_standard(&witness);

        // 与该电路的运行实例折叠
        let running = &mut self.running_instances[circuit_index];
        let (cross_term, proof) = self.fold(running, &new_instance, &witness);

        self.step += 1;

        SuperNovaProof {
            circuit_index,
            cross_term_commitment: commit(&cross_term),
            new_instance_commitment: new_instance.commitment(),
        }
    }

    /// NIVC (Non-uniform IVC) 验证
    pub fn verify(
        &self,
        proofs: &[SuperNovaProof<F>],
        initial_input: Vec<F>,
        final_outputs: &[RelaxedR1CSInstance<F>],
    ) -> bool {
        // 重放所有折叠步骤
        let mut running: Vec<RelaxedR1CSInstance<F>> =
            (0..self.circuits.len())
                .map(|_| RelaxedR1CSInstance::trivial())
                .collect();

        for proof in proofs {
            let idx = proof.circuit_index;
            // 重新计算挑战
            let r = hash_to_field(&[
                &running[idx].commitment(),
                &proof.new_instance_commitment,
                &proof.cross_term_commitment,
            ]);

            // 更新运行实例
            running[idx] = running[idx].fold_commitment(
                &proof.new_instance_commitment,
                &proof.cross_term_commitment,
                r,
            );
        }

        // 验证最终状态
        for (i, final_inst) in final_outputs.iter().enumerate() {
            if running[i].commitment() != final_inst.commitment() {
                return false;
            }
        }

        // 对每个非平凡的累积实例执行 SNARK 验证
        for (i, inst) in final_outputs.iter().enumerate() {
            if !inst.is_trivial() {
                if !self.verify_snark(&self.circuits[i], inst) {
                    return false;
                }
            }
        }

        true
    }
}
```

### 11.6 Nova 在 zkVM 中的应用

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Nova-based zkVM 架构                             │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                      VM 执行                                  │ │
│  │                                                               │ │
│  │  指令序列: [ADD, MUL, LOAD, STORE, JUMP, ...]                │ │
│  │                                                               │ │
│  │  每条指令是一个"步骤",使用 Nova 折叠                         │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                              ↓                                      │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                    单步电路                                   │ │
│  │                                                               │ │
│  │  状态 S = (PC, Registers, Memory Root)                       │ │
│  │                                                               │ │
│  │  F(S, instr) = S'                                            │ │
│  │                                                               │ │
│  │  电路验证:                                                    │ │
│  │  1. 指令解码正确                                             │ │
│  │  2. 操作数获取正确                                           │ │
│  │  3. 运算结果正确                                             │ │
│  │  4. 状态更新正确                                             │ │
│  │  5. Memory Root 更新正确 (Merkle 证明)                       │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                              ↓                                      │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                    Nova 折叠                                  │ │
│  │                                                               │ │
│  │  (U₀, W₀) → 执行指令 1 → Fold → (U₁, W₁)                    │ │
│  │           → 执行指令 2 → Fold → (U₂, W₂)                    │ │
│  │           → ...                                              │ │
│  │           → 执行指令 N → Fold → (Uₙ, Wₙ)                    │ │
│  │                                                               │ │
│  │  每步只需 ~10ms,比传统 SNARK 快 100x                        │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                              ↓                                      │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                    最终压缩                                   │ │
│  │                                                               │ │
│  │  使用 Spartan 或 Groth16 压缩最终累积实例                    │ │
│  │  生成可在链上验证的简洁证明                                  │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  性能数据 (单线程):                                                 │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 每步折叠时间: ~5-15ms                                    │   │
│  │  • 1000 步总时间: ~10 秒                                    │   │
│  │  • 最终压缩时间: ~1 秒                                      │   │
│  │  • 证明大小: ~10KB (压缩前) / ~200B (压缩后)                │   │
│  │  • 验证时间: ~5ms                                           │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 11.7 HyperNova: 高度优化的折叠

```rust
// HyperNova: 使用 CCS (Customizable Constraint System)
// 比 R1CS 更通用,支持更高效的约束表示

/// CCS 结构 (替代 R1CS)
pub struct CCS<F: Field> {
    /// 矩阵数量
    pub t: usize,

    /// 约束度数
    pub q: usize,

    /// 稀疏矩阵
    pub matrices: Vec<SparseMatrix<F>>,

    /// 多线性扩展系数
    pub coefficients: Vec<F>,

    /// 约束组合
    pub selector: Vec<Vec<usize>>,
}

// CCS 约束:
// ∑ⱼ cⱼ · ∏ᵢ∈Sⱼ (Mᵢ · z) = 0
//
// 其中:
// - cⱼ: 系数
// - Sⱼ: 选择器 (选择哪些矩阵相乘)
// - Mᵢ: 稀疏矩阵

// R1CS 是 CCS 的特例:
// A · z ⊙ B · z - C · z = 0
// 等价于: c₁(M₀·z ⊙ M₁·z) + c₂(M₂·z) = 0
// 其中 c₁ = 1, c₂ = -1, S₁ = {0,1}, S₂ = {2}

/// Linearized CCS (LCCCS) - 折叠目标
pub struct LCCCS<F: Field> {
    pub commitment: Commitment,
    pub x: Vec<F>,           // 公共输入
    pub u: F,                // 放松因子
    pub r: Vec<F>,           // 随机点
    pub v: Vec<F>,           // 多线性求值
}

/// HyperNova 折叠
impl<F: Field> HyperNova<F> {
    pub fn fold(
        cccs: &CCCS<F>,    // 新实例
        lcccs: &LCCCS<F>,  // 运行实例
    ) -> (LCCCS<F>, F) {
        // 使用 sumcheck 协议进行折叠
        // 比 Nova 更高效,支持更高度的约束

        // Step 1: 随机线性组合
        let rho = random_field_element();

        // Step 2: 运行 sumcheck
        let (claim, rx, ry) = sumcheck(cccs, lcccs, rho);

        // Step 3: 折叠承诺
        let new_commitment = lcccs.commitment + cccs.commitment * rho;

        // Step 4: 更新多线性求值
        let new_v = fold_mle_evals(&lcccs.v, &cccs.v, rho);

        (
            LCCCS {
                commitment: new_commitment,
                x: fold_vec(&lcccs.x, &cccs.x, rho),
                u: lcccs.u + cccs.u * rho,
                r: rx,
                v: new_v,
            },
            claim,
        )
    }
}

// HyperNova 优势:
// 1. 支持高度约束 (d > 2)
// 2. 更好的并行性
// 3. 更小的证明开销
// 4. 与 Lookup Arguments 更好的集成
```

### 11.8 折叠方案对比

```
┌─────────────────────────────────────────────────────────────────────┐
│                    折叠方案对比总结                                 │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌─────────────┬────────────┬────────────┬────────────┬──────────┐ │
│  │ 特性        │ Nova       │ SuperNova  │ HyperNova  │ Protostar│ │
│  ├─────────────┼────────────┼────────────┼────────────┼──────────┤ │
│  │ 约束系统    │ R1CS       │ R1CS       │ CCS        │ Plonkish │ │
│  │ 多电路支持  │ 单一       │ 多个       │ 多个       │ 多个     │ │
│  │ 约束度数    │ 2          │ 2          │ 任意       │ 任意     │ │
│  │ Lookup      │ 需扩展     │ 需扩展     │ 原生支持   │ 原生支持 │ │
│  │ 每步开销    │ O(n)       │ O(n)       │ O(n)       │ O(n)     │ │
│  │ 验证器开销  │ O(1)       │ O(k)       │ O(1)       │ O(1)     │ │
│  │ 实现成熟度  │ 高         │ 中         │ 中         │ 低       │ │
│  └─────────────┴────────────┴────────────┴────────────┴──────────┘ │
│                                                                     │
│  开源实现:                                                          │
│  • Nova/SuperNova: https://github.com/microsoft/Nova               │
│  • HyperNova: https://github.com/privacy-scaling-explorations      │
│  • Arecibo (Nova fork): https://github.com/lurk-lab/arecibo        │
│  • Sonobe (多后端): https://github.com/privacy-scaling-explorations │
│                                                                     │
│  使用建议:                                                          │
│  • 简单 IVC: Nova                                                   │
│  • zkVM: SuperNova 或 HyperNova                                    │
│  • Plonk 兼容: Protostar (实验性)                                  │
│  • 生产环境: Nova + Spartan                                        │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

## Part 12: Cairo 语言

### 12.1 Cairo 概述

Cairo 是 StarkWare 开发的专为 STARK 证明系统设计的图灵完备语言。

```
┌─────────────────────────────────────────────────────────────────────┐
│                       Cairo 语言特性                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐ │
│  │  专为 ZK 设计   │    │   Rust 风格    │    │   高效证明      │ │
│  │                 │    │                 │    │                 │ │
│  │ 原生支持 felt  │    │ 类型安全       │    │ 优化的 AIR     │ │
│  │ 不可变内存     │    │ 所有权系统     │    │ 自动 trace     │ │
│  └─────────────────┘    └─────────────────┘    └─────────────────┘ │
│                                                                     │
│  Cairo 1.0 vs Cairo 0.x:                                           │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                                                             │   │
│  │  Cairo 0.x          →    Cairo 1.0 (Sierra → CASM)         │   │
│  │  • 低级语言              • 高级语言 (类似 Rust)            │   │
│  │  • 手动内存管理          • 自动内存管理                    │   │
│  │  • 容易写错              • 类型安全,编译时检查            │   │
│  │  • 直接生成 CASM        • Sierra 中间层保证安全           │   │
│  │                                                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  编译流程:                                                          │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                                                               │ │
│  │  Cairo 1.0  →  Sierra  →  CASM  →  AIR  →  STARK 证明       │ │
│  │   (高级)      (安全IR)   (Cairo ASM)  (代数中间表示)         │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 12.2 Cairo 基础语法

```cairo
// Cairo 1.0 基础语法示例

// 1. 变量和类型
fn basics() {
    // felt252 是 Cairo 的原生域元素类型
    let x: felt252 = 100;
    let y: u256 = 1000_u256;

    // 不可变绑定 (默认)
    let a = 10;
    // 可变绑定
    let mut b = 20;
    b = 30;

    // 元组
    let point: (felt252, felt252) = (10, 20);
    let (px, py) = point;

    // 数组 (Cairo 中数组是不可变的)
    let arr = array![1, 2, 3, 4, 5];

    // Option 和 Result
    let opt: Option<u32> = Option::Some(42);
    let res: Result<u32, felt252> = Result::Ok(42);
}

// 2. 函数
fn add(a: felt252, b: felt252) -> felt252 {
    a + b  // 最后一个表达式是返回值
}

fn fibonacci(n: felt252) -> felt252 {
    if n == 0 {
        return 0;
    }
    if n == 1 {
        return 1;
    }
    fibonacci(n - 1) + fibonacci(n - 2)
}

// 3. 结构体
#[derive(Drop, Copy)]
struct Point {
    x: felt252,
    y: felt252,
}

impl PointImpl of PointTrait {
    fn new(x: felt252, y: felt252) -> Point {
        Point { x, y }
    }

    fn distance_squared(self: @Point, other: @Point) -> felt252 {
        let dx = *self.x - *other.x;
        let dy = *self.y - *other.y;
        dx * dx + dy * dy
    }
}

// 4. 枚举
#[derive(Drop)]
enum Message {
    Quit,
    Move: (i32, i32),
    Write: ByteArray,
    ChangeColor: (u8, u8, u8),
}

fn process_message(msg: Message) {
    match msg {
        Message::Quit => { /* 处理退出 */ },
        Message::Move((x, y)) => { /* 移动到 (x, y) */ },
        Message::Write(text) => { /* 写入文本 */ },
        Message::ChangeColor((r, g, b)) => { /* 改变颜色 */ },
    }
}

// 5. 泛型
fn swap<T, +Drop<T>, +Copy<T>>(a: T, b: T) -> (T, T) {
    (b, a)
}

// 6. Trait
trait Calculable<T> {
    fn calculate(self: @T) -> felt252;
}

impl PointCalculable of Calculable<Point> {
    fn calculate(self: @Point) -> felt252 {
        *self.x + *self.y
    }
}
```

### 12.3 Cairo 智能合约

```cairo
// StarkNet 合约示例

#[starknet::interface]
trait ICounter<TContractState> {
    fn get_count(self: @TContractState) -> u256;
    fn increment(ref self: TContractState);
    fn decrement(ref self: TContractState);
    fn increment_by(ref self: TContractState, amount: u256);
}

#[starknet::contract]
mod Counter {
    use starknet::ContractAddress;
    use starknet::get_caller_address;

    // 存储变量
    #[storage]
    struct Storage {
        count: u256,
        owner: ContractAddress,
    }

    // 事件
    #[event]
    #[derive(Drop, starknet::Event)]
    enum Event {
        CountChanged: CountChanged,
    }

    #[derive(Drop, starknet::Event)]
    struct CountChanged {
        #[key]
        caller: ContractAddress,
        old_value: u256,
        new_value: u256,
    }

    // 构造函数
    #[constructor]
    fn constructor(ref self: ContractState, initial_count: u256) {
        self.count.write(initial_count);
        self.owner.write(get_caller_address());
    }

    // 实现接口
    #[abi(embed_v0)]
    impl CounterImpl of super::ICounter<ContractState> {
        fn get_count(self: @ContractState) -> u256 {
            self.count.read()
        }

        fn increment(ref self: ContractState) {
            let old_value = self.count.read();
            let new_value = old_value + 1;
            self.count.write(new_value);

            self.emit(CountChanged {
                caller: get_caller_address(),
                old_value,
                new_value,
            });
        }

        fn decrement(ref self: ContractState) {
            let old_value = self.count.read();
            assert(old_value > 0, 'Count cannot be negative');
            let new_value = old_value - 1;
            self.count.write(new_value);

            self.emit(CountChanged {
                caller: get_caller_address(),
                old_value,
                new_value,
            });
        }

        fn increment_by(ref self: ContractState, amount: u256) {
            let old_value = self.count.read();
            let new_value = old_value + amount;
            self.count.write(new_value);

            self.emit(CountChanged {
                caller: get_caller_address(),
                old_value,
                new_value,
            });
        }
    }

    // 内部函数
    #[generate_trait]
    impl InternalFunctions of InternalFunctionsTrait {
        fn only_owner(self: @ContractState) {
            let caller = get_caller_address();
            let owner = self.owner.read();
            assert(caller == owner, 'Only owner can call');
        }
    }
}
```

### 12.4 Cairo 高级特性

```cairo
// 1. 哈希计算
use core::hash::{HashStateTrait, HashStateExTrait};
use core::pedersen::PedersenTrait;
use core::poseidon::PoseidonTrait;

fn hash_examples() {
    // Pedersen 哈希 (StarkNet 默认)
    let pedersen_hash = PedersenTrait::new(0)
        .update(100)
        .update(200)
        .finalize();

    // Poseidon 哈希 (更高效)
    let poseidon_hash = PoseidonTrait::new()
        .update(100)
        .update(200)
        .finalize();
}

// 2. 签名验证
use starknet::secp256k1::{Secp256k1Point, Signature};
use starknet::eth_signature::verify_eth_signature;

fn verify_signature(
    eth_address: EthAddress,
    msg_hash: u256,
    signature: Signature
) -> bool {
    verify_eth_signature(msg_hash, signature, eth_address)
}

// 3. 递归和循环优化
// Cairo 中循环被展开为递归,需要注意 gas 消耗

fn sum_array(arr: Span<felt252>) -> felt252 {
    let mut sum: felt252 = 0;
    let mut i: usize = 0;
    loop {
        if i >= arr.len() {
            break;
        }
        sum = sum + *arr.at(i);
        i += 1;
    };
    sum
}

// 使用迭代器更优雅
fn sum_array_iter(arr: Span<felt252>) -> felt252 {
    let mut sum: felt252 = 0;
    for elem in arr {
        sum = sum + *elem;
    };
    sum
}

// 4. 字典 (Felt252Dict)
use core::dict::Felt252Dict;

fn dict_example() {
    let mut dict: Felt252Dict<u64> = Default::default();

    // 插入
    dict.insert('key1', 100);
    dict.insert('key2', 200);

    // 读取
    let value = dict.get('key1');  // 返回 100

    // 注意: Cairo 字典是不可变的
    // 每次操作返回新的字典
}

// 5. 组件系统 (Cairo 1.0)
#[starknet::component]
mod ownable_component {
    use starknet::ContractAddress;
    use starknet::get_caller_address;

    #[storage]
    struct Storage {
        owner: ContractAddress,
    }

    #[event]
    #[derive(Drop, starknet::Event)]
    enum Event {
        OwnershipTransferred: OwnershipTransferred,
    }

    #[derive(Drop, starknet::Event)]
    struct OwnershipTransferred {
        previous_owner: ContractAddress,
        new_owner: ContractAddress,
    }

    #[generate_trait]
    impl InternalImpl<
        TContractState, +HasComponent<TContractState>
    > of InternalTrait<TContractState> {
        fn initializer(ref self: ComponentState<TContractState>, owner: ContractAddress) {
            self.owner.write(owner);
        }

        fn assert_only_owner(self: @ComponentState<TContractState>) {
            let caller = get_caller_address();
            let owner = self.owner.read();
            assert(caller == owner, 'Not owner');
        }

        fn transfer_ownership(
            ref self: ComponentState<TContractState>,
            new_owner: ContractAddress
        ) {
            self.assert_only_owner();
            let previous = self.owner.read();
            self.owner.write(new_owner);
            self.emit(OwnershipTransferred {
                previous_owner: previous,
                new_owner,
            });
        }
    }
}
```

### 12.5 Cairo 底层: AIR 与 Trace

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Cairo VM 与 AIR                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Cairo VM 是一个特殊的虚拟机,执行过程生成代数执行轨迹 (Trace)      │
│                                                                     │
│  寄存器模型:                                                        │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │  PC  - 程序计数器                                             │ │
│  │  AP  - 分配指针 (Allocation Pointer)                          │ │
│  │  FP  - 帧指针 (Frame Pointer)                                 │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  内存模型:                                                          │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                                                               │ │
│  │  • 内存是只写的 (Write-Once)                                  │ │
│  │  • 一旦写入,值不可更改                                       │ │
│  │  • 这简化了 ZK 证明中的内存一致性检查                         │ │
│  │                                                               │ │
│  │  地址空间:                                                    │ │
│  │  [0, program_size) - 程序代码                                 │ │
│  │  [program_size, ...) - 执行内存                              │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  执行轨迹 (Execution Trace):                                        │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │  步骤  │  PC   │  AP   │  FP   │  指令  │  操作数 │  结果   │ │
│  │ ─────────────────────────────────────────────────────────────  │ │
│  │   0    │   0   │  100  │  100  │  ADD   │  a, b  │   c     │ │
│  │   1    │   1   │  101  │  100  │  MUL   │  c, d  │   e     │ │
│  │   2    │   2   │  102  │  100  │  JMP   │  10    │   -     │ │
│  │  ...   │  ...  │  ...  │  ...  │  ...   │  ...   │  ...    │ │
│  │                                                               │ │
│  │  轨迹被转换为多项式,然后使用 FRI 证明                         │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  AIR 约束 (以乘法为例):                                             │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                                                               │ │
│  │  约束: op0 × op1 = res                                       │ │
│  │                                                               │ │
│  │  转换为多项式约束:                                            │ │
│  │  P(x) = T_op0(x) × T_op1(x) - T_res(x) = 0 (在 trace 域上)  │ │
│  │                                                               │ │
│  │  其中 T_* 是轨迹多项式                                       │ │
│  │                                                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 12.6 Cairo 实战: ERC-20 代币

```cairo
#[starknet::interface]
trait IERC20<TContractState> {
    fn name(self: @TContractState) -> ByteArray;
    fn symbol(self: @TContractState) -> ByteArray;
    fn decimals(self: @TContractState) -> u8;
    fn total_supply(self: @TContractState) -> u256;
    fn balance_of(self: @TContractState, account: ContractAddress) -> u256;
    fn allowance(self: @TContractState, owner: ContractAddress, spender: ContractAddress) -> u256;
    fn transfer(ref self: TContractState, recipient: ContractAddress, amount: u256) -> bool;
    fn transfer_from(
        ref self: TContractState,
        sender: ContractAddress,
        recipient: ContractAddress,
        amount: u256
    ) -> bool;
    fn approve(ref self: TContractState, spender: ContractAddress, amount: u256) -> bool;
}

#[starknet::contract]
mod ERC20 {
    use starknet::ContractAddress;
    use starknet::get_caller_address;
    use core::num::traits::Zero;

    #[storage]
    struct Storage {
        name: ByteArray,
        symbol: ByteArray,
        decimals: u8,
        total_supply: u256,
        balances: LegacyMap<ContractAddress, u256>,
        allowances: LegacyMap<(ContractAddress, ContractAddress), u256>,
    }

    #[event]
    #[derive(Drop, starknet::Event)]
    enum Event {
        Transfer: Transfer,
        Approval: Approval,
    }

    #[derive(Drop, starknet::Event)]
    struct Transfer {
        #[key]
        from: ContractAddress,
        #[key]
        to: ContractAddress,
        value: u256,
    }

    #[derive(Drop, starknet::Event)]
    struct Approval {
        #[key]
        owner: ContractAddress,
        #[key]
        spender: ContractAddress,
        value: u256,
    }

    #[constructor]
    fn constructor(
        ref self: ContractState,
        name: ByteArray,
        symbol: ByteArray,
        decimals: u8,
        initial_supply: u256,
        recipient: ContractAddress
    ) {
        self.name.write(name);
        self.symbol.write(symbol);
        self.decimals.write(decimals);
        self._mint(recipient, initial_supply);
    }

    #[abi(embed_v0)]
    impl ERC20Impl of super::IERC20<ContractState> {
        fn name(self: @ContractState) -> ByteArray {
            self.name.read()
        }

        fn symbol(self: @ContractState) -> ByteArray {
            self.symbol.read()
        }

        fn decimals(self: @ContractState) -> u8 {
            self.decimals.read()
        }

        fn total_supply(self: @ContractState) -> u256 {
            self.total_supply.read()
        }

        fn balance_of(self: @ContractState, account: ContractAddress) -> u256 {
            self.balances.read(account)
        }

        fn allowance(
            self: @ContractState,
            owner: ContractAddress,
            spender: ContractAddress
        ) -> u256 {
            self.allowances.read((owner, spender))
        }

        fn transfer(ref self: ContractState, recipient: ContractAddress, amount: u256) -> bool {
            let sender = get_caller_address();
            self._transfer(sender, recipient, amount);
            true
        }

        fn transfer_from(
            ref self: ContractState,
            sender: ContractAddress,
            recipient: ContractAddress,
            amount: u256
        ) -> bool {
            let caller = get_caller_address();
            let current_allowance = self.allowances.read((sender, caller));
            assert(current_allowance >= amount, 'Insufficient allowance');

            self.allowances.write((sender, caller), current_allowance - amount);
            self._transfer(sender, recipient, amount);
            true
        }

        fn approve(ref self: ContractState, spender: ContractAddress, amount: u256) -> bool {
            let owner = get_caller_address();
            self.allowances.write((owner, spender), amount);
            self.emit(Approval { owner, spender, value: amount });
            true
        }
    }

    #[generate_trait]
    impl InternalImpl of InternalTrait {
        fn _transfer(
            ref self: ContractState,
            sender: ContractAddress,
            recipient: ContractAddress,
            amount: u256
        ) {
            assert(!sender.is_zero(), 'Transfer from zero address');
            assert(!recipient.is_zero(), 'Transfer to zero address');

            let sender_balance = self.balances.read(sender);
            assert(sender_balance >= amount, 'Insufficient balance');

            self.balances.write(sender, sender_balance - amount);
            self.balances.write(recipient, self.balances.read(recipient) + amount);

            self.emit(Transfer { from: sender, to: recipient, value: amount });
        }

        fn _mint(ref self: ContractState, recipient: ContractAddress, amount: u256) {
            assert(!recipient.is_zero(), 'Mint to zero address');

            self.total_supply.write(self.total_supply.read() + amount);
            self.balances.write(recipient, self.balances.read(recipient) + amount);

            self.emit(Transfer {
                from: Zero::zero(),
                to: recipient,
                value: amount,
            });
        }
    }
}
```

### 12.7 Cairo 开发工具链

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Cairo 开发工具                                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  编译器与工具:                                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • scarb: Cairo 包管理器 (类似 Cargo)                       │   │
│  │  • cairo-lang: 官方编译器                                   │   │
│  │  • starkli: StarkNet CLI 工具                               │   │
│  │  • katana: 本地开发网络                                     │   │
│  │  • protostar/foundry-starknet: 测试框架                     │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  项目结构:                                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  my_project/                                                │   │
│  │  ├── Scarb.toml          # 项目配置                         │   │
│  │  ├── src/                                                   │   │
│  │  │   ├── lib.cairo       # 库入口                           │   │
│  │  │   └── contracts/      # 合约目录                         │   │
│  │  │       └── counter.cairo                                  │   │
│  │  └── tests/              # 测试目录                         │   │
│  │      └── test_counter.cairo                                 │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  Scarb.toml 配置:                                                   │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  [package]                                                  │   │
│  │  name = "my_project"                                        │   │
│  │  version = "0.1.0"                                          │   │
│  │  cairo-version = "2.4.0"                                    │   │
│  │                                                             │   │
│  │  [dependencies]                                             │   │
│  │  starknet = "2.4.0"                                         │   │
│  │  openzeppelin = { git = "..." }                             │   │
│  │                                                             │   │
│  │  [[target.starknet-contract]]                               │   │
│  │  sierra = true                                              │   │
│  │  casm = true                                                │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  常用命令:                                                          │
│  • scarb build         # 编译项目                                  │
│  • scarb test          # 运行测试                                  │
│  • scarb fmt           # 格式化代码                                │
│  • starkli declare     # 声明合约类                                │
│  • starkli deploy      # 部署合约                                  │
│  • starkli invoke      # 调用合约                                  │
│  • starkli call        # 查询合约                                  │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

## Part 13: 硬件加速

### 13.1 ZK 证明硬件加速概述

```
┌─────────────────────────────────────────────────────────────────────┐
│                    ZK 证明计算瓶颈                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ZK 证明的主要计算密集型操作:                                       │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  1. 多标量乘法 (MSM)                         ~60-70%        │   │
│  │     ∑ᵢ sᵢ · Gᵢ  (标量与椭圆曲线点的乘法)                    │   │
│  │                                                             │   │
│  │  2. 快速傅里叶变换 (NTT/FFT)                 ~20-30%        │   │
│  │     多项式在系数形式和求值形式之间的转换                    │   │
│  │                                                             │   │
│  │  3. 哈希计算                                  ~5-10%        │   │
│  │     Poseidon, Pedersen, Keccak 等                          │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  加速方案对比:                                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                                                             │   │
│  │  方案          │ 加速比  │ 成本      │ 灵活性   │ 功耗     │   │
│  │ ─────────────────────────────────────────────────────────── │   │
│  │  CPU 多核      │ 4-16x   │ $100-1K   │ 高       │ 65-300W  │   │
│  │  GPU           │ 10-50x  │ $1K-10K   │ 中       │ 300-450W │   │
│  │  FPGA          │ 20-100x │ $5K-50K   │ 中       │ 50-200W  │   │
│  │  ASIC          │ 100+x   │ $1M+      │ 低       │ 50-500W  │   │
│  │                                                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 13.2 GPU 加速

```cuda
// CUDA MSM 实现框架
#include <cuda_runtime.h>

// 椭圆曲线点结构 (BN254)
struct ECPoint {
    uint64_t x[4];  // 256-bit
    uint64_t y[4];
    uint64_t z[4];  // 投影坐标
};

// 标量结构
struct Scalar {
    uint64_t limbs[4];  // 256-bit
};

// Pippenger MSM 算法 (GPU 版本)
__global__ void msm_bucket_accumulation(
    ECPoint* points,
    Scalar* scalars,
    ECPoint* buckets,
    int n,
    int window_size,
    int window_idx
) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;

    // 提取窗口内的标量位
    int bucket_idx = extract_window(scalars[tid], window_idx, window_size);
    if (bucket_idx == 0) return;

    // 原子加到对应桶
    // 使用投影坐标避免除法
    atomic_point_add(&buckets[bucket_idx - 1], &points[tid]);
}

__global__ void msm_bucket_reduction(
    ECPoint* buckets,
    ECPoint* result,
    int num_buckets
) {
    // 并行桶归约
    // sum = B[1] + 2*B[2] + 3*B[3] + ... + (2^w - 1)*B[2^w - 1]
    // 使用前缀和优化

    __shared__ ECPoint shared_buckets[256];

    // 复制到共享内存
    int tid = threadIdx.x;
    if (tid < num_buckets) {
        shared_buckets[tid] = buckets[tid];
    }
    __syncthreads();

    // 从后向前累加
    ECPoint running_sum = point_identity();
    ECPoint total = point_identity();

    for (int i = num_buckets - 1; i >= 0; i--) {
        running_sum = point_add(running_sum, shared_buckets[i]);
        total = point_add(total, running_sum);
    }

    if (tid == 0) {
        *result = total;
    }
}

// NTT (Number Theoretic Transform) GPU 实现
__global__ void ntt_butterfly(
    uint64_t* data,
    uint64_t* twiddles,
    int n,
    int stage
) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int half_n = n >> 1;
    if (tid >= half_n) return;

    int step = 1 << stage;
    int group = tid / step;
    int pos = tid % step;

    int i = 2 * group * step + pos;
    int j = i + step;

    // Cooley-Tukey 蝶形运算
    uint64_t twiddle = twiddles[group * step];
    uint64_t a = data[i];
    uint64_t b = mul_mod(data[j], twiddle);

    data[i] = add_mod(a, b);
    data[j] = sub_mod(a, b);
}

// 主机端 MSM 调用
void msm_gpu(
    ECPoint* h_points,
    Scalar* h_scalars,
    int n,
    ECPoint* result
) {
    const int WINDOW_SIZE = 16;
    const int NUM_WINDOWS = 256 / WINDOW_SIZE;
    const int NUM_BUCKETS = (1 << WINDOW_SIZE) - 1;

    // 分配设备内存
    ECPoint *d_points, *d_buckets;
    Scalar *d_scalars;

    cudaMalloc(&d_points, n * sizeof(ECPoint));
    cudaMalloc(&d_scalars, n * sizeof(Scalar));
    cudaMalloc(&d_buckets, NUM_BUCKETS * sizeof(ECPoint));

    // 复制数据到 GPU
    cudaMemcpy(d_points, h_points, n * sizeof(ECPoint), cudaMemcpyHostToDevice);
    cudaMemcpy(d_scalars, h_scalars, n * sizeof(Scalar), cudaMemcpyHostToDevice);

    ECPoint window_results[NUM_WINDOWS];

    for (int w = 0; w < NUM_WINDOWS; w++) {
        // 初始化桶
        cudaMemset(d_buckets, 0, NUM_BUCKETS * sizeof(ECPoint));

        // 桶累积
        int threads = 256;
        int blocks = (n + threads - 1) / threads;
        msm_bucket_accumulation<<<blocks, threads>>>(
            d_points, d_scalars, d_buckets, n, WINDOW_SIZE, w
        );

        // 桶归约
        ECPoint* d_window_result;
        cudaMalloc(&d_window_result, sizeof(ECPoint));
        msm_bucket_reduction<<<1, 256>>>(d_buckets, d_window_result, NUM_BUCKETS);

        cudaMemcpy(&window_results[w], d_window_result, sizeof(ECPoint),
                   cudaMemcpyDeviceToHost);
        cudaFree(d_window_result);
    }

    // 组合窗口结果 (在 CPU 上)
    *result = window_results[NUM_WINDOWS - 1];
    for (int w = NUM_WINDOWS - 2; w >= 0; w--) {
        for (int i = 0; i < WINDOW_SIZE; i++) {
            *result = point_double(*result);
        }
        *result = point_add(*result, window_results[w]);
    }

    cudaFree(d_points);
    cudaFree(d_scalars);
    cudaFree(d_buckets);
}
```

### 13.3 GPU ZK 证明库

```
┌─────────────────────────────────────────────────────────────────────┐
│                    主流 GPU 加速库                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1. ICICLE (Ingonyama)                                             │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 语言: CUDA + Rust/Go 绑定                                │   │
│  │  • 支持: MSM, NTT, Poseidon, Pedersen                       │   │
│  │  • 曲线: BN254, BLS12-377, BLS12-381, Grumpkin              │   │
│  │  • 开源: https://github.com/ingonyama-zk/icicle             │   │
│  │                                                             │   │
│  │  使用示例:                                                   │   │
│  │  ```rust                                                    │   │
│  │  use icicle_cuda_runtime::memory::HostOrDeviceSlice;        │   │
│  │  use icicle_bn254::curve::{G1Projective, ScalarField};      │   │
│  │  use icicle_bn254::msm;                                     │   │
│  │                                                             │   │
│  │  let result = msm::msm(                                     │   │
│  │      &scalars,                                              │   │
│  │      &points,                                               │   │
│  │      &msm::MSMConfig::default(),                            │   │
│  │  )?;                                                        │   │
│  │  ```                                                        │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  2. ec-gpu (Filecoin)                                              │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 语言: OpenCL + Rust                                      │   │
│  │  • 支持: MSM, FFT                                           │   │
│  │  • 特点: 跨平台 (NVIDIA/AMD)                                │   │
│  │  • 开源: https://github.com/filecoin-project/ec-gpu         │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  3. Blitzar (Space and Time)                                       │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 语言: CUDA + C++                                         │   │
│  │  • 支持: 内积证明, MSM                                      │   │
│  │  • 特点: 高度优化的批处理                                   │   │
│  │  • 开源: https://github.com/spaceandtimelabs/blitzar        │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  性能对比 (2^20 MSM, RTX 4090):                                    │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  库        │ BN254    │ BLS12-381 │ 内存使用                │   │
│  │ ──────────────────────────────────────────────────────────  │   │
│  │  ICICLE    │ 120ms    │ 180ms     │ 3.2 GB                  │   │
│  │  ec-gpu    │ 180ms    │ 250ms     │ 4.0 GB                  │   │
│  │  Blitzar   │ 150ms    │ 220ms     │ 3.5 GB                  │   │
│  │  CPU (单核)│ 8000ms   │ 12000ms   │ 0.5 GB                  │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 13.4 FPGA 加速

```verilog
// FPGA MSM 模块 (Verilog 伪代码)
module msm_engine #(
    parameter CURVE_BITS = 256,
    parameter WINDOW_SIZE = 16,
    parameter NUM_CORES = 16
) (
    input  wire                     clk,
    input  wire                     rst_n,

    // 输入接口
    input  wire [CURVE_BITS-1:0]    scalar_in,
    input  wire [2*CURVE_BITS-1:0]  point_in,
    input  wire                     valid_in,
    output wire                     ready_out,

    // 输出接口
    output reg  [2*CURVE_BITS-1:0]  result_out,
    output reg                      valid_out
);

    // 桶存储 (BRAM)
    reg [2*CURVE_BITS-1:0] buckets [0:(1<<WINDOW_SIZE)-2];

    // 点加法核心
    wire [2*CURVE_BITS-1:0] point_add_result [0:NUM_CORES-1];
    wire [NUM_CORES-1:0] point_add_valid;

    genvar i;
    generate
        for (i = 0; i < NUM_CORES; i = i + 1) begin : point_add_cores
            point_add_module #(
                .CURVE_BITS(CURVE_BITS)
            ) point_adder (
                .clk(clk),
                .rst_n(rst_n),
                .point_a(point_a[i]),
                .point_b(point_b[i]),
                .result(point_add_result[i]),
                .valid(point_add_valid[i])
            );
        end
    endgenerate

    // 模数乘法器 (蒙哥马利乘法)
    montgomery_mult #(
        .BITS(CURVE_BITS)
    ) mont_mult (
        .clk(clk),
        .a(mult_a),
        .b(mult_b),
        .n(curve_modulus),
        .result(mult_result)
    );

    // FSM 状态机
    localparam IDLE = 0, ACCUMULATE = 1, REDUCE = 2, OUTPUT = 3;
    reg [1:0] state;

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            state <= IDLE;
            valid_out <= 0;
        end else begin
            case (state)
                IDLE: begin
                    if (valid_in) begin
                        state <= ACCUMULATE;
                    end
                end

                ACCUMULATE: begin
                    // 提取窗口值,累加到对应桶
                    // ...
                end

                REDUCE: begin
                    // 桶归约
                    // ...
                end

                OUTPUT: begin
                    result_out <= final_result;
                    valid_out <= 1;
                    state <= IDLE;
                end
            endcase
        end
    end

endmodule

// 蒙哥马利模数乘法
module montgomery_mult #(
    parameter BITS = 256
) (
    input  wire             clk,
    input  wire [BITS-1:0]  a,
    input  wire [BITS-1:0]  b,
    input  wire [BITS-1:0]  n,
    output reg  [BITS-1:0]  result
);
    // 实现蒙哥马利乘法算法
    // T = a * b
    // m = (T mod R) * N' mod R
    // U = (T + m * N) / R
    // if U >= N then U - N else U

    reg [2*BITS-1:0] product;
    reg [BITS-1:0] m;
    reg [BITS:0] u;

    always @(posedge clk) begin
        product <= a * b;
        m <= (product[BITS-1:0] * n_prime) & ((1 << BITS) - 1);
        u <= (product + m * n) >> BITS;
        result <= (u >= n) ? (u - n) : u[BITS-1:0];
    end

endmodule
```

### 13.5 ZK 专用硬件

```
┌─────────────────────────────────────────────────────────────────────┐
│                    ZK 专用硬件方案                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1. Cysic                                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 类型: ASIC 芯片                                          │   │
│  │  • 目标: 通用 ZK 加速                                       │   │
│  │  • 特点: 专用 MSM/NTT 单元                                  │   │
│  │  • 状态: 开发中                                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  2. Ingonyama                                                       │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 类型: FPGA + GPU 混合方案                                │   │
│  │  • 产品: ICICLE (GPU), FPGA IP 核                           │   │
│  │  • 特点: 灵活的证明系统支持                                 │   │
│  │  • 状态: 生产可用                                           │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  3. Ulvetanna                                                       │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 类型: FPGA 加速器                                        │   │
│  │  • 目标: MSM 优化                                           │   │
│  │  • 特点: 高带宽内存接口                                     │   │
│  │  • 状态: 生产可用                                           │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  4. Supranational                                                   │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 类型: GPU/FPGA 软件库                                    │   │
│  │  • 产品: blst (BLS 签名), sppark (GPU 加速)                 │   │
│  │  • 特点: 高度优化的密码学原语                               │   │
│  │  • 开源: https://github.com/supranational                   │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  硬件选型指南:                                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                                                             │   │
│  │  场景                │ 推荐方案                             │   │
│  │ ──────────────────────────────────────────────────────────  │   │
│  │  开发/测试           │ CPU + 单 GPU                         │   │
│  │  中等吞吐量          │ 多 GPU (4-8 卡)                      │   │
│  │  高吞吐量            │ GPU 集群 或 FPGA                     │   │
│  │  超高吞吐量          │ 定制 ASIC                            │   │
│  │  低延迟              │ FPGA                                 │   │
│  │  成本敏感            │ GPU (性价比最优)                     │   │
│  │                                                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 13.6 证明生成集群架构

```
┌─────────────────────────────────────────────────────────────────────┐
│                    分布式证明生成架构                               │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                      任务调度层                               │ │
│  │  ┌───────────┐  ┌───────────┐  ┌───────────┐                 │ │
│  │  │ Sequencer │  │  Prover   │  │ Aggregator│                 │ │
│  │  │           │──│ Scheduler │──│           │                 │ │
│  │  └───────────┘  └───────────┘  └───────────┘                 │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                              ↓                                      │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                      计算层                                   │ │
│  │                                                               │ │
│  │  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐            │ │
│  │  │ Worker  │ │ Worker  │ │ Worker  │ │ Worker  │            │ │
│  │  │ (GPU×8) │ │ (GPU×8) │ │ (GPU×8) │ │ (GPU×8) │            │ │
│  │  └─────────┘ └─────────┘ └─────────┘ └─────────┘            │ │
│  │                                                               │ │
│  │  Worker 内部架构:                                             │ │
│  │  ┌─────────────────────────────────────────────────────────┐ │ │
│  │  │  ┌───────┐ ┌───────┐ ┌───────┐ ┌───────┐              │ │ │
│  │  │  │ GPU 0 │ │ GPU 1 │ │ GPU 2 │ │ GPU 3 │              │ │ │
│  │  │  │  MSM  │ │  MSM  │ │  NTT  │ │  NTT  │              │ │ │
│  │  │  └───────┘ └───────┘ └───────┘ └───────┘              │ │ │
│  │  │           ↓           ↓           ↓           ↓        │ │ │
│  │  │  ┌───────────────────────────────────────────────────┐│ │ │
│  │  │  │           高速 NVLink 互联                        ││ │ │
│  │  │  └───────────────────────────────────────────────────┘│ │ │
│  │  │  ┌───────┐ ┌───────┐ ┌───────┐ ┌───────┐              │ │ │
│  │  │  │ GPU 4 │ │ GPU 5 │ │ GPU 6 │ │ GPU 7 │              │ │ │
│  │  │  │ Hash  │ │ Hash  │ │ Verify│ │ Verify│              │ │ │
│  │  │  └───────┘ └───────┘ └───────┘ └───────┘              │ │ │
│  │  └─────────────────────────────────────────────────────────┘ │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                              ↓                                      │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                      存储层                                   │ │
│  │  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐     │ │
│  │  │ 状态数据库    │  │ 证明存储      │  │ 中间数据缓存  │     │ │
│  │  │ (PostgreSQL)  │  │ (S3/MinIO)    │  │ (Redis)       │     │ │
│  │  └───────────────┘  └───────────────┘  └───────────────┘     │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 13.7 优化策略总结

```
┌─────────────────────────────────────────────────────────────────────┐
│                    ZK 证明优化策略                                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  算法层面:                                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • MSM: Pippenger 算法, 窗口大小优化                        │   │
│  │  • NTT: 4-step FFT, 混合基算法                              │   │
│  │  • 多项式: Lagrange 基直接求值                              │   │
│  │  • 批处理: 多证明并行生成                                   │   │
│  │  • 预计算: SRS 点预处理, 查找表                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  硬件层面:                                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • GPU: 最大化 SM 利用率, 减少内存传输                      │   │
│  │  • FPGA: 流水线设计, 专用蒙哥马利乘法器                     │   │
│  │  • 内存: 使用高带宽内存 (HBM), 数据对齐                     │   │
│  │  • 互联: NVLink/InfiniBand 减少通信延迟                    │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  系统层面:                                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • 并行化: 任务级, 数据级, 指令级                           │   │
│  │  • 流水线: 重叠计算和 I/O                                   │   │
│  │  • 缓存: 减少重复计算, 预取数据                             │   │
│  │  • 调度: 负载均衡, 故障恢复                                 │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  性能指标参考 (2024):                                               │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                                                             │   │
│  │  目标: 2^24 约束 PLONK 证明                                 │   │
│  │                                                             │   │
│  │  • 单 CPU:   ~30 分钟                                       │   │
│  │  • 单 GPU:   ~30 秒                                         │   │
│  │  • 8 GPU:    ~5 秒                                          │   │
│  │  • FPGA:     ~10 秒                                         │   │
│  │  • 定制ASIC: ~1 秒 (理论)                                   │   │
│  │                                                             │   │
│  │  zkRollup 实际需求:                                         │   │
│  │  • 批次大小: 1000-5000 笔交易                               │   │
│  │  • 证明时间: < 5 分钟 (保持出块节奏)                        │   │
│  │  • 成本: < $0.01/交易                                       │   │
│  │                                                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 总结

本文档涵盖了零知识证明在区块链中的完整知识体系:

1. **基础理论**: ZK 三大性质、SNARKs vs STARKs、数学基础
2. **核心协议**: PLONK、FRI、Lookup Arguments
3. **实现框架**: Circom、gnark、Halo2
4. **高级技术**: 递归证明、折叠方案 (Nova/HyperNova)
5. **应用场景**: zkRollup、zkEVM、隐私交易
6. **开发实践**: Cairo 语言、电路设计
7. **性能优化**: GPU/FPGA 硬件加速

持续学习建议:

- 理论: 学习椭圆曲线密码学、多项式承诺（见附录 A）
- 实践: 从 Circom 入门,逐步深入 Halo2（见附录 B）
- 前沿: 关注 Nova、折叠方案、zkVM 发展

---

## 附录 A: 椭圆曲线密码学与多项式承诺

### A.1 椭圆曲线基础

#### A.1.1 椭圆曲线定义

```
椭圆曲线 (Weierstrass 形式):

    y² = x³ + ax + b  (mod p)

其中 4a³ + 27b² ≠ 0 (确保曲线非奇异)

点加法几何意义:
- 过两点 P, Q 作直线
- 与曲线交于第三点
- 取其关于 x 轴对称点即为 P + Q
```

#### A.1.2 ZK 常用曲线

| 曲线        | 特点               | 用途                  |
|-----------|------------------|---------------------|
| secp256k1 | y²=x³+7          | Bitcoin/Ethereum 签名 |
| BN254     | 配对友好             | 以太坊预编译              |
| BLS12-381 | 128-bit 安全       | Zcash/ETH2          |
| Pasta     | Pallas+Vesta 循环对 | Halo2               |
| Grumpkin  | BN254 嵌入曲线       | 递归证明                |

#### A.1.3 Go 实现椭圆曲线点运算

```go
package ecc

import "math/big"

// 有限域元素
type FieldElement struct {
	Value *big.Int
	Prime *big.Int
}

func NewFieldElement(v, p *big.Int) *FieldElement {
	return &FieldElement{Value: new(big.Int).Mod(v, p), Prime: p}
}

func (fe *FieldElement) Add(other *FieldElement) *FieldElement {
	r := new(big.Int).Add(fe.Value, other.Value)
	return NewFieldElement(r, fe.Prime)
}

func (fe *FieldElement) Mul(other *FieldElement) *FieldElement {
	r := new(big.Int).Mul(fe.Value, other.Value)
	return NewFieldElement(r, fe.Prime)
}

// 费马小定理求逆: a^(-1) = a^(p-2) mod p
func (fe *FieldElement) Inverse() *FieldElement {
	exp := new(big.Int).Sub(fe.Prime, big.NewInt(2))
	r := new(big.Int).Exp(fe.Value, exp, fe.Prime)
	return &FieldElement{Value: r, Prime: fe.Prime}
}

// 椭圆曲线点
type Point struct {
	X, Y     *FieldElement
	Curve    *Curve
	Infinity bool
}

// 曲线 y² = x³ + ax + b
type Curve struct {
	A, B  *FieldElement
	Prime *big.Int
	Order *big.Int
}

// 点加法
func (p *Point) Add(q *Point) *Point {
	if p.Infinity {
		return q
	}
	if q.Infinity {
		return p
	}

	// P + (-P) = O
	if p.X.Value.Cmp(q.X.Value) == 0 {
		sum := new(big.Int).Add(p.Y.Value, q.Y.Value)
		if sum.Mod(sum, p.Curve.Prime).Sign() == 0 {
			return &Point{Infinity: true, Curve: p.Curve}
		}
	}

	var slope *FieldElement
	if p.X.Value.Cmp(q.X.Value) == 0 {
		// 点倍乘: λ = (3x² + a) / (2y)
		three := NewFieldElement(big.NewInt(3), p.Curve.Prime)
		two := NewFieldElement(big.NewInt(2), p.Curve.Prime)
		num := three.Mul(p.X.Mul(p.X)).Add(p.Curve.A)
		slope = num.Mul(two.Mul(p.Y).Inverse())
	} else {
		// 点加法: λ = (y₂-y₁)/(x₂-x₁)
		dy := NewFieldElement(new(big.Int).Sub(q.Y.Value, p.Y.Value), p.Curve.Prime)
		dx := NewFieldElement(new(big.Int).Sub(q.X.Value, p.X.Value), p.Curve.Prime)
		slope = dy.Mul(dx.Inverse())
	}

	// x₃ = λ² - x₁ - x₂
	x3 := slope.Mul(slope)
	x3 = NewFieldElement(new(big.Int).Sub(x3.Value, p.X.Value), p.Curve.Prime)
	x3 = NewFieldElement(new(big.Int).Sub(x3.Value, q.X.Value), p.Curve.Prime)

	// y₃ = λ(x₁-x₃) - y₁
	y3 := slope.Mul(NewFieldElement(new(big.Int).Sub(p.X.Value, x3.Value), p.Curve.Prime))
	y3 = NewFieldElement(new(big.Int).Sub(y3.Value, p.Y.Value), p.Curve.Prime)

	return &Point{X: x3, Y: y3, Curve: p.Curve}
}

// 标量乘法 (Double-and-Add)
func (p *Point) ScalarMul(k *big.Int) *Point {
	result := &Point{Infinity: true, Curve: p.Curve}
	addend := p
	scalar := new(big.Int).Set(k)

	for scalar.Sign() > 0 {
		if scalar.Bit(0) == 1 {
			result = result.Add(addend)
		}
		addend = addend.Add(addend)
		scalar.Rsh(scalar, 1)
	}
	return result
}
```

### A.2 双线性配对

#### A.2.1 配对基础

```
双线性配对 e: G₁ × G₂ → Gₜ

性质:
1. 双线性: e(aP, bQ) = e(P, Q)^(ab)
2. 非退化: e(G₁, G₂) ≠ 1
3. 可计算: 存在高效算法

应用:
- KZG 多项式承诺验证
- BLS 签名聚合
- Groth16 证明验证
```

#### A.2.2 配对类型

| 类型       | 说明            | 曲线示例             |
|----------|---------------|------------------|
| Type I   | G₁ = G₂       | 超奇异曲线            |
| Type II  | G₁ ≠ G₂, 存在同态 | MNT 曲线           |
| Type III | G₁ ≠ G₂, 无同态  | BN254, BLS12-381 |

### A.3 多项式承诺

#### A.3.1 KZG 承诺方案

```
KZG 多项式承诺流程:

1. Setup (可信设置):
   - 选择秘密 τ
   - 生成 SRS: [G, τG, τ²G, ..., τⁿG]

2. Commit (承诺):
   - 多项式 f(x) = Σ cᵢxⁱ
   - 承诺 C = f(τ)·G = Σ cᵢ·(τⁱG)

3. Open (打开):
   - 在点 z 处求值 y = f(z)
   - 计算商多项式 q(x) = (f(x)-y)/(x-z)
   - 证明 π = q(τ)·G

4. Verify (验证):
   - 检查 e(C - yG, G) = e(π, τG - zG)
   - 即 e(C - yG, G) = e(π, (τ-z)G)
```

#### A.3.2 Go 实现 KZG

```go
package kzg

import (
	"math/big"
	bn254 "github.com/consensys/gnark-crypto/ecc/bn254"
)

type SRS struct {
	G1Powers []bn254.G1Affine // [G, τG, τ²G, ...]
	G2Gen    bn254.G2Affine   // G₂
	G2Tau    bn254.G2Affine   // τG₂
}

type Commitment struct {
	Point bn254.G1Affine
}

type Proof struct {
	Point bn254.G1Affine
}

// 承诺多项式
func Commit(srs *SRS, coeffs []*big.Int) *Commitment {
	var commitment bn254.G1Affine
	commitment.MultiExp(srs.G1Powers[:len(coeffs)], coeffs, nil)
	return &Commitment{Point: commitment}
}

// 生成证明
func CreateProof(srs *SRS, coeffs []*big.Int, z *big.Int) (*Proof, *big.Int) {
	// 计算 y = f(z)
	y := evaluatePoly(coeffs, z)

	// 计算商多项式 q(x) = (f(x) - y) / (x - z)
	quotient := divideByLinear(coeffs, z, y)

	// π = q(τ)·G
	var proof bn254.G1Affine
	proof.MultiExp(srs.G1Powers[:len(quotient)], quotient, nil)

	return &Proof{Point: proof}, y
}

// 验证证明
func Verify(srs *SRS, c *Commitment, proof *Proof, z, y *big.Int) bool {
	// e(C - yG, G₂) = e(π, τG₂ - zG₂)
	var yG, cMinusY bn254.G1Affine
	yG.ScalarMultiplication(&srs.G1Powers[0], y)
	cMinusY.Sub(&c.Point, &yG)

	var zG2, tauMinusZ bn254.G2Affine
	zG2.ScalarMultiplication(&srs.G2Gen, z)
	tauMinusZ.Sub(&srs.G2Tau, &zG2)

	// 配对检查
	left, _ := bn254.Pair([]bn254.G1Affine{cMinusY}, []bn254.G2Affine{srs.G2Gen})
	right, _ := bn254.Pair([]bn254.G1Affine{proof.Point}, []bn254.G2Affine{tauMinusZ})

	return left.Equal(&right)
}

func evaluatePoly(coeffs []*big.Int, x *big.Int) *big.Int {
	result := big.NewInt(0)
	xPow := big.NewInt(1)
	for _, c := range coeffs {
		term := new(big.Int).Mul(c, xPow)
		result.Add(result, term)
		xPow.Mul(xPow, x)
	}
	return result
}
```

#### A.3.3 承诺方案对比

| 方案           | 证明大小     | 验证时间     | 可信设置 | 适用场景       |
|--------------|----------|----------|------|------------|
| KZG          | O(1)     | O(1) 配对  | 需要   | PLONK, 以太坊 |
| FRI          | O(log²n) | O(log²n) | 无    | STARKs     |
| IPA          | O(log n) | O(n)     | 无    | Halo2      |
| Bulletproofs | O(log n) | O(n)     | 无    | 隐私交易       |

---

## 附录 B: 从 Circom 到 Halo2 学习路径

### B.1 学习路线图

```
阶段一: Circom 入门 (2-4周)
├── 基本语法与信号
├── 模板与组件
├── snarkjs 工具链
└── 简单应用 (哈希, Merkle)

阶段二: gnark 进阶 (2-4周)
├── Go 语言电路
├── 约束系统理解
├── 测试与调试
└── 实际应用开发

阶段三: Halo2 深入 (4-8周)
├── Plonkish 算术化
├── 自定义门设计
├── Lookup 表使用
└── 递归证明

阶段四: 专业方向
├── zkEVM 电路
├── zkVM 开发
└── 协议研究
```

### B.2 Circom 快速入门

#### B.2.1 基本语法

```circom
// 信号声明
pragma circom 2.0.0;

template Multiplier() {
    // 输入信号 (私有)
    signal input a;
    signal input b;

    // 输出信号 (公开)
    signal output c;

    // 约束: c = a * b
    c <== a * b;
}

// 主组件
component main = Multiplier();
```

#### B.2.2 范围证明

```circom
// 证明 n 位范围内
template RangeProof(n) {
    signal input value;
    signal bits[n];

    var sum = 0;
    for (var i = 0; i < n; i++) {
        bits[i] <-- (value >> i) & 1;
        bits[i] * (1 - bits[i]) === 0;  // 0 或 1
        sum += bits[i] * (1 << i);
    }

    sum === value;  // 重构检查
}
```

#### B.2.3 Merkle 证明

```circom
include "circomlib/poseidon.circom";

template MerkleProof(levels) {
    signal input leaf;
    signal input root;
    signal input pathElements[levels];
    signal input pathIndices[levels];

    signal hashes[levels + 1];
    hashes[0] <== leaf;

    component hashers[levels];

    for (var i = 0; i < levels; i++) {
        hashers[i] = Poseidon(2);

        // 根据 pathIndex 决定左右
        var left = (1 - pathIndices[i]) * hashes[i]
                 + pathIndices[i] * pathElements[i];
        var right = pathIndices[i] * hashes[i]
                  + (1 - pathIndices[i]) * pathElements[i];

        hashers[i].inputs[0] <== left;
        hashers[i].inputs[1] <== right;
        hashes[i + 1] <== hashers[i].out;
    }

    root === hashes[levels];
}
```

### B.3 gnark 进阶教程

#### B.3.1 基本电路

```go
package main

import (
	"github.com/consensys/gnark/frontend"
	"github.com/consensys/gnark/backend/groth16"
	"github.com/consensys/gnark-crypto/ecc"
)

// 电路定义
type MulCircuit struct {
	A frontend.Variable `gnark:"a"`
	B frontend.Variable `gnark:"b"`
	C frontend.Variable `gnark:",public"`
}

func (c *MulCircuit) Define(api frontend.API) error {
	result := api.Mul(c.A, c.B)
	api.AssertIsEqual(result, c.C)
	return nil
}

func main() {
	var circuit MulCircuit

	// 编译
	cs, _ := frontend.Compile(ecc.BN254.ScalarField(), r1cs.NewBuilder, &circuit)

	// Setup
	pk, vk, _ := groth16.Setup(cs)

	// 证明
	assignment := &MulCircuit{A: 3, B: 5, C: 15}
	witness, _ := frontend.NewWitness(assignment, ecc.BN254.ScalarField())
	proof, _ := groth16.Prove(cs, pk, witness)

	// 验证
	publicWitness, _ := witness.Public()
	groth16.Verify(proof, vk, publicWitness)
}
```

#### B.3.2 条件选择

```go
func (c *Circuit) Define(api frontend.API) error {
// 条件选择: if cond then a else b
result := api.Select(c.Cond, c.A, c.B)
api.AssertIsEqual(result, c.Expected)
return nil
}
```

#### B.3.3 位分解

```go
func (c *Circuit) Define(api frontend.API) error {
// 分解为 8 位
bits := api.ToBinary(c.Value, 8)

// 验证每一位
for _, bit := range bits {
api.AssertIsBoolean(bit)
}

// 重构
reconstructed := api.FromBinary(bits...)
api.AssertIsEqual(reconstructed, c.Value)
return nil
}
```

### B.4 Halo2 高级教程

#### B.4.1 Plonkish 核心概念

```
Halo2 电路结构:

┌─────────────────────────────────────────────────────────┐
│ Region (区域)                                           │
│ ┌──────┬──────┬──────┬────────┬─────────┐              │
│ │ a    │ b    │ c    │ selector│ fixed   │              │
│ ├──────┼──────┼──────┼────────┼─────────┤              │
│ │ x    │ y    │ z    │   1    │   0     │  <- Row 0    │
│ │ a    │ b    │ c    │   0    │   1     │  <- Row 1    │
│ └──────┴──────┴──────┴────────┴─────────┘              │
│                                                         │
│ 门约束: s_mul · (a · b - c) = 0                        │
│ 复制约束: 连接不同单元格                                 │
└─────────────────────────────────────────────────────────┘
```

#### B.4.2 Rust 简单电路

```rust
use halo2_proofs::{
    circuit::{Layouter, SimpleFloorPlanner, Value},
    plonk::{Circuit, ConstraintSystem, Error, Column, Advice, Selector},
    poly::Rotation,
};

#[derive(Clone)]
struct MulConfig {
    a: Column<Advice>,
    b: Column<Advice>,
    c: Column<Advice>,
    s_mul: Selector,
}

struct MulCircuit {
    a: Value<F>,
    b: Value<F>,
}

impl<F: Field> Circuit<F> for MulCircuit {
    type Config = MulConfig;
    type FloorPlanner = SimpleFloorPlanner;

    fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
        let a = meta.advice_column();
        let b = meta.advice_column();
        let c = meta.advice_column();
        let s_mul = meta.selector();

        meta.create_gate("mul", |meta| {
            let a = meta.query_advice(a, Rotation::cur());
            let b = meta.query_advice(b, Rotation::cur());
            let c = meta.query_advice(c, Rotation::cur());
            let s = meta.query_selector(s_mul);

            vec![s * (a * b - c)]
        });

        MulConfig { a, b, c, s_mul }
    }

    fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<F>) -> Result<(), Error> {
        layouter.assign_region(
            || "mul",
            |mut region| {
                config.s_mul.enable(&mut region, 0)?;

                let a = region.assign_advice(|| "a", config.a, 0, || self.a)?;
                let b = region.assign_advice(|| "b", config.b, 0, || self.b)?;
                let c_val = self.a.and_then(|a| self.b.map(|b| a * b));
                region.assign_advice(|| "c", config.c, 0, || c_val)?;

                Ok(())
            },
        )
    }
}
```

### B.5 学习资源

```
官方文档:
- Circom: https://docs.circom.io
- gnark: https://docs.gnark.consensys.io
- Halo2: https://zcash.github.io/halo2

教程:
- 0xPARC: ZK Learning Group
- RareSkills: ZK Book
- ZK Podcast

开源项目:
- circomlib: 标准电路库
- gnark-crypto: 密码学原语
- halo2-examples: 示例合集
```
